...
if ( $delete_all ) 
{
    ...
    $src_ip        = ( !empty($src_ip) )       ? $src_ip : null;
    $dst_ip        = ( !empty($dst_ip) )       ? $dst_ip : null;
    $from_date     = ( !empty($from_date) )    ? $from_date : null; 
    $to_date       = ( !empty($to_date) )      ? $to_date : null; 
    $sensor_query  = ( !empty($sensor_query) ) ? $sensor_query : null;
      
    Alarm::delete_from_backlog($conn, "all", null, $hide_closed, $src_ip, $dst_ip, $from_date, $to_date, $sensor_query);
    exit();
}
...
if ($only_delete) 
{
    ...
    for ($i = 1; $i <= $only_delete; $i++) 
	{
		...
		list ($list,$num_rows) = AlarmGroups::get_alarms ($conn,$sensor_query,$src_ip,$dst_ip,0,"",null,null,$timestamp,$timestamp_date,$name,"only_delete");
		/* already deleted in get_alarms
...
if ($only_close) 
{
	for ($i = 1; $i <= $only_close; $i++) 
	{
		...
		list ($list,$num_rows) = AlarmGroups::get_alarms ($conn,$sensor_query,$src_ip,$dst_ip,0,"",null,null,$from_date,$to_date,$name,"only_close");
		/* already closed in get_alarms
		foreach ($list as $s_alarm) {
			$s_backlog_id = $s_alarm->get_backlog_id();
			$s_event_id = $s_alarm->get_event_id();
...
if ($only_open) 
{
	for ($i = 1; $i <= $only_open; $i++) {
		...
		list ($list,$num_rows) = AlarmGroups::get_alarms ($conn,$sensor_query,$src_ip,$dst_ip,0,"",null,null,$from_date,$to_date,$name);
		foreach ($list as $s_alarm) {
			$s_backlog_id = $s_alarm->get_backlog_id();
			$s_event_id = $s_alarm->get_event_id();
...
list ($list,$num_rows) = AlarmGroups::get_alarms ($conn,$sensor_query,$src_ip,$dst_ip,$hide_closed,"",$from,$top,$from_date,$to_date,$name);

list($_sensors, $hosts, $icons) = Host::get_ips_and_hostname($conn,true);
...
<table class="transparent" width="100%">
    ...
    ?>
        <tr>
            ...
            <td style='text-align: center; background-color:#9DD131;font-weight:bold'><?=gettext("Start")."<br>".Util::timezone($tz)?></td>
    ...
    { // hidden header ?>
        <tr>
            ...
            <td style='text-align: center; background-color:transparent;font-weight:bold;color:transparent;border-bottom:0px'><?=gettext("Start Date")?></td>
    ...
    if ( empty($list) )
    {
        ?>
        <tr>
            <td class="nobborder center" colspan='11' style='height:40px; color: #4F4F4F;'><?php echo _("No alarm with these criteria")?></td>
        </tr>
        <?php
    }
    else
    {
        foreach ($list as $s_alarm) 
        {
            $bgcolor      = (++$i%2 == 0) ? "background-color:#FAFAFA;" : "background-color:#F2F2F2;";
            ...
            */
            $s_src_link = "../forensics/base_stat_ipaddr.php?clear_allcriteria=1&ip=$s_src_ip&hmenu=Forensics&smenu=Forensics";
            $src_title = gettext("Src Asset:")." <b>$s_asset_src</b><br>IP: <b>$s_src_ip</b>";
            $s_dst_link = "../forensics/base_stat_ipaddr.php?clear_allcriteria=1&ip=$s_dst_ip&hmenu=Forensics&smenu=Forensics";
			...
			// Reputation info
			$event_info       = Alarm::get_event($conn,$s_alarm->get_event_id());
			...
			if ($rep_src_bgcolor=="") $rep_src_bgcolor = $bgcolor;
			...
			if ($rep_dst_bgcolor=="") $rep_dst_bgcolor = $bgcolor;
            ...
            $homelan = (($match_cidr = Net::is_ip_in_cache_cidr($conn, $s_src_ip)) || in_array($s_src_ip, $hosts_ips)) ? " <a href='javascript:;' class='scriptinfo' style='text-decoration:none' ip='$s_src_ip'><img src=\"".Host::get_homelan_icon($s_src_ip,$icons,$match_cidr,$conn)."\" border=0></a>" : "";
            
            $source_link = "<a href='" . $s_src_link . "' title='" . $s_src_ip . "' >" . $s_src_name . "</a>:" . $s_src_port . " $s_src_img $src_country_img $homelan";
            
            $source_balloon  = "<div id=\"".$s_src_ip.";".$s_src_name."\" id2=\"".$s_src_ip.";".$s_dst_ip."\" class=\"HostReportMenu\">";
            $source_balloon .= $rep_src_icon.$source_link;
            $source_balloon .= "</div>";

            $homelan = (($match_cidr = Net::is_ip_in_cache_cidr($conn, $s_dst_ip)) || in_array($s_dst_ip, $hosts_ips)) ? " <a href='javascript:;' class='scriptinfo' style='text-decoration:none' ip='$s_dst_ip'><img src=\"".Host::get_homelan_icon($s_dst_ip,$icons,$match_cidr,$conn)."\" border=0></a>" : "";
                        
            $dest_link = "<a href='" . $s_dst_link . "' title='" . $s_dst_ip . "' >" . $s_dst_name . "</a>:" . $s_dst_port . " $s_dst_img $dst_country_img $homelan";
            
            $dest_balloon  = "<div id=\"".$s_dst_ip.";".$s_dst_name."\" id2=\"".$s_dst_ip.";".$s_src_ip."\" class=\"HostReportMenu\">";
            $dest_balloon .= $rep_dst_icon.$dest_link;
            $dest_balloon .= "</div>";
			...
			$timestamp_utc = Util::get_utc_unixtime($conn,$s_alarm->get_timestamp());
			$s_date        = gmdate("Y-m-d H:i:s",$timestamp_utc+(3600*$tz));
                                    
            //if ($s_backlog_id != 0 && $s_alarm->get_since()!="-")
            //if ($s_backlog_id && $s_id==1505 && $s_event_count>0) 
            if ($s_backlog_id && $s_id==1505 && $s_event_count>0)
            {
                $aux_date      = Util::timestamp2date($s_alarm->get_since());
                $timestamp_utc = Util::get_utc_unixtime($conn,$aux_date);
            ...
            if ( Session::menu_perms("MenuIncidents", "IncidentsOpen") )
                $incident_link  = '<a class="greybox2" title="'._("New alarm ticket").'" href="../incidents/newincident.php?nohmenu=1&' . "ref=Alarm&title=" . urlencode($s_alarm_name) . "&" . "priority=$s_risk&" . "src_ips=$s_src_ip&" . "event_start=$s_since&" . "event_end=$s_date&" . "src_ports=$s_src_port&" . "dst_ips=$s_dst_ip&" . "dst_ports=$s_dst_port" . '">' . '<img src="../pixmaps/script--pencil.png" alt="'._("New alarm ticket").'" border="0"/>' . '</a>';
            else
        ...
        ?>
            ...
            <tr>
                <td class="nobborder <?php echo (($event_ocurrences==1 && $s_backlog_id && $s_id==1505 && $event_ocurrences > 0) ? "auto_display":"")?>" style='<?php echo $bgcolor ?>text-align: center;padding-left:30px' width='3%' id="eventplus<?=$s_backlog_id . "-" . $s_event_id?>"><?php echo $expand_button ?></td>
                <td class="nobborder" style='<?php echo $bgcolor ?>text-align: center'><?=$checkbox?></td>
                <td class="nobborder" style='<?php echo $bgcolor ?>text-align: left; padding-left:10px' width='30%'><strong><?=$balloon_name?></strong></td>
                    <?=$risk_field?>
                    <!-- sensor -->
                    ...
                    {
                        ?>
                        <a href="../sensor/sensor_plugins.php?hmenu=Plugins&smenu=Plugins&sensor=<?php echo $sensor ?>">
                            <?php echo ($no_resolv) ? $sensor : Host::ip2hostname($conn, $sensor) ?>
                        </a>  
                        <?php
                    }
                    if (!count($sensors)) {
                ...
                <!-- end sensor -->
                <td class="nobborder" style='<?php echo $bgcolor ?>text-align: center' width='12%'><?=$s_since?></td>
                <td class="nobborder" style='<?php echo $bgcolor ?>text-align: center' width='12%'><?=$s_date?></td>
                <td class="nobborder" nowrap style='<?php echo $rep_src_bgcolor ?>text-align: center;'><?=$source_balloon?></td>
                <td class="nobborder" nowrap style='<?php echo $rep_dst_bgcolor ?>text-align: center;'><?=$dest_balloon?></td>
                <td class="nobborder" bgcolor='<?=(($s_status == "open") ? "#ECE1DC" : "#DEEBDB")?>' style='text-align: center; border-width: 0px;'><b><?=$status_link?></b></td>
                <td class="nobborder" style='<?php echo $bgcolor ?>text-align: center'><?=$s_delete_link." ".$incident_link?></td>
            </tr>
            <tr>
                <td class="nobborder"></td>
                <td class="nobborder" colspan='9' name='eventbox<?=$s_backlog_id . "-" . $s_event_id?>"' id='eventbox<?=$s_backlog_id . "-" . $s_event_id?>'></td>
            </tr>
            <?php 
        } 
