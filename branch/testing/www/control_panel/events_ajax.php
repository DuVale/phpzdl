	...
	foreach($alarm_list as $alarm) 
	{
		...
		<tr bgcolor="<?=$color?>">
			...
			<td style="text-align:left">
				...
				{
					$href = str_replace("//","/","$acid_link/" . $acid_prefix . "_qry_alert.php?submit=%230-%28" . "$snort_sid-$snort_cid%29");
					?>
					<div class="balloon">
						<?php
						...
						<a class="greybox trlnka" id="<?php echo $id.";".$sid ?>" title="<?=_("Event detail")?>" href="<?php echo $href_sim."&minimal_view=1" ?><? if (GET('box') == "") echo "&noback=1"?>"><?php echo $name ?></a>
							...
							{ 
								...
								<span class="tooltip">
									...
									<span class="middle ne11">
										<?=_("Src Asset")?>: <strong><?php echo $asset_src?></strong><br>
										<?=_("Dst Asset")?>: <strong><?php echo $asset_dst?></strong><br>
										<?=_("Priority")?>:  <strong><?php echo $sid_priority?></strong>
									</span>
			...
			<td nowrap='nowrap'>
				...
				{ 
					?>
					<a href="<?php echo Util::get_acid_date_link($date, $src_ip, "ip_src") ?>"><font color="black"><?php echo $date ?></font></a>
					<?php
				} 
				else 
				{ 
			...
			<td nowrap='nowrap'>
				<?php
				if ($src_country)
					echo "<a href=\"$src_link\" class=\"HostReportMenu\" id2=\"$src_ip;$dst_ip\" id=\"$src_ip;$src_name\">$src_name</a>:$src_port $src_img $src_country_img";
				else
					echo "<a href=\"$src_link\" class=\"HostReportMenu\" id2=\"$src_ip;$dst_ip\" id=\"$src_ip;$src_name\">$src_name</a>:$src_port $src_img"; ?>
			</td>
			
			<td nowrap='nowrap'>
				<?php
				if ($dst_country)
					echo "<a href=\"$dst_link\" class=\"HostReportMenu\" id2=\"$dst_ip;$src_ip\" id=\"$dst_ip;$dst_name\">$dst_name</a>:$dst_port $dst_img $dst_country_img";
				else
					echo "<a href=\"$dst_link\" class=\"HostReportMenu\" id2=\"$dst_ip;$src_ip\" id=\"$dst_ip;$dst_name\">$dst_name</a>:$dst_port $dst_img";
				?>
			</td>
			<!-- src & dst hosts -->
	...
	if ( $hide == "" ) 
	{
		...
		<tr>
			<td colspan="6" bgcolor="#eeeeee" style="text-align:left;padding-left:5px">
				<a href='events_ajax.php?backlog_id=<?=$backlog_id?>&show_all=2&box=1&hide=directive' class="greybox trlnka" id="<?php echo $id.";".$sid ?>" title="<?=_("Events detail")?>"><img src="../pixmaps/plus-small.png" align="absmiddle"><?php echo $summary["total_count"] - $summ_event_count; ?></a> <?php echo _("Total events matched after highest rule level, before timeout."); ?>
			</td>
