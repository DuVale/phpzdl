...
function html_incident() {
    ...
    if ( Session::menu_perms("MenuIncidents", "IncidentsOpen") )
	{
        ...
        $html = "<a class='greybox' title='"._("New metric ticket")."' href='../incidents/newincident.php?nohmenu=1&ref=Metric&" . "title=" . urlencode("$title ($target)") . "&" . "priority=$priority&" . "target=" . urlencode($target) . "&" . "metric_type=$type&" . "metric_value=$metric&" . "event_start=$max_date&" . "event_end=$max_date'>" . '<img src="../pixmaps/script--pencil.png" width="12" alt="'._("New metric ticket").'" border="0"/>' . '</a>';
    }
    else
    {
        $html = "<span class='disabled'><img src='../pixmaps/script--pencil-gray.png' width='12' alt='"._("New metric ticket")."' title='"._("New metric ticket")."' border='0'/></span>";
    }
...
function html_host_report($ip, $name, $title = '') {
    ...
    if ($host_report_perms) {
        return "<a href='../report/host_report.php?asset_type=host&asset_key=$ip' $title  id='$ip;$name' class='HostReportMenu'>
                $name</a>";
    } else {
