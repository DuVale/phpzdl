...
{
	$orph_users     = array();
	$orph_users_aux = Acl::get_orph_users($conn);
	
	foreach ($orph_users_aux as $orph_user)
	{
		$icon = ( !Session::is_admin($conn, $orph_user['login']) ) ? "../../pixmaps/user-green.png" : "../../pixmaps/user-gadmin.png";
		$orph_users[$orph_user['login']] = $icon;
	}
}
...
else if(preg_match("/^e_(.*)_netgroup$/",$key,$found))
{
    ...
    $net_groups    = Net_group::get_list($conn, "", "",  array_keys($entityPerms['sensors']));
    $netgroup_list = array();
    

    foreach($net_groups as $net_group) 
    {
        $tallowed=1; // to test netgroup permissions
	...
	foreach($netgroup_list as $netgroup_name) 
	{
        ...
        $li      = "key:'e_".$found[1]."_netgroup_".base64_encode($netgroup_name)."', isLazy:true, h:'$h', url:'../net/newnetgroupform.php?name=".urlencode($netgroup_name)."', icon:'../../pixmaps/theme/net_group.png', title:'$title', tooltip:'$tooltip'\n";
        $buffer .= (($j > 0) ? "," : "") . "{ $li }";
        $j++;
    }
...
else if(preg_match("/^e_(.*)_netgroup_(.*)/",$key)) {
    echo Net::draw_nets_by_class($conn, $key, $filter, $length_name);
}
...
else if(preg_match("/^ue_(.*)/",$key,$found))
{
	...
	foreach ($user_list as $k => $login)
	{
		...
		$u_key        = "u_".$login;
		$user_allowed = Acl::userAllowed($login); 
		...
		$li  = "title:'$title', icon:'$icon', tooltip:'$tooltip', noLink:true, key:'$u_key'";
		$li .= ( $user_allowed > 1 ) ? ", isLazy:true" : "";
...
else if(preg_match("/ou/",$key)) 
{
	...
	foreach ($orph_users as $k => $v)
	{
		...
		$li  = "title:'".$title."', tooltip:'$tooltip', icon:'$v', noLink:true, key:'$u_key'";
		$li .= ( Acl::userAllowed($k) > 1 ) ? ", isLazy:true" : "";
...
else 
{
   ...
   /* SIEM Components tree */
	...
	{
        echo ",{title:'<font style=\"font-weight:normal\">"._("Alienvault Components")."</font>', isFolder:true, icon:'../../pixmaps/theme/server_role.png', expand:true, children:[";
	...
	$my_entities    = Acl::get_my_entities($conn);
	...
	if ( $num_entities > 0 || ( Session::am_i_admin() && count($entities[0]) > 0) )
	{
		...
		foreach ($entities[0] as $entity)
		{
			...
			if ( $entity['parent_id'] > 0 || $entity['type'] <= 0 || $entity_allowed == 0 ) 
				continue;
            ...
            $entity_name  = ( preg_match('/&#(\d{4,5});/',$entity['name']) ) ? mb_convert_encoding($entity['name'],'UTF-8','HTML-ENTITIES') : $entity['name'] ;
            $e_sn    	  = ( strlen($entity_name) > $length_name )	? substr($entity_name, 0, $length_name)."..." : $entity_name;		
	...
	if ( $withusers && ( Session::am_i_admin() || !empty($orph_users[$me])) ) 
...
function echochildrens($entities,$parent_id, $withusers) {
	...
	$me                = Session::get_session_user();
	$entities_types    = Acl::get_entities_types($conn);
	$assigned_entities = Session::get_entities_to_assign($conn);
	
	if ( !empty($assigned_entities[$parent_id]) )
	{
	...
	if ( !empty($children) )
	{
		...
		foreach ($children as $index => $child_id)
		{
			...
			if ( $entity_flag && !empty($assigned_entities[$parent_id]) )
			{
				echo ", ";
				$entity_flag = false;
			}
            ...
            $child_sn   = ( strlen($child_name) > $length_name )	? substr($child_name, 0, $length_name)."..." : $child_name;	
			
            $ch_name    = Util::htmlentities($child_name);	
			...
			if ( $child['parent_id'] == $parent_id )
			{
				...
				echo "{title:'".$title."', h:'$h', ".$child_link.", url:'".$child_url."', tooltip:'$tooltip', key:'".$child_key."', icon:'$icon', expand:true, name:'$ch_name'";
					echochildrens($entities, $child_id, $withusers);
				echo "}";
			}	
		}
	}
