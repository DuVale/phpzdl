...
ossim_valid($sourcetype, OSS_ALPHA, OSS_SPACE, OSS_NULLABLE, OSS_SLASH, 'illegal:' . _("Source Type"));
if (ossim_error()) {
	die(ossim_error());
}
if ($category_id != "") {
...
<body>
	...
	<?php if ($cloud_instance && $av_features) {  $gheight=250; ?>
	...
	function action(com,grid) {
        ...
        if (com=='<?=_("Insert new event type")?>') {
			if (typeof(items[0]) != 'undefined')
				...
				<?php } ?>
			else alert('<?php echo Util::js_entities(_("You must select a data source"))?>');
		}
        	...
        	if (typeof(items[0]) != 'undefined')
                ...
                alert('<?php echo Util::js_entities(_("You must select a data source"))?>');
        }
	}
...
?>
			...
			searchitems : [
				...
				{display: 'Data Source ID', name : 'id'}
				],
