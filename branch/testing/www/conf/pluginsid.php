...
<head>
	<title> <?php echo gettext("Event types"); ?> </title>
	...
	<script type='text/javascript'>
		...
		function action(com,grid) {
			...
			if (com=='<?=("Edit")?>') {
				...
				else alert('<?php echo Util::js_entities(_("You must select a event type"))?>');
				...
				if (typeof(items[0]) != 'undefined')
                    ...
                    alert('<?php echo Util::js_entities(_("You must select a event type"))?>');
		...
		$("#flextable").flexigrid({
			...
			?>
				...
				searchitems : [
					...
					{display: 'Event type ID', name : 'sid' }
					],
