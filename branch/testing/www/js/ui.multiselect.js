...
$.widget("ui.multiselect", {
    ...
    **************************************/
	...
	_getOptionNode: function(option) {
        ...
        var long_name = option.text();
        var name = option.text();
        
        if ((this.options.sizelength != -1) && (option.text().length > this.options.sizelength))
            var name = option.text().substr(0,this.options.sizelength-3) + "...";
            
        long_name = long_name.replace(/\"/, '');

        var node = $('<li class="ui-state-default ui-element" title="' + long_name + '"><span class="ui-icon"/>' + name + '<a href="#" class="ui-state-default action"><span class="ui-corner-all ui-icon"/></a></li>').hide();
   ...
   // @param bool noclone (optional) true only if item should not be cloned on the other list
	...
	_applyItemState: function(item, selected) {
		if (selected) {
            ...
            //item.children('span').addClass('ui-helper-hidden').addClass('ui-icon-arrowthick-2-n-s');
            ...
            item.children('span').removeClass('ui-icon-arrowthick-2-n-s').addClass('ui-helper-hidden').removeClass('ui-icon');
            //item.children('span').addClass('ui-helper-hidden').removeClass('ui-icon').removeClass('ui-icon-arrowthick-2-n-s');
