...
function get_value(id)
{
	...
	function check_form()
	{
		...
		if ( check_session_expired(res) == true ) 
		{
			document.location.href = '../session/login.php';
			return;
		}
	...
	$.ajax({
		...
		success: function(html){
			...
			if ( isNaN(status) )
			{
				...
				if ( check_session_expired(html) == true ) 
				{
					document.location.href='../session/login.php';
					return;
				}
...
function check_session_expired(data)
{
	if ( data.match(/\<meta /i) )
		return true;
	else
		return false;
}
