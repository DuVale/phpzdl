...
function scan2html($scan) {
	...
	foreach($scan as $host) 
	{
        ...
        if ($host['mac_vendor']!="") $html.= "&nbsp;[" . $host['mac_vendor'] . "]";
        $html.= "</td>\n<td nowrap='nowrap'>" . $host['os'] . "&nbsp;";
		...
		$html.= "&nbsp</td>\n";
        $html.= "<td><input checked='checked' type=\"checkbox\" class=\"mc\"
    ...
    function m_checkboxes() {
        if( $('.mc:checked').length > 0 ) {
            $('.mc').attr('checked', '');
        }
        else {
            $('.mc').attr('checked', 'checked'); 
        }
    }
    </script>
	<form action="../host/newhostform.php" method="POST">
		...
		<table align="center" width="80%" cellpadding="2">
			<tr>
				...
				<th style='width:80px; padding: 3px;'><div><?php echo _("Insert")?></div><div style="padding-top:4px;"><a href="#" onclick="m_checkboxes();return false;"><span style="font-size:10px"><?php echo _("Select / Unselect all")?></span></a></div></th>
			</tr>
			<?php echo $html?>
			<tr>
