...
//Max length Net name
$length_name = ( !empty($_GET['length_name']) ) ? GET('length_name') : 15;
...
foreach ($all_nets as $net) 
{
	if ( ($entity > 0 && $current_entity_perms['assets'][$net->get_ips()] || empty($current_entity_perms['assets']) ) || $entity == 0 ) 
	{
        ...
        foreach($acidrs as $cidr) 
		{
            ...
            if($bclasses[$data[0].".".$data[1]]!=1)               $bclasses[$data[0].".".$data[1]] = 1;
            if($aclasses[$data[0]]!=1)                            $aclasses[$data[0]] = 1;
        }
...
if ($key=="") {
	...
	if(count($bclasses) <= $low_limit ) 
	{
        foreach($cclasses as $cclass => $v) 
		{
			...
			{
				if(preg_match("/$cclass\..*/",$net_cidrs)) 
				{
					$net_key = $net_name;
					...
					$tooltip = Util::htmlentities($net_name). " (". Net::get_cidrs_summary($conn, $net_name) .")";
					$url     = $tooltip;
					
					$buffer .= "{ key:'$net_key', isFolder:false, isLazy:false, icon:'../../pixmaps/theme/net.png', url:'$url', title:'$title', tooltip:'$tooltip'},";
				}
			}
        ...
        foreach($bclasses as $bclass => $v)
		{
           	$title   = $bclass.".---.---/--";
			$tooltip = $title;
			$buffer .= "{ key:'bclass_$bclass', isFolder:true, isLazy:true, icon:'../../pixmaps/theme/net.png', title:'$title', tooltip:'$tooltip'},";
		}
    }
    else 
	{
        foreach($aclasses as $aclass => $v)
		{
            $title   = $aclass.".---.---.---/--";
			$tooltip = $title;
			$buffer .= "{ key:'aclass_$aclass', isFolder:true, isLazy:true, icon:'../../pixmaps/theme/net.png', title:'$title', tooltip:'$tooltip'},";
		}
    }
	...
	if (preg_match("/children:\[\n\] \} \]$/", $buffer) ) 
		$buffer = "[{title:'<span>"._("No Networks Found")."<span>', key:'_no_nets_', noLink:true, icon:'../../pixmaps/theme/any.png', addClass:'grey_12'}]";
}
    ...
    foreach($cclasses as $cclass => $v)
	{
      	$title   = $cclass.".---/--";
		$tooltip = $title;
		$buffer .= "{ key:'cclass_$cclass', isFolder:true, isLazy:true, icon:'../../pixmaps/theme/net.png', title:'$title', tooltip:'$tooltip'},";
	}
    $buffer  = preg_replace("/,$/", "", $buffer);
    $buffer .= "]";
}
...
if ( $buffer == "" || $buffer == "[]" )
    $buffer = "[{title:'<span>"._("No Networks Found")."<span>', noLink:true, key:'_no_nets_', icon:'../../pixmaps/theme/any.png', addClass:'grey_12'}]";
