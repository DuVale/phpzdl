...
if (!Session::menu_perms("MenuConfiguration", "NetworkDiscovery"))
	Session::unallowed_section(null, "noback", "MenuConfiguration", "NetworkDiscovery");
...
if ($device_list[0]) {
    ...
    if ($total == 0) $total = count($device_list);
} else $total = 0;
$xml.= "<rows>\n";
$xml.= "<page>$page</page>\n";
...
foreach($device_list as $device) {
    ...
    $xml.= "<cell><![CDATA[" . $community . "]]></cell>";
    $desc = $device->get_descr();
    if ($desc == "") $desc = "&nbsp;";
