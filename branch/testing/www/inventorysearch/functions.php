...
$basic_search[5] = array(
	...
	"match"=>"LIKE",
	"query"=>"SELECT DISTINCT INET_NTOA(ac.ip_src) as ip FROM snort.acid_event ac,ossim.plugin_sid s WHERE s.plugin_id=ac.plugin_id AND s.sid=ac.plugin_sid AND ac.timestamp >= '$date_from' AND ac.timestamp <= '$date_to' AND s.name %op% ?
		UNION
		SELECT DISTINCT INET_NTOA(ac.ip_dst) as ip FROM snort.acid_event ac,ossim.plugin_sid s WHERE s.plugin_id=ac.plugin_id AND s.sid=ac.plugin_sid AND ac.timestamp >= '$date_from' AND ac.timestamp <= '$date_to' AND s.name %op% ?",
	"query_match"=>"text");
	...
	<tr>
		<td style="padding-bottom:10px" class="nobborder">
			<table class="noborder" style="background-color:white">
				<tr>
					<td class="nobborder"><a href="../report/host_report.php?asset_type=host&asset_key='.$ip.'" id="'.$ip.';'.$host->get_hostname().'" class="HostReportMenu" style="color:#17457c;text-decoration:underline;font-size:15px;text-align:left"><b>'.$ip.'</b> <font style="font-size:12px">HostName: <b>'.($host->get_hostname()).'</b>'.$country_img.$homelan.' '.$os.'</font></a></td>
				</tr>
				<tr>
					<td class="nobborder">
...
function host_row_basic ($host,$conn,$criterias,$has_criterias,$networks,$hosts_ips,$i,$icons=array()) {
	...
	$vuln_caption = ($num_vuln > 0) ?  ' class="greybox_caption" style="text-decoration:none" data="'.$vuln_list_str.'"' : ' class="greybox"';
	// Incidents
	...
	list($sem_events_week,$sem_foundrows_week,$sem_date,$sem_wplot_y,$sem_wplot_x) = Status::get_SEM("",$start_week,$end,5,uniqid(rand(),true),$ip);
	...
	$row = '<tr bgcolor="'.$color.'">
				<td class="nobborder" style="text-align:center;padding:2px"><a href="../report/host_report.php?asset_type=host&asset_key='.$ip.'&star_date='.$start_week_temp.'&end_date='.$end_temp.'" id="'.$ip.';'.$host->get_hostname().'" class="HostReportMenu" style="color:#17457c;font-size:15px;text-align:left"><b>'.$host_name.'</b>'.$country_img.$homelan.'</font></a><br><font style="color:gray">'.$net.'</font></td>
