	...
	if (is_dir($configs_dir)) {
		if ($dh = opendir($configs_dir)) {
			while (($file = readdir($dh)) !== false) {
				if (preg_match("/^".$user."\_[^\_]+\_\d+\.avt/",$file) || preg_match("/^".$user."\_[^\_]+\_\d+\_disabled\.avt/",$file)) {
					list($avt_id,$avt_values) = getavt($file,$configs_dir);
					if (Session::am_i_admin() || !$cloud_instance || ($cloud_instance && !in_array($avt_id,array(1007,1002))) ) // if cloud disable Vulnerabilities and Network Tab
...
function getavt($file,$configs_dir="") {
	if (file_exists($configs_dir."/".$file)) {
		...
		if (preg_match("/([^\_]+)\_([^\_]+)\_([^\_]+)\_disabled\.avt/",$file,$found)){
			$tabicon = ( strlen(trim($avt_icons[$found[3]])) == 0 ) ? "../pixmaps/alienvault_icon.gif" : $avt_icons[$found[3]];
			return array($found[3],array("tab_name"=>base64_decode($found[2]),"tab_file"=>$file,"tab_data"=>$data,"tab_icon_url"=>$tabicon,"disable"=>1));
		}elseif (preg_match("/([^\_]+)\_([^\_]+)\_([^\_]+)\.avt/",$file,$found)){
			$tabicon = ( strlen(trim($avt_icons[$found[3]])) == 0 ) ? "../pixmaps/alienvault_icon.gif" : $avt_icons[$found[3]];
			return array($found[3],array("tab_name"=>base64_decode($found[2]),"tab_file"=>$file,"tab_data"=>$data,"tab_icon_url"=>$tabicon,"disable"=>0));
		}
...
ossim_valid($panel_id, OSS_DIGIT, 'illegal:' . _("panel id"));
...
if (GET('edit_tabs') == 1 && $show_edit) {
    ...
    if (GET('mode')) {
    	...
    	if (strlen($tab_name) > $len_max) {
			...
			$truncmsg = _("Warning: Tab name too long, truncated to 25 characters.");
		}
        ...
        if (GET('mode') != "delete") {
			...
			if (GET('mode') == "clone") {
				...
				if (ossim_error()) {
					echo ossim_error();
					echo('<center><INPUT TYPE="button" VALUE="'._("Go Back").'" onClick="history.go(-1);return true;"></center>');
					die();
				}
...
<table align="center">
	...
	<tr>
		...
		<td nowrap='nowrap' style='background-color: <?=$back_color?>'>
			...
			&nbsp;<a href="" onclick="<?php if ($tabdefault != $tab_id) { ?>if (confirm('<?php echo  Util::js_entities(_("Are you sure?"))?>')) { document.ftabs<?=$tab_id?>.mode.value='delete';document.getElementById('ftabs<?=$tab_id?>').submit(); } <?php } ?>return false;"><img src="../vulnmeter/images/delete.gif" alt="<?=_("Delete")?>" title="<?=_("Delete")?>" border="0" <?php if ($tabdefault == $tab_id) echo "style='filter:alpha(opacity=50);-moz-opacity:0.5;-khtml-opacity: 0.5;opacity: 0.5'" ?>></a>
...
if (GET('interface') == 'ajax') {
    if (GET('ajax_method') == 'showWindowContents') {
        ...
        if (count($options)) {
            ...
            if (isset($options['window_opts']['title'])) $data['TITLE'] = _(trim($options['window_opts']['title'] . $indicator));
            else $data['TITLE'] = "";
        ...
        $data['ID'] = GET('id');
        /*
        if (preg_match("/\<img .*src=\"([^\"]+)\"/i",$data['CONTENTS'],$found)) {
        	if (!file_exists(preg_replace("/\?/","",$found[1]))) {
        		$data['CONTENTS'] = _("Empty image or not available with actual data");
        	}
        }*/
...
<?php
    ...
    } elseif (strlen($tabsavt[$panel_id]["tab_icon_url"]) > 0) {
        $image_string = "<img src=\"" . $tabsavt[$panel_id]["tab_icon_url"] . "\">";
		print "<center><h2> [" . $tabsavt[$panel_id]["tab_name"] . $image_string . "] </h2></center>";
    }
...
if (GET('edit') && $tabsavt[$panel_id] != "") {
	...
	if ($last_tab_id < 1) 
		$last_tab_id = 1;
	$var_tmp = _("This tab can not be edited, ").'<a style="color: #D8000C;" href="panel.php?edit_tabs=1&panel_id='.$panel_id.'&mode=clone&clonefrom='.$panel_id.'&tab_name='.$tabsavt[$panel_id]["tab_name"].'Clone&tab_id='. ($last_tab_id + 1) .'">'. _("click here") .'</a>'. _(" to clone it and edit a copy of this tab") .'<i> ['. _("This will be create a new tab called") .' <b>'. $tabsavt[$panel_id]['tab_name'].' Clone</b>]</i>';
	echo ossim_error($var_tmp);
 exit; 
}
