...
//if ($tabs) {
    ...
    $i=0;
    $orders = array("1001","1005","1008","1003","1007","1004","1002","1006","1500");
	if (is_array($tabsavt)) {
	...
	if (!empty($tabsmerge) && is_array($tabsmerge))
	{
		...
		$avt_icons = array(
			...
			"1500" => "../risk_maps/pixmaps/standard/Virus.png"
		);
		foreach($tabshow as $tab_id => $tab_name) {
			...
			$txt = "<table cellpadding='0' cellspacing='0' border='0' class='transparent'><tr><td class='nobborder'><a href='$url' style='text-decoration:none'>".$image_string ."</a></td><td class='nobborder'><a href='$url' class='gristab$on'>".((trim($tabsmerge[$tab_id]["tab_name"] != "")) ? _(trim($tabsmerge[$tab_id]["tab_name"])) : "&nbsp;")."</a></td></tr></table>";
			if ($panel_id == $tab_id) { ?>
				...
				<table cellpadding='0' cellspacing='0' border='0' width="100%" style="background:transparent;display: <?php
					...
					<tr>
						...
						<td align="right" class="noborder" nowrap='nowrap'>
							<small class="white">
								<?php if ($show_edit && !$can_edit && $panel_id < 1000) { ?>
							...
							<?php } ?>
								...
								echo ($show_edit) ? gettext("Manage tabs")."</a>	|" : ""; ?>
							...
							<table class="noborder" border='0' cellpadding='0' cellspacing='0' align="right" height="26">
								<tr>
									...
									<td class="nobborder" style="background:url(../pixmaps/menu/bgts.gif) repeat-x bottom left;padding-right:4px" nowrap='nowrap'>
										<?php if ($show_edit && !$can_edit && $panel_id < 1000) { ?><a href="<?php echo $_SERVER['SCRIPT_NAME'] ?>?edit=1&panel_id=<?php echo $panel_id ?>"><img align="absmiddle" src="../pixmaps/menu/configuration.gif" border="0" title="<?=_("Configure")?>"></b></a><?php } ?>
                                        <? if (file_exists("/usr/share/ossim/www/voice_urls.php")) { ?>
                                        <script language="JavaScript">
                                           <?php include("../voice_urls.php"); ?>
