...
function SIEM_trends_week($param="") {
	...
	$sqlgraph = "SELECT COUNT(acid_event.sid) as num_events, day(convert_tz(timestamp,'+00:00','$tzc')) as intervalo, monthname(convert_tz(timestamp,'+00:00','$tzc')) as suf FROM $tax_join snort.acid_event LEFT JOIN ossim.plugin ON acid_event.plugin_id=plugin.id WHERE timestamp BETWEEN '".gmdate("Y-m-d 00:00:00",gmdate("U")-604800)."' AND '".gmdate("Y-m-d 23:59:59")."' $plugins_sql $sensor_where $tax_where GROUP BY suf,intervalo ORDER BY suf,intervalo";
	//echo $sqlgraph;
	if (!$rg = & $dbconn->Execute($sqlgraph)) {
	    print $dbconn->ErrorMsg();
	} else {
...
function Logger_trends() {
	...
	$allowed_sensors = ($_SESSION["_allowed_sensors"] != "") ? implode("|",$_SESSION["_allowed_sensors"]) : "";
	...
	foreach ($logger_servers as $ip) if ($ip!="127.0.0.1") {
		$aux = array();
		$cmd = "sudo /usr/share/ossim/www/sem/fetchremote_graph.pl panel $tz '$allowed_sensors' $ip"; //echo $cmd;
		exec($cmd, $aux);
		foreach ($aux as $line) if (preg_match("/(.*);(.*)\=(.*)/",$line,$fnd)) {
			// 9 12h;192.168.10.1=703
			//if (Session::sensorAllowed($fnd[2])) $data[$fnd[1]] += intval(trim($fnd[3]));
			// Already filtered
			$data[$fnd[1]] += intval(trim($fnd[3]));
		}
	}
	// local server
	$today = gmdate("j");
	...
	$csy = get_day_csv(gmdate("Y",strtotime("-1 day")),gmdate("m",strtotime("-1 day")),gmdate("d",strtotime("-1 day")), $allowed_sensors);
	$csv = get_day_csv(gmdate("Y"),gmdate("m"),gmdate("d"), $allowed_sensors);
...
if (GET("type")=="siemday") { 
    ...
    $max = count($hours);*/
    $siem_url = "../top.php?hmenu=".md5("Analysis")."&smenu=".md5("Forensics")."&url=".urlencode("forensics/base_qry_main.php?clear_allcriteria=1&category%5B0%5D=19&time_range=day&time[0][0]=+&time[0][1]=>%3D&time[0][2]=MM&time[0][3]=DD&time[0][4]=".gmdate("Y",$timetz)."&time[0][5]=00&time[0][6]=00&time[0][7]=00&time[0][8]=+&time[0][9]=AND&time[1][0]=+&time[1][1]=<%3D&time[1][2]=MM&time[1][3]=DD&time[1][4]=".gmdate("Y",$timetz)."&time[1][5]=23&time[1][6]=59&time[1][7]=59&time[1][8]=+&time[1][9]=+&submit=Query+DB&num_result_rows=-1&time_cnt=2&sort_order=time_d&hmenu=Forensics&smenu=Forensics");
} else {
    $js = "analytics_duo";
    $data = SIEM_trends();
    ...
    $siem_url = "../top.php?hmenu=".md5("Analysis")."&smenu=".md5("Forensics")."&url=".urlencode("forensics/base_qry_main.php?clear_allcriteria=1&time_range=day&time[0][0]=+&time[0][1]=>%3D&time[0][2]=".gmdate("m",$timetz)."&time[0][3]=".gmdate("d",$timetz)."&time[0][4]=".gmdate("Y",$timetz)."&time[0][5]=HH&time[0][6]=00&time[0][7]=00&time[0][8]=+&time[0][9]=AND&time[1][0]=+&time[1][1]=<%3D&time[1][2]=".gmdate("m",$timetz)."&time[1][3]=".gmdate("d",$timetz)."&time[1][4]=".gmdate("Y",$timetz)."&time[1][5]=HH&time[1][6]=59&time[1][7]=59&time[1][8]=+&time[1][9]=+&submit=Query+DB&num_result_rows=-1&time_cnt=2&sort_order=time_d&hmenu=Forensics&smenu=Forensics");
    $siem_url_y = "../top.php?hmenu=".md5("Analysis")."&smenu=".md5("Forensics")."&url=".urlencode("forensics/base_qry_main.php?clear_allcriteria=1&time_range=day&time[0][0]=+&time[0][1]=>%3D&time[0][2]=".gmdate("m",$timetz-86400)."&time[0][3]=".gmdate("d",$timetz-86400)."&time[0][4]=".gmdate("Y",$timetz-86400)."&time[0][5]=HH&time[0][6]=00&time[0][7]=00&time[0][8]=+&time[0][9]=AND&time[1][0]=+&time[1][1]=<%3D&time[1][2]=".gmdate("m",$timetz-86400)."&time[1][3]=".gmdate("d",$timetz-86400)."&time[1][4]=".gmdate("Y",$timetz-86400)."&time[1][5]=HH&time[1][6]=59&time[1][7]=59&time[1][8]=+&time[1][9]=+&submit=Query+DB&num_result_rows=-1&time_cnt=2&sort_order=time_d&hmenu=Forensics&smenu=Forensics");    
}
//
$empty = true;
?>
