...
switch ($target_url) {
        //case "top_nets":
        //$url="/ossim/report/host_report.php?asset_type=host&asset_key=$category";
        //break;
        
    case "top_hosts":
        if($_GET['category']=="no data")
            $url = "/ossim/report/host_report.php??asset_type=host&asset_key=0.0.0.0";
        else 
            $url = "/ossim/report/host_report.php?asset_type=host&asset_key=" . $_GET['category'];
        break;
    ...
    case "events_sensor":
        ...
        $plugin_param = "";
        $sensor_param = "";
        if (strlen($sensor) > 0 && strlen($plugin) < 2) {
        	require_once("classes/Sensor.inc");
            // Assume Sensor
            $sensor_sids = "";
            $sensor_ip = "";
            $sensor_aux = Sensor::get_list($conn, "WHERE name='$sensor'");
            if ($sensor_aux[0] != "") {
                $sensor_ip = $sensor_aux[0]->get_ip();
            } elseif (preg_match("/^\d+\.\d+\.\d+\.\d+$/", $sensor)) {
            	$sensor_ip = $sensor;
            }
            if ($sensor_ip != "") {
                    $sql = "select sid from snort.sensor where snort.sensor.hostname LIKE '$sensor_ip%'";
                    if (!$rs = & $conn->Execute($sql)) {
                        print $conn->ErrorMsg();
                        exit();
                    }
                    while (!$rs->EOF) {
                        $sensor_sids .= ($sensor_sids != "") ? ",".$rs->fields["sid"] : $rs->fields["sid"];
                        $rs->MoveNext();
                    }
            }
            $sensor_param = "&sensor=$sensor_sids";
        } elseif (strlen($plugin) > 2 && strlen($sensor) < 1) {
            // Assume Plugin
            ...
            if ($sensor_aux[0] != "") {
            	$sensor_ip = $sensor_aux[0]->get_ip();
            	if ($plugin == "snort") {
            		$sql = "select DISTINCT sid from snort.sensor, ossim.sensor where snort.sensor.hostname LIKE '$sensor_ip'";
            	} else {
            		$sql = "select DISTINCT sid from snort.sensor, ossim.sensor where snort.sensor.hostname LIKE '$sensor_ip%' AND snort.sensor.hostname LIKE '%$plugin%'";
            	}
            	$params = array();
            } else {
        ...
        $url = "/ossim/forensics/base_qry_main.php?clear_allcriteria=1&num_result_rows=-1&submit=Query+DB&clear_criteria=time&hmenu=Forensics&smenu=Forensics$sensor_param$plugin_param";
        break;

    case "host_report":
        $url = "/ossim/report/host_report.php?asset_type=host&asset_key=" . $_REQUEST[$target_var];
        break;

    case "incident_status":
        $url = "/ossim/index.php?hmenu=".md5("Incidents")."&smenu=".md5("Tickets")."&url=incidents%2Findex.php%3F%26status%3D" . $_REQUEST[$target_var];
        break;
    ...
    case "inventory":
        $url = "/ossim/index.php?hmenu=".md5("Incidents")."&smenu=".md5("Alarms")."&url=".urlencode("policy/ocs_index.php?hmenu=Assets&smenu=Inventory");
        break;

    case "alarms_events":
        if ($_REQUEST[$target_var] == "Alarms") {
            $url = "/ossim/index.php?hmenu=".md5("Incidents")."&smenu=".md5("Alarms")."&url=/ossim/control_panel/alarm_console.php";
        }
