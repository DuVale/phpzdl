...
class Rule {
    ...
    function Rule($id, $level, $rule, $name, $plugin_id, $plugin_type, $plugin_sid, $from, $port_from, $to, $port_to, $protocol, $sensor, $occurrence, $time_out, $reliability, $condition, $value, $interval, $absolute, $sticky, $sticky_different, $groups, $iface, $filename, $username, $password, $userdata1, $userdata2, $userdata3, $userdata4, $userdata5, $userdata6, $userdata7, $userdata8, $userdata9) {
        $this->id = $id;
        $this->level = $level;
        $this->rule = $rule;
        $this->name = $name;
        ...
        $this->userdata1 = ($userdata1 == "") ? "" : $userdata1;
        $this->userdata2 = ($userdata2 == "") ? "" : $userdata2;
        $this->userdata3 = ($userdata3 == "") ? "" : $userdata3;
        $this->userdata4 = ($userdata4 == "") ? "" : $userdata4;
        $this->userdata5 = ($userdata5 == "") ? "" : $userdata5;
        $this->userdata6 = ($userdata6 == "") ? "" : $userdata6;
        $this->userdata7 = ($userdata7 == "") ? "" : $userdata7;
        $this->userdata8 = ($userdata8 == "") ? "" : $userdata8;
        $this->userdata9 = ($userdata9 == "") ? "" : $userdata9;
    }
    ...
    <?php
		...
		<?php
            ...
            if (!$this->is_new()) {
                ...
                //removeRule button
                print '<td>';
                print "<a onclick=\"javascript:if (confirm('" .  Util::js_entities(_("Are you sure you want to delete this rule ?")) . "')) { window.open('../include/utils.php?query=del_rule&id=" . $this->id . "','right'); }\" style=\"marging-left:20px; cursor:pointer\" TITLE=\"" . gettext("Delete this rule") . "\">x</a>";
                print '</td>';
                //left button
                print '<td>';
...
<?php
    ...
    function getXMLNode($dom) {
        ...
        if ($this->iface != NULL && $this->iface != "") $node->set_attribute("iface", $this->iface);
...
function left($dom, $id, &$tab_rules, $directive) {
    ...
    if ($tab_rules[$id_rule]->level > 1) {
        ...
        $newlevel = $tab_rules[$id_rule]->level;
        $_POST['new_level'] = $newlevel;
        echo "<script>alert(\"" . Util::js_entities(_("Not allowed for this rule")) . "\")</script>";
    }
}
