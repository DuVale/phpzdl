...
?>	
        ...
        echo  Util::js_entities(_("Are you sure you want to delete all rules ?")); ?>')) { window.open('../include/utils.php?query=del_all_rule','right'); }" style="marging-left:20px; cursor:pointer" TITLE="<?php
    ...
    if ($directive->has_child_nodes()) {
        ...
        foreach($rules as $rule) {
            if (($rule->type == XML_ELEMENT_NODE) && ($rule->tagname() == 'rule')) {
                ...
                $temp = new Rule($id, $level, $rule, $rule->get_attribute('name') , $rule->get_attribute('plugin_id') , $rule->get_attribute('type') , $rule->get_attribute('plugin_sid') , $rule->get_attribute('from') , $rule->get_attribute('port_from') , $rule->get_attribute('to') , $rule->get_attribute('port_to') , $rule->get_attribute('protocol') , $rule->get_attribute('sensor') , $rule->get_attribute('occurrence') , $rule->get_attribute('time_out') , $rule->get_attribute('reliability') , $rule->get_attribute('condition') , $rule->get_attribute('value') , $rule->get_attribute('interval') , $rule->get_attribute('absolute') , $rule->get_attribute('sticky') , $rule->get_attribute('sticky_different') , $rule->get_attribute('groups'), $rule->get_attribute('iface'), $rule->get_attribute('filename'), $rule->get_attribute('username'), $rule->get_attribute('password'), $rule->get_attribute('userdata1'), $rule->get_attribute('userdata2'), $rule->get_attribute('userdata3'), $rule->get_attribute('userdata4'), $rule->get_attribute('userdata5'), $rule->get_attribute('userdata6'), $rule->get_attribute('userdata7'), $rule->get_attribute('userdata8'), $rule->get_attribute('userdata9'));
                $tab_rules[$ind] = $temp;
