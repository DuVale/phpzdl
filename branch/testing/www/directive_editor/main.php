...
function get_disabled_directives() {
	$ret = array();
	$lines = (file_exists("/etc/ossim/server/disabled_directives.data")) ? file("/etc/ossim/server/disabled_directives.data") : array();
	foreach ($lines as $line) {
...
function xml_userdata() {
	$ret = array();
	$lines = file('/etc/ossim/server/user.xml');
	foreach ($lines as $line) {
		if (preg_match("/directive id\=\"(\d+)\" name\=\"([^\"]+)\"/",$line,$found)) {
			$ret[$found[2]] = $found[1];
		}
	}
	return $ret;
...
$disabled_directives = get_disabled_directives();
$inuser_directives = xml_userdata();
?>
...
if ($_GET["right"] != "") { ?>
				...
				function restart()
				{
					...
					if (confirm('<?php echo Util::js_entities(gettext('Are you sure you want to restart the OSSIM server ?')); ?>'))
					{
...
function background_clone_user(id,xml_file,mini) {
	var url = './include/utils.php?query=copy_directive_user&id='+id+'&directive_xml='+xml_file+'&mini='+mini;
	$.ajax({
		type: "GET",
		url: url,
		data: "",
		success: function(msg){
			background_disable(id,xml_file);
		}
	});
}
function background_disable(id,xml_file) {
	var url = './include/utils.php?query=disable_directive&id='+id+'&directive_xml='+xml_file;
	$.ajax({
		type: "GET",
		url: url,
		data: "",
...
function background_enable(id,xml_file) {
	var url = './include/utils.php?query=enable_directive&id='+id+'&directive_xml='+xml_file;
	$.ajax({
		type: "GET",
		url: url,
		data: "",
				...
				<td style="border:0px" valign="top">
									...
									<table cellpadding='0' cellspacing='0' style="border:0px; background-color:transparent;" width="100%">
										<tr>
											...
											<span id="add_dir" name="add_dir">
											  ...
											  <a href="index.php?action=add_directive&xml_file=<?php echo $category->xml_file?>&id=<?php echo $category->id . $onlydir; ?>" style="marging-left:20px;" title="<?php echo gettext("Add a directive in this category"); ?>"><img src="../pixmaps/plus-small-blue.png" border="0" alt="<?php echo gettext("Add a directive in this category"); ?>" title="<?php echo gettext("Add a directive in this category"); ?>"></img></a>
											...
											<td width="20" align="right">
											  <?php if ($category->active) { ?>
											  <a href="main.php?action=disable_category&xml_file=<?php echo $category->xml_file?>&name=<?php echo $category->name ?>" style="margin-left:20px;" title="<?php echo gettext("Disable this category"); ?>"><img src="../pixmaps/switch--minus.png" border="0" alt="<?php echo gettext("Disable this category"); ?>" title="<?php echo gettext("Disable this category"); ?>"/></a>
											  <?php } else { ?>
											  <a href="main.php?action=enable_category&xml_file=<?php echo $category->xml_file?>&name=<?php echo $category->name ?>" style="margin-left:20px; " title="<?php echo gettext("Enable this category"); ?>"><img src="../pixmaps/switch--plus.png" border="0" alt="<?php echo gettext("Enable this category"); ?>" title="<?php echo gettext("Enable this category"); ?>"/></a>
											  <?php } ?>
											</td>
											<td width="20" align="right">
											  <?php if ($category->active) { ?>
											  <a href="main.php?action=disable_category&xml_file=<?php echo $category->xml_file?>&name=<?php echo $category->name ?>" style="margin-left:5px;" title="<?php echo gettext("Disable this category"); ?>"><img src="../pixmaps/tick.png" border="0" alt="<?php echo gettext("Disable this category"); ?>" title="<?php echo gettext("Disable this category"); ?>"/></a>
											  <?php } else { ?>
											  <a href="main.php?action=enable_category&xml_file=<?php echo $category->xml_file?>&name=<?php echo $category->name ?>" style="margin-left:5px; " title="<?php echo gettext("Enable this category"); ?>"><img src="../pixmaps/tick_gray.png" border="0" alt="<?php echo gettext("Enable this category"); ?>" title="<?php echo gettext("Enable this category"); ?>"/></a>
											  <?php } ?>
											</td>
										</tr>
									</table>
					...
					<div id="<?php echo $id_div; ?>" <?php if ($query == "" || count($tab_this_category) < 1) { ?>style="display:none"<?php } ?>>
						<table width="100%">
							...
							?>				
								<tr>
									<td style="text-align: center;background-color:<?php echo $color?>;border:0px;white-space:nowrap" width="40px" nowrap>
										<?php if ($category->xml_file == "user.xml") { ?>
										<a onclick="javascript:if (confirm('<?php echo Util::js_entities(gettext("Are you sure you want to delete this directive ?")); ?>')) { background_delete(<?php echo $directive->get_attribute('id'); ?>,'<?=$category->xml_file?>'); }" style="marging-left:20px; cursor:pointer" title="<?php echo gettext("Delete this directive"); ?>"><img src="../pixmaps/delete.gif" alt="<?php echo _("Delete")?>" title="<?php echo _("Delete")?>" border="0"></img></a>
										<a onclick="javascript:if (confirm('<?php echo Util::js_entities(gettext("Are you sure you want to clone this directive ?")); ?>')) { background_clone(<?php echo $directive->get_attribute('id'); ?>,'<?=$category->xml_file?>','<?=$category->mini?>'); }" style="marging-left:20px; cursor:pointer" title="<?php echo gettext("Clone this directive"); ?>"><img src="../pixmaps/copy.png" alt="<?php echo _("Clone")?>" title="<?php echo _("Clone")?>" border="0"></img></a>
										<?php if ($disabled_directives[$category->xml_file][$directive->get_attribute('id')]) { ?>
										<a onclick="javascript:background_enable(<?php echo $directive->get_attribute('id'); ?>,'<?=$category->xml_file?>');return false;" style="marging-left:20px; cursor:pointer" title="<?php echo gettext("Enable this directive"); ?>"><img src="../pixmaps/cross.png" alt="<?php echo _("Enable")?>" title="<?php echo _("Enable")?>" border="0"></img></a>
										<?php } else { ?>
										<a onclick="javascript:background_disable(<?php echo $directive->get_attribute('id'); ?>,'<?=$category->xml_file?>');return false;" style="marging-left:20px; cursor:pointer" title="<?php echo gettext("Disable this directive"); ?>"><img src="../pixmaps/tick.png" alt="<?php echo _("Disable")?>" title="<?php echo _("Disable")?>" border="0"></img></a>
										<?php } ?>
										<?php } else { ?>
											<?php if ($inuser_directives[$directive->get_attribute('name')]) { ?>
											<img src="../pixmaps/cross-gray.png" alt="<?php echo _("Enable")?>" title="<?php echo _("Enable")?>" border="0"></img>
											<a onclick="javascript:if (confirm('<?php echo Util::js_entities(gettext("Are you sure you want to remove this directive from user category ?")); ?>')) { background_delete(<?php echo $inuser_directives[$directive->get_attribute('name')] ?>,'user.xml'); }" style="marging-left:20px; cursor:pointer" title="<?php echo gettext("Remove this directive from user"); ?>"><img src="../pixmaps/lock-unlock.png" alt="<?php echo _("Delete from user")?>" title="<?php echo _("Delete from user")?>" border="0"></img></a>
											<?php } else { ?>
												<?php if ($disabled_directives[$category->xml_file][$directive->get_attribute('id')]) { ?>
												<a onclick="javascript:background_enable(<?php echo $directive->get_attribute('id'); ?>,'<?=$category->xml_file?>');return false;" style="marging-left:20px; cursor:pointer" title="<?php echo gettext("Enable this directive"); ?>"><img src="../pixmaps/cross.png" alt="<?php echo _("Enable")?>" title="<?php echo _("Enable")?>" border="0"></img></a>
												<?php } else { ?>
												<a onclick="javascript:background_disable(<?php echo $directive->get_attribute('id'); ?>,'<?=$category->xml_file?>');return false;" style="marging-left:20px; cursor:pointer" title="<?php echo gettext("Disable this directive"); ?>"><img src="../pixmaps/tick.png" alt="<?php echo _("Disable")?>" title="<?php echo _("Disable")?>" border="0"></img></a>
												<?php } ?>
											<a onclick="javascript:if (confirm('<?php echo Util::js_entities(gettext("Are you sure you want to clone this directive to user category ?")); ?>')) { background_clone_user(<?php echo $directive->get_attribute('id'); ?>,'<?=$category->xml_file?>','<?=$category->mini?>'); }" style="marging-left:20px; cursor:pointer" title="<?php echo gettext("Clone this directive to user"); ?>"><img src="../pixmaps/lock.png" alt="<?php echo _("Clone to user")?>" title="<?php echo _("Clone to user")?>" border="0"></img></a>
											<?php } ?>
										<?php } ?>
									</td>
