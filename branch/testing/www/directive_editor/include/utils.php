...
elseif ($query == "add_rule") {
    ...
    $temp = new Rule($id, $newlevel, "", "New rule", "", "", "ANY", "ANY", "ANY", "ANY", "ANY", "ANY", "ANY", "1", "None", "0", "Default", "Default", "Default", "Default", "true", "None");
...
/* Disable a directive */
elseif ($query == "disable_directive") {
    $dir_id = $_GET['id'];
	$file = $_GET['directive_xml'];
	$disabled_file = "/etc/ossim/server/disabled_directives.data";
	ossim_valid($file, OSS_ALPHA, OSS_DOT, OSS_SCORE, OSS_NULLABLE, 'illegal:' . _("directive_xml"));
	if (ossim_error()) {
		die(ossim_error());
	}
...
/* copy a directive to user category */
elseif ($query == "copy_directive_user") {
	$dir_id = $_GET['id'];   
    ...
    $id_category = get_category_id_by_directive_id($dir_id);
    $dom_orig = open_file($file);
    $directive = getDirectiveFromXML($dom_orig, $dir_id);
    release_file($file);
    $dom =  open_file("/etc/ossim/server/user.xml");
    if($directive->directive=="") {
        header("Location: ../viewer/index.php?directive=$dir_id");
    }  
    $mini = $_GET['mini'];
    $new_id = new_directive_id_by_directive_file("/etc/ossim/server/user.xml", 500000);
	//$new_id = new_directive_id($id_category);
    $new_directive = $dom->create_element('directive');
    $new_directive->set_attribute('id', $new_id);
    $new_directive->set_attribute('name', $directive->name);
    $new_directive->set_attribute('priority', $directive->priority);
    $tab_rules = $directive->rules;
    for ($ind = 1; $ind <= count($tab_rules); $ind++) {
        $rule = $tab_rules[$ind];
	...
	$tab_directives = $dom->get_elements_by_tagname('directives');
    $directives = $tab_directives[0];
    $directives->append_child($new_directive);
    $dom->dump_file("/etc/ossim/server/user.xml");
    release_file("/etc/ossim/server/user.xml");
    sleep(0.5);
    echo "<html><body onload=\"top.frames['main'].document.location.href='../index.php?level=1&directive=" . $new_id . "&directive_xml=" . $_GET['directive_xml'] . "&category_mini=$mini'\"></body></html>";
    //echo "<html><body onload=\"top.frames['main'].document.location.href='../index.php?directive=" . $new_id . "&action=copy_directive&id=" . $new_id . "&xml_file=" . $_GET['directive_xml'] . "'\"></body></html>";
    //echo "<html><body><a href=\"../index.php?directive=".$new_id."&action=copy_directive&id=".$new_id."\">test</a></body></html>";
