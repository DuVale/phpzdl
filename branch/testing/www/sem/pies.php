...
$cmd = process($a, $start, $end, $offset, $sort_order, "all", $uniqueid, 51, 0, $allowed_sensors);
...
if ($cmd != "") {
	if ($ip_list != "") {
	    ...
	    while (!feof($fp)) {
	        ...
	        elseif(preg_match("/^\s+(\S+)\s+(\d+)\s+(\S+)/", $res, $matches)) {
	            //sensors
	            if($matches[1]=="sensor"){
	            	$_name = Sensor::get_sensor_name($conn,$matches[3]);
	                if ($_name != "" && $_name != $matches[3]) {
	                    $sensors[$_name." (".$matches[3].")"] += $matches[2];
	                } else {
	                    $sensors[$matches[3]] += $matches[2];
	                }
	            }
	            //Events types
	            if($matches[1]=="plugin_id") {
	            	$_name = Plugin::get_name_by_id($conn,$matches[3]);
	                if ($_name != "") {
	                    $event_type[$_name] += $matches[2];
	                }
	                else {
	                    $event_type[$matches[3]] += $matches[2];
	                }
	            }
	            // sources
	            if($matches[1]=="src_ip") {
	            	$_name = Host::ip2hostname($conn, $matches[3]);
	                if ($_name != "" && $_name != $matches[3]) {
	                    $ips_src[$_name." (".$matches[3].")"] += $matches[2];
	                }
	                else {
	                    $ips_src[$matches[3]] += $matches[2];
	                }
	            }
	            // destinations
	            if($matches[1]=="dst_ip") {
	                $_name = Host::ip2hostname($conn, $matches[3]);
	                if ($_name != "" && $_name != $matches[3]) {
	                    $ips_dst[$_name." (".$matches[3].")"] += $matches[2];
	                }
	                else {
	                    $ips_dst[$matches[3]] += $matches[2];
	                }
	            }
	        }
	    }
    }
    // Export locales to remote
    if ($only_json) {
    ...
    arsort($sensors);
    foreach ($sensors as $k => $v) $sensors1[$k." [".Util::number_format_locale($v,0)."]"] = $v;
    $sensors = $sensors1; unset($sensors1);
    //
    arsort($event_type);
    foreach ($event_type as $k => $v) $event_type1[$k." [".Util::number_format_locale($v,0)."]"] = $v;
    $event_type = $event_type1; unset($event_type1);
    //
    arsort($ips_src);
    foreach ($ips_src as $k => $v) $ips_src1[$k." [".Util::number_format_locale($v,0)."]"] = $v;
    $ips_src = $ips_src1; unset($ips_src1);
    //
    arsort($ips_dst);
    foreach ($ips_dst as $k => $v) $ips_dst1[$k." [".Util::number_format_locale($v,0)."]"] = $v;
    $ips_dst = $ips_dst1; unset($ips_dst1);
}
?>
...
<tr height="410">
	<td style="border-left: 1px solid rgb(170, 170, 170);border-bottom: 1px solid rgb(170, 170, 170);border-right: 1px solid rgb(170, 170, 170);" valign="top">
		<table cellpadding=0 cellspacing=0 width="100%" align="center"><tr><td class="noborder" style="padding:5px 5px 5px 5px" align="center" height="385" valign="top">
		...
		<?php if (count($sensors) > 10) { ?>
		<tr>
			<td valign="bottom" style="text-align:center;padding:5px">
				<select name="" onchange="parent.display_info('','','',this.value,'','sensor')" style="font-size:10px">
				<option value="">- <?php echo _("Filter by another sensor") ?> -
				<?php $k = 0; foreach ($sensors as $key => $value) { if ($k < 10) { $k++; continue; } ?>
				<option value="<?php echo preg_replace("/\s+\(.*|\s+\[.*/","",$key) ?>"><?php echo $key ?>
				<?php } ?>
				</select>
			</td>
		</tr>
	...
	<td style="border-left: 1px solid rgb(170, 170, 170);border-bottom: 1px solid rgb(170, 170, 170);border-right: 1px solid rgb(170, 170, 170);" valign="top">
		...
		<?php if (count($event_type) > 10) { ?>
		<tr>
			<td valign="bottom" style="text-align:center;padding:5px">
				<select name="" onchange="parent.display_info('','','',this.value,'','plugin')" style="font-size:10px">
				<option value="">- <?php echo _("Filter by another Event Type") ?> -
				<?php $k = 0; foreach ($event_type as $key => $value) { if ($k < 10) { $k++; continue; } ?>
				<option value="<?php echo preg_replace("/\s+\[.*/","",$key) ?>"><?php echo $key ?>
				<?php } ?>
				</select>
			</td>
		</tr>
...
$(document).ready(function(){
	...
	<? if (count($sensors) > 0) { ?>
	$.plot($("#sensors_pie"), [
	<? $i=0;foreach ($sensors as $key => $value) if ($i<10 && $value>0) { ?>
	...
	<? if (count($event_type) > 0) { ?>
		$.plot($("#event_type_pie"), [
	<? $i=0;foreach ($event_type as $key => $value) if ($i<10 && $value>0) { ?>
	...
	<? if (count($ips_src) > 0) { ?>
		$.plot($("#sources_pie"), [
	<? $i=0;foreach ($ips_src as $key => $value) if ($i<10 && $value>0) { preg_match("/(\d+\.\d+\.\d+\.\d+)/",$key,$found); ?>
	...
	<? $i=0;foreach ($ips_dst as $key => $value) if ($i<10 && $value>0) { preg_match("/(\d+\.\d+\.\d+\.\d+)/",$key,$found); ?>
