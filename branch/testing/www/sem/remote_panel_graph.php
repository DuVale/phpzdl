...
function get_day_csv($year, $month, $day, $allowed_sensors="") {
	...
	$allowed_sensors_arr = ($allowed_sensors != "") ? explode(",",$allowed_sensors) : array();
	$sensors = array();
	...
	if ($dir = opendir($path)) { 
		while (($file = readdir($dir)) !== false) {
			if (preg_match("/\.csv_total_events_(.+)/",$file,$fnd)) 
				if ($allowed_sensors == "" || in_array($fnd[1], $allowed_sensors_arr)) {
					$sensors[] = $fnd[1];
				}
		}
	}
	...
	foreach ($sensors as $s) if ($s != "") {
		...
		if (count($lines) > 1) {
			foreach($lines as $line) {
				...
				if ($val[0]!="") $days[$s][sprintf("%d", $val[0])] += intval($val[1]);
			}
		}
...
$allowed_sensors = str_replace("|",",",$argv[2]);
$data = array();
$today = gmdate("j");
