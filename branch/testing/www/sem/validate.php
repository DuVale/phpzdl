...
ossim_valid($signature, OSS_BASE64, OSS_NULLABLE, 'illegal:' . _("signature"));
...
if ($server != "127.0.0.1") {
	$cmd = "sudo ./fetchremote_validate.pl \"$signature\" \"".base64_encode($log_line)."\" \"$start\" \"$end\" \"$logfile\" $server";
	$salida = explode("\n",`$cmd`);
