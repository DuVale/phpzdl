...
$start = $ARGV[0];
$end = $ARGV[1];
$end_match = $end - $delay_seconds_todate;
$start_line = $ARGV[2];
$num_lines = $ARGV[3];
$filter = $ARGV[4];
...
#
if ($ARGV[5] ne "date") {
	my @tm = gmtime($end); $tm[5]+=1900; $tm[4]++;
	...
	close L;
	open (L,"find ".$log_dir."/".$tm[5]."/".$tm[4]."/".$tm[3]."/ -name \\*log.gz 2>/dev/null | sort -r -u |");
	while($file=<L>) {
		chomp($file);  
		print "Adding log: $file\n" if ($debug);
		 if ($debug_log ne "") {
...
foreach my $file (@files) {
    ...
    if ($start<=$edirtime && $end>=$sdirtime) { #if ($edirtime > $start && $sdirtime < $end) {
		...
		if ($fields[4].$fields[5].$fields[6].$fields[7]==$hourday || $complete_lines<$lines_threshold) { # read files while same hourday or need more events
            ...
            if ( $pre_filter !~ /^[^ ]*\!=]*/ && $pre_filter !~ /taxonomy|plugingroup|dsgroup|plugin_list/ && $pre_filter !~ /\||#.*=/) {
                #$pre_filter =~ s/\[/\\\[/g;
                #$pre_filter =~ s/\]/\\\]/g;
                #$pre_filter =~ s/\(/\\\(/g;
                #$pre_filter =~ s/\)/\\\)/g;
                ...
                #$pre_filter =~ s/\./\\./g; # removed as the -F option with grep suppresses pattern matching and there is no need to escape the dots on IPs
                #$pre_filter =~ s/#.*//g;
                $pre_filter =~ s/[^\=]+\=//;

		# msandulescu: run case sensitive search on IPs, IP and PORT, CiscoID and any combination of numbers and special characters. Also preselect pre_metching entries before parsing them one by one

		if ( $pre_filter =~ /^[0-9\.\\\[\]\{\}\|\(\)\-\_\=\+\<\>\,\~\?\:\;\'\"\/]+$/ ||  $pre_filter =~ /^[%]/ ||  $pre_filter == '') {
                	$pre_cmd = ($file =~ /\.gz$/) ? "zcat \"$file\" |grep -Fl \"$pre_filter\"|" : "grep -Fl \"$pre_filter\" \"$file\" |"; #stops on first match
                	$cmd = ($file =~ /\.gz$/) ? "zcat \"$file\" |grep -F \"$pre_filter\"|" : "$order_by \"$file\" |grep -F \"$pre_filter\"|"; #gets pre_matching entries
		} else {
                	$pre_cmd = ($file =~ /\.gz$/) ? "zcat \"$file\" |grep -Fil \"$pre_filter\"|" : "grep -Fil \"$pre_filter\" \"$file\" |"; #stops on first match
                	$cmd = ($file =~ /\.gz$/) ? "zcat \"$file\" |grep -Fi \"$pre_filter\"|" : "$order_by \"$file\" |grep -Fi \"$pre_filter\"|"; #gets pre_matching entries
		}
            } else {
			...
			if ( $first_line ne "" ) {
				...
				LINE: while (<F>) {
					...
					if (/entry id='([^']+)'\s+fdate='([^']+)'\s+date='([^']+)'\s+plugin_id='([^']+)'\s+sensor='([^']+)'\s+src_ip='([^']+)'\s+dst_ip='([^']+)'\s+src_port='([^']+)'\s+dst_port='([^']+)'\s+tzone='([^']+)'+\s+(datalen='\d+'\s+)?data='(.*)'/i) {
						...
						# Always get the date-erroneous events
						if (abs($sdirtime - $currentdate) > $delay_seconds_todate) {
							$currentdate = $start;
						}
                        
						last LINE if ($reverse && $complete_lines>=$lines_threshold && $currentdate<$lastdate); # jump innecesary events
						last LINE if (!$reverse && $complete_lines>=$lines_threshold && $currentdate>$lastdate); # jump innecesary events
						#print "Evento: $currentdate > $start && $currentdate < $end_match\n" if ($debug);
						if ($currentdate >= $start && $currentdate <= $end_match && pass_filters($_,$plugin_id,$plugin_sid,$sensor,$src_ip,$dst_ip,$src_port,$dst_port,$data)) {
