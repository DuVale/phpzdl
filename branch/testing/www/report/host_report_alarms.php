...
if (  $asset_counter < 2 )
{
	...
	<script type="text/javascript">
       $("#pbar").progressBar(80);
       $("#progressText").html('<?=gettext("Loading")?> <b><?=gettext("Alarms")?></b>...');</script>
    </script>
    <?php
}
...
<script type="text/javascript">
    $('#statusbar_unresolved_alarms_<?php echo generate_id($host)?>').html('<?php echo Util::number_format_locale((int)0,0)?>');
    $('#statusbar_alarm_max_risk_<?php echo generate_id($host)?>').removeAttr("href");
    $('#statusbar_alarm_max_risk_<?php echo generate_id($host)?>').html('<?php echo preg_replace("/\n|\r/","",Incident::get_priority_in_html("-"))?>');
</script>

<table align="center" style='<?php echo $table_style?>'>
	<tr>
		<td class="headerpr" height="20">
            <?php $alarms = (  count($param['assets']['data']['ip_cidr']) > 1 ) ? _("Alarms"). " <i>($host)</i>" : _("Alarms");?>
            <a style="color:black" href="<?php echo $temp_url?>"><?php echo $alarms; ?></a>
        </td>
	</tr>
	...
	<?php 
		...
		{ 
					...
					<table class="noborder">
						...
						foreach($alarm_list as $alarm) 
						{
							...
							{ 
								...
								?>
								<script type="text/javascript">
                                    $('#alarms_date_<?php echo generate_id($host)?>').html("<?php echo $a_date?>");
   								</script>
								<?php
							} 
							...
							$src_link         = "../report/host_report.php?asset_type=host&asset_key=$src_ip$target";
							$dst_link         = "../report/host_report.php?asset_type=host&asset_key=$dst_ip$target";
			...
			<script type="text/javascript">
                $('#statusbar_unresolved_alarms_<?php echo generate_id($host)?>').html('<?php echo Util::number_format_locale((int)$count,0)?>');
				$('#statusbar_alarm_max_risk_<?php echo generate_id($host)?>').html('<?php echo preg_replace("/\n|\r/","",Incident::get_priority_in_html($a_maxrisk))?>');
				$('#statusbar_alarm_max_risk_<?php echo generate_id($host)?>').attr('href', '<?php echo $alarm_link?>');
				$('#statusbar_alarm_max_risk_txt_<?php echo generate_id($host)?>').attr('href', '<?php echo $alarm_link?>');
            </script>
