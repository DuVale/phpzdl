...
$order  = ( !empty($_POST['sortname']) )  ? POST('sortname')  : "name";
$order .= ( !empty($_POST['sortorder']) ) ? " ".POST('sortorder') : " desc";
...
if ($order == "ip") 
	$order = "INET_ATON(ip)"; // Numeric ORDER for IP
...
//Search
$search = ( !empty($search) && !empty($field) ) ? "WHERE $field like '%$search%'" : "";
...
$sensor_list       = Sensor::get_all($conn, "$search ORDER BY $order $limit");
$sensor_list_total = Sensor::get_all($conn, "$search");
