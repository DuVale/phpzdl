...
$unresolved_alarms_prev = $_SESSION["_unresolved_alarms"];

if ($unresolved_alarms!=$unresolved_alarms_prev && $_SESSION["_unresolved_alarms"]!="") {
	$new_alarms = $unresolved_alarms - $unresolved_alarms_prev;
} else {
	$new_alarms = 0;
}
$_SESSION["_unresolved_alarms"]           = $unresolved_alarms;
...
?>
    document.getElementById('statusbar_incident_max_priority').href = 'top.php?hmenu=<?php echo md5("Incidents") ?>&smenu=<?php echo md5("Tickets") ?>&url=<?php echo urlencode("incidents/incident.php?id=$incident_max_priority_id&hmenu=Tickets&smenu=Tickets") ?>';
    document.getElementById('statusbar_incident_max_priority_txt').href = 'top.php?hmenu=<?php echo md5("Incidents") ?>&smenu=<?php echo md5("Tickets") ?>&url=<?php echo urlencode("incidents/incident.php?id=$incident_max_priority_id&hmenu=Tickets&smenu=Tickets") ?>';
<?php
}
else {
?>
    document.getElementById('statusbar_incident_max_priority').href = 'javascript:;';
    document.getElementById('statusbar_incident_max_priority_txt').href = 'javascript:;';
<?php 
}
?>
...
?>
    document.getElementById('statusbar_alarm_max_risk').href = 'top.php?hmenu=<?php echo md5("Incidents") ?>&smenu=<?php echo md5("Alarms") ?>&url=<?php echo urlencode("control_panel/alarm_console.php?order=risk DESC&hide_closed=1&hmenu=Alarms&smenu=Alarms") ?>';
    document.getElementById('statusbar_alarm_max_risk_txt').href = 'top.php?hmenu=<?php echo md5("Incidents") ?>&smenu=<?php echo md5("Alarms") ?>&url=<?php echo urlencode("control_panel/alarm_console.php?order=risk DESC&hide_closed=1&hmenu=Alarms&smenu=Alarms") ?>';
<?php
}
else {
?>
    document.getElementById('statusbar_alarm_max_risk').href = 'javascript:;';
    document.getElementById('statusbar_alarm_max_risk_txt').href = 'javascript:;';
<?php
}
?>
...
// Alarm notifications
var new_alarms = <?php echo $new_alarms ?>;
if (window.webkitNotifications && new_alarms>0) {
<?php
if ($new_alarms>0) {
	list($alarm_list, $count) = Alarm::get_list3($conn, "", "", 1, "ORDER BY timestamp DESC", 0, $new_alarms);
	$i=0;
	foreach ($alarm_list as $alarm) {
?>
	av_notification ('<?php echo _("New alarm")?>','<?php echo str_replace("'","\'",Util::translate_alarm($conn, $alarm->get_sid_name(), $alarm))?>',<?php echo (($i % 4) + 1)?>);
<?php
		$i++;
	}
}	
