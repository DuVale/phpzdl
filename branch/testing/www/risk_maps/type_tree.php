...
/*
$cachefile = "/var/ossim/sessions/".$_SESSION["_user"]."_policy_".base64_encode($key)."_$page.json";
    ...
    if (count($hg_list)>0) 
	{
		...
		foreach($hg_list as $hg) 
		{
            if($j>=$from && $j<$to) 
			{
				...
				$hg_key   = utf8_encode("host_group;".$hg_name);
    ...
    if ($net_group_list = Net_group::get_list($conn))
	{
        ...
        foreach($net_group_list as $net_group)
		{
            if ($j>=$from && $j<$to) 
			{
				...
				$ng_key   = utf8_encode("net_group;".$ng_name);
				$ng_url   = "NETGROUP:".Util::htmlentities($ng_name);
...
else if (preg_match("/^net_group;(.*)/",$key,$found))
{
	...
	foreach($nets as $net)
	{
        ...
        if ($j>=$from && $j<$to) {
            $html.= "{ key:'$net_key', url:'$net_url', icon:'../../pixmaps/theme/net.png', title:'$title', tooltip:'$tooltip' },\n";
            $k++;
        }
        $j++;
    }
	...
	if ($j>$to) 
	{
        $li = "key:'".utf_encode($key)."', page:'$nextpage', isFolder:true, isLazy:true, icon:'../../pixmaps/theme/net.png', title:'"._("next")." $maxresults "._("net")."'";
        $buffer .= ",{ $li }\n";
    }
...
else if ( $key !="all" ) 
{
    ...
    $buffer .= "{ key:'net_group', page:'', isFolder:true, isLazy:true, icon:'../../pixmaps/theme/net_group.png', title:'"._("Network Groups")."'},\n";
    $buffer .= "{ key:'sensor', page:'', isFolder:true, isLazy:true, icon:'../../pixmaps/theme/server.png', title:'"._("Sensors")."'}\n";
