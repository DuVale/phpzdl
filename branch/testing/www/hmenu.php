...
if($menu_opc != "")
    $menu_opc = (preg_match('/^[A-Fa-f0-9]{32}$/',$menu_opc)) ? $menu_opc : md5($menu_opc);
if($menu_sopc != "")
    $menu_sopc = (preg_match('/^[A-Fa-f0-9]{32}$/',$menu_sopc)) ? $menu_sopc : md5($menu_sopc);
			...
			<tr>
				...
				<td class="nobborder" style="padding-top:7px;">
					...
					<script type="text/javascript">
						function load_contextmenu_hmenu() {
							$('.tab2menu').contextMenu({
								...
								function(action, el, pos)
								{
									...
									if(preg_match("/pro|demo/i",$version)) //Pro
									{
										$addAll= ( Session::am_i_admin() || Acl::am_i_proadmin() ) ? true : false;
									}
									else // Open source
									{
										$addAll= ( Session::am_i_admin() ) ? true : false;
									}
						...
						<?php if (file_exists("/usr/share/ossim/www/voice_urls.php")) include("voice_urls.php"); ?>
					...
					<table class="noborder" width="100%" border='0' cellpadding='0' cellspacing='0' style="background:transparent;">
						...
						if ($hmenu[$menu_opc]) 
						{
							...
							foreach($hmenu[$menu_opc] as $j => $op)
							{									
								if ($op["ghost"] && md5($op["id"]) != $menu_sopc) 
									unset($hmenu[$menu_opc][$j]);
							}
							...
							foreach($hmenu[$menu_opc] as $j => $op) 
							{
								if ($op["name"] != "") 
								{
									...
									if (preg_match("/^http/",$op['url']))
										$url = $op["url"] . (preg_match("/\?/", $op["url"]) ? "&" : "?") . "hmenu=" . $menu_opc . "&smenu=" . $id_md5;
									else
										$url = "../" . $op["url"] . (preg_match("/\?/", $op["url"]) ? "&" : "?") . "hmenu=" . $menu_opc . "&smenu=" . $id_md5;
									...
									else 
									{
										...
										<td style="vertical-align:bottom" class="nobborder">
											<table class="noborder" border='0' cellpadding='0' cellspacing='0' height="26">
												<tr>
												<?php
												if (md5($hmenu[$menu_opc][$j - 1]["id"]) != $menu_sopc) 
												{ 
													?>
								...
								<td align="right" style="vertical-align:bottom;padding:0px;padding-left:15px;" class="nobborder">
									<table class="noborder" border='0' cellpadding='0' cellspacing='0' height="26">
										<tr>
											...
											<td class="nobborder" style="background:url(../pixmaps/menu/bgts.gif) repeat-x bottom left;padding-right:4px;line-height:16px" nowrap='nowrap'>
												<?php if ( ($hmenu[$menu_opc][$help]["config"] != "" && !$cloud_instance) ||
                                                            ($hmenu[$menu_opc][$help]["config"] != "" && $cloud_instance && Session::am_i_admin()) ) {
                                                ?><a href="" onclick="GB_show('<?php echo _('Configuration');?>','../conf/main.php?section=<?php echo $hmenu[$menu_opc][$help]["config"] ?>' ,480,'80%');return false"><img align="absmiddle" src="../pixmaps/menu/configuration.gif" border="0" title="<?=_("Configure")?>"></b></a><?php } 
                                                if (file_exists("/usr/share/ossim/www/voice_urls.php")) { ?>
                                                <script language="JavaScript">
                                                	if (document.createElement("input").webkitSpeech != undefined) {
                                                		document.write('<input type="text" size="1" style="border:0px none;width:20px;vertical-align:bottom;color:white;font-size:13px;line-height:16px" speech x-webkit-speech x-webkit-grammar="builtin:search" onwebkitspeechchange="dovoice(this.value)">');
													}
                                                </script>
                                                <?php } ?>
