...
function TimeSlotUpdate ($detail_opts) {
	...
	if ( isset($_POST['tleft']) ) {
		...
		if ( is_numeric($_tmp) ) {
			...
			if ( ($_SESSION['tleft'] < $_SESSION['tstart'])  || ($_SESSION['tleft'] > $_SESSION['tend'])) {
	...
	if ( isset($_POST['tright']) ) {
		...
		if ( is_numeric($_tmp) ) {
			...
			if ( ($_SESSION['tright'] < $_SESSION['tstart'])  || ($_SESSION['tright'] > $_SESSION['tend'])) {
...
if(preg_match("/^\d+$/", $_SESSION['tend'])) { ?>
    <input type="hidden" name="tend" value="<?php echo $_SESSION['tend'];?>" />
<?php 
} 
if(preg_match("/^\d+$/", $_SESSION['tleft'])) { ?>
    <input type="hidden" name="tleft" value="<?php echo $_SESSION['tleft'];?>" />
<?php
}
if(preg_match("/^\d+$/", $_SESSION['tright'])) { ?>
    <input type="hidden" name="tright" value="<?php echo $_SESSION['tright'];?>" />
<?php
}
if( $_SESSION["detail_opts"]["cursor_mode"] != "" ) {?>
    <input type="hidden" name="cursor_mode" value="<?php echo $_SESSION["detail_opts"]["cursor_mode"];?>" />
<?php
}
if( $_SESSION["detail_opts"]["wsize"] != "" ) {?>
    <input type="hidden" name="wsize" value="<?php echo $_SESSION["detail_opts"]["wsize"];?>" />
<?php
}
if( $_SESSION["detail_opts"]["logscale"] != "" ) {?>
    <input type="hidden" name="logscale" value="<?php echo $_SESSION["detail_opts"]["logscale"];?>" />
<?php
}
if( $_SESSION["detail_opts"]["linegraph"] != "" ) {?>
    <input type="hidden" name="linegraph" value="<?php echo $_SESSION["detail_opts"]["linegraph"];?>" />
<?php
}
?>
 		...
 		<table border="0" id="ProcessOptionTable" style="font-size:14px;font-weight:bold;border:none;width:100%">
			<tr>
                ...
                <td class='TDnfprocControl' >
                    <table class='nobborder' style='margin: auto;'>
                        <tr>
                            <td class='nobborder'><input class="button" type="button" name="JSbutton1" value="<?php echo _("Clear Form")?>" onClick="ResetProcessingForm()"/></td>
                            <td class='nobborder'><input class="button" type="submit" name="process" value="<?php echo _("Process")?>" id="process_button" onClick="clean_remote_data();form_ok=true;" size="1"/></td>
                            <?php 
                            if( count($RemoteInterfacesData)>0 && (!isset($_POST['login']))) 
                            { 
                                ?>
                                <td class='nobborder'><input class="button" type="button" name="remote_process" value="<?php echo _("Remote Process")?>" id="remote_process_button" onclick="$('#rinterfaces').toggle()"/>
                                    <div id='container_rmp' style='position:relative;'>
                                        <div id="rinterfaces" style="position:absolute; top:0; right:0;display:none; margin:1px 0px 0px 2px; text-align:right;">
                                            ...
                                            foreach($RemoteInterfacesData as $data) 
                                            {
                                                ...
                                                ?>
                                                <input type="button" class="button" onclick="remote_interface('<?php echo $data["url"]; ?>')" style="width:180px; font-size: 11px;" title="<?php echo $data["name"]." [".$data["url"]."]";?>" value="<?php echo $short_name." [".$data["url"]."]";?>"/><br />
                                                <?php
                                            }
                                            ?>
                                        </div>
