...
$option = REQUEST("hmenu");
$soption = REQUEST("smenu");
$url = REQUEST("url");

ossim_valid($option, OSS_DIGIT, OSS_LETTER, OSS_NULLABLE, 'illegal:' . _("option"));
ossim_valid($soption, OSS_DIGIT, OSS_LETTER, OSS_NULLABLE, 'illegal:' . _("soption"));
...
if($prodemo) {
    ...
    if(!Session::login_three_times(Session::get_session_user()) && $cloud_instance && Session::get_session_user()!="admin") {
        $option  = md5("My Test");
        $soption = md5("MyTest");
        $url     = "./cloud/mytest.php?hmenu=MyTest&smenu=MyTest";
    }
}
?>
...
<frameset cols="180,*" border="0" frameborder="0">
	<frame src="top.php?hmenu=<?php echo $option ?>&smenu=<?php echo $soption ?>&url=<?php echo urlencode($url) ?>" name="topmenu" scrolling="no" marginwidth='0' marginheight='0'>
