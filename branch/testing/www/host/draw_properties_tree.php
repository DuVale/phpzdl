...
switch ($tree)
{
	case "tree_container_1":
		...
		$services_list 		= Host_services::get_ip_data($conn, $ip, '', 0);
		...
		if (count($properties) > 0 )
		{
			...
			{
				if ($v['property']=="operating-system" && $already_os) continue; // only first OS property
				if ($v['property']=="operating-system") $already_os=1;
		...
		foreach ($services_list as $k => $v )
		{
            ...
            $class       = ( $v['anom'] == 1 ) ? "size12ig" : "size12n";
            $class2      = ( $v['anom'] == 1 ) ? "size10ig" : "size10n";
            
            
			$service_name = "<span class=\'".$class."\'>".$v['service']."</span><span class=\'".$class2." ml3\'> (". $v['port']."/".getprotobynumber($v['protocol']).")</span>";
			
			$service_key      = "item_prop_8_$cont_3###".$v['host']."###".$v['port']."###".$v['protocol']."###".$v['version']."###".$v['date']."###4";
	...
	case "tree_container_2":
		...
		foreach ($services_list as $k => $v )
		{
			...
			$nagios           = "<img src=\'../pixmaps/theme/".getPropertyImage($img_nagios)."\'/>";
			$service_key      = "nagios_$cont###".$v['port']."###".$v['protocol']."###".$v['version']."###".$v['date']."###".$img_nagios;
	...
	case "tree_container_3":
		...
		$cont      		  = 0;
		$is_folder 		  = ( $num_s > 0 ) ? "true" : "false";
		$json_properties  = "[{title: '<span>"._("Anomalies")."</span>', addClass:'size12', key:'property_8', isFolder:".$is_folder.", hideCheckbox: true, icon:'".$image_url.getPropertyImage('services')."', children:[" ;
        
		foreach ($services_list as $k => $v )
		{
			$cont++;
			
			$service_name = "<span class=\'size12ig \'>".$v['service']."</span><span class=\'size10ig ml3 \'> (". $v['port']."/".getprotobynumber($v['protocol']).")</span>";
			
			$select_nagios 	  = "";
			$service_key      = "anom_$cont###".$v['port']."###".$v['protocol']."###".$v['version']."###".$v['date']."###anom_ko";
			$json_properties .= "{ title: '<span>".$service_name."</span>', key:'$service_key', $select_nagios isFolder:true, icon:'".$image_url.getPropertyImage("")."', children:[";
			$json_properties .= "{ title: '<span class=\'size12n\'>"._("Version").": </span><span class=\'ml3 size12b\'>".$v['version']."</span>', key:'serv_version_".$cont."', hideCheckbox: true, isFolder:false, icon:'".$image_url.getPropertyImage('property')."'},";
			$json_properties .= "{ title: '<span class=\'size12n\'>"._("Date").": </span><span class=\'ml3 size12b\'> ".$v['date']."</span>', key:'serv_date_".$cont."', hideCheckbox: true, isFolder:false, icon:'".$image_url.getPropertyImage('property')."'},";
			$json_properties .= ($num_s == $cont) ? "]}" : "]},";
