		...
		<table align='center' id='form_container'>
			...
			<tr>
				<td class='nobborder'>
					...
					<div id='format'><?php echo _('"IP"**;"hostname";"FQDNs(FQDN1,FQDN2,... )";"Description";"Asset value";"Sensors(Sensor1,Sensor2,...)"**;"Operating System"***;"Latitude";"Longitude"')?></div>
				</td>
			</tr>
			<tr>
				<td class='nobborder'>
					<div style='padding-top: 10px'>
						...
						<div id='example'>"192.168.10.3";"Host_1";"www.example-1_esp.es,www.example-2_esp.es";"<?php echo _("Short description of host");?>";"2";"192.168.10.2,192.168.10.3";"Windows";"23.783445";"121.456652"</div>
					</div>
				</td>
			</tr>
			<tr>
				<td class='nobborder'>
					...
					<div style='padding-top: 5px'>(**)&nbsp;&nbsp; <?php echo _("Values marked with (*) are mandatory");?></div>
					<div style='padding: 5px 0px'>
						(***) <?php echo _("Valid Operating System values")?>: Windows, Linux, FreeBSD, NetBSD, OpenBSD, MacOS, Solaris, Cisco, AIX,HP-UX, Tru64, IRIX, BSD/OS, SunOS, Plan9 <?php echo _("or")?> IPhone
					</div>
				</td>
			</tr>
			<tr>
...
function csv_host_allowed($ip)
{
	...
	else
	{
		...
		{
			$ip_num   = Host::ip2ulong($ip);
			$ip_range = NET::expand_CIDR($net, "SHORT", "LONG");
			
			if ($ip_num >= $ip_range[0] && $ip_num <= $ip_range[1])
				return true;
		}
...
function clean_iic($string)
{
	...
	$val_str = OSS_SCORE.OSS_LETTER.OSS_DIGIT.OSS_DOT;
	$res     = null;
...
function import_assets_csv($filename, $iic){
	...
	foreach ($data as $k => $v)
	{
		...
		{ 
			if ( !empty($iic) )
				$param[1] = clean_iic($param[1]);
			
			if ( !ossim_valid($param[1], OSS_HOST_NAME, 'illegal:' . _("Hostname")) )
			{
		...
		if ( !empty ($param[2]) )
		{
			...
			{
				if ( !ossim_valid($fqdn, OSS_FQDNS, OSS_NULLABLE, 'illegal:' . _("FQDN/Aliases")) )
				{
		...
		if ( !ossim_valid($param[3], OSS_NULLABLE, OSS_AT, OSS_TEXT, '\t', 'illegal:' . _("Description")) )
		{
		...
		//Operating System
		if ( !empty($param[6]) && !in_array($param[6], $list_os) )
			$param[6] = "Unknown";
		...
		if ( !empty ($param[7]) )
		{
			if ( !ossim_valid(trim($param[7]), OSS_NULLABLE, OSS_DIGIT, OSS_DOT, '\-', 'illegal:' . _("Latitude")) )
			{
				$response ['line_errors'][$cont][] = array("Latitude", ossim_get_error_clean());
				$response ['status'] = false;
				ossim_clean_error();
			}
		}
		...
		if ( !empty ($param[8]) )
		{
			if ( !ossim_valid(trim($param[8]), OSS_NULLABLE, OSS_DIGIT, OSS_DOT, '\-', 'illegal:' . _("Longitude")) )
			{
				$response ['line_errors'][$cont][] = array("Longitude", ossim_get_error_clean());
				$response ['status'] = false;
				ossim_clean_error();
			}
		}
		...
		if ( csv_host_allowed($param[0]) == false )
		{
			$response ['line_errors'][$cont][] = array("IP", _("IP")." ".$param[0]." "._("not allowed. You don't have permission to import this host"));
			$response ['status'] = false;
		}	
		...
		if ( $response ['status'] == true )
		{
			...
			if ( mb_detect_encoding($param[3]." ",'UTF-8,ISO-8859-1') == 'UTF-8')
				$descr = mb_convert_encoding($param[3],'HTML-ENTITIES', 'UTF-8');
			else
				$descr = $param[3];
