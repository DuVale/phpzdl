...
if ($update==1) 
{
    $success_message = gettext("Host successfully updated");
...
if ( GET('update') == 'services' )
{
    ...
    $cmd = "($nmap -sV -P0 -O -n -oX - $ip;sleep 2) > /tmp/nmap_scan_$ip.log 2>&1 &";
    system($cmd);
}
...
<head>
	...
	<script type="text/javascript">
		...
		function saveService(){
			...
			{
				alert('<?php echo Util::js_entities(_("Error: Invalid port.  Insert a value between 0 and 65535"))?>');
				return false;
			}
		...
		function load_tree_2(container, ip){
			...
			$(layer_2).dynatree({
				...
				onSelect: function(select, node) {
					...
					var key = node.data.key.split("###");					
					if ( key[5] == "nagios_ok" )
						node.data.key = key[0]+"###"+key[1]+"###"+key[2]+"###"+key[3]+"###"+key[4]+"###nagios_ko";
					else
						node.data.key = key[0]+"###"+key[1]+"###"+key[2]+"###"+key[3]+"###"+key[4]+"###nagios_ok";
				},	
				cookieId: "dynatree_2"
			});
		...
		function load_tree_3(container, ip){
			
			if (nodetree_3!=null) {
				nodetree_3.removeChildren();
				$(layer_3).remove();
			}
			
			layer_3 = '#srctree3_'+i;
			$('#'+container).append('<div id="srctree3_'+i+'" style="width:100%"></div>');
			$(layer_3).html(messages[6]);
									
			$(layer_3).dynatree({
				initAjax: {
                    url: "draw_properties_tree.php", 
                    data: {ip: ip, tree: container}
                },
                // onPostInit: function(isReloading, isError) {
                                                   
                    // if ( $(layer_3).dynatree("getTree").getNodeByKey('property_8').hasChildren() )
                        // $('#container_anomalies').show(); 
                    // else
                        // $('#container_anomalies').hide();
				...
				onSelect: function(select, node) {
					...
					var key = node.data.key.split("###");					
					if ( key[5] == "anom_ok" )
						node.data.key = key[0]+"###"+key[1]+"###"+key[2]+"###"+key[3]+"###"+key[4]+"###anom_ko";
					else
                        node.data.key = key[0]+"###"+key[1]+"###"+key[2]+"###"+key[3]+"###"+key[4]+"###anom_ok";
				},	
				cookieId: "dynatree_3"
			});
			
			nodetree_3 = $(layer_3).dynatree("getRoot");
				...
				$.ajax({
					...
					success: function(msg){
						...
						if ( items.match("item_prop_8") != null ){
                            ...
                            load_tree_3('tree_container_3', ip);
                        }
        ...
        function update_anom_services(items)
		{
			var ip = $('#ip').val();
			$('#info_error').html('');
			$.ajax({
				type: "POST",
				url: "properties_actions.php",
				data: "action=anom&ip="+ip+"&data="+items,
				success: function(msg){
					load_tree_3('tree_container_3', ip);
                    load_tree_2('tree_container_2', ip);
					load_tree_1('tree_container_1', ip);
        ...
        function delete_anom_services(items)
		{
			var ip = $('#ip').val();
			$('#info_error').html('');
			$.ajax({
				type: "POST",
				url: "properties_actions.php",
				data: "action=delete_anom&ip="+ip+"&data="+items,
				success: function(msg){
					load_tree_3('tree_container_3', ip);
                    load_tree_2('tree_container_2', ip);
					load_tree_1('tree_container_1', ip);
		...
		$(document).ready(function(){
			...
			load_tree_3('tree_container_3', '<?php echo $ip?>');
            ...
            $('#anomaly').bind('click', function() {
			    if ( $('#anomaly').hasClass("hide") )
				{
					$('#anomaly').removeClass();
					$('#anomaly').addClass("show");
					$('#cont_sam_anom').show();
				}
				else
				{
            ...
            $("#update_anom_selected").bind('click', function() {
				var node = $(layer_3).dynatree("getRoot");
				
				var nagios_keys = new Array();
				var i = 0;
				
				$(layer_3).dynatree("getRoot").visit(function(node){
					key = node.data.key;
					if ( key.match("anom_ok") != null )
					{
						nagios_keys[i] = key;
                        console.log(key);
						i++;
				...
				nagios_keys = nagios_keys.join(",");
				update_anom_services(nagios_keys);		
				
			});
            
			
            $("#delete_anom_selected").bind('click', function() {
				var node = $(layer_3).dynatree("getRoot");
				...
				nagios_keys = nagios_keys.join(",");
				delete_anom_services(nagios_keys);		
            ...
            $("#check_all_anom").bind('click', function() {
                $(layer_3).dynatree("getRoot").visit(function(node){
                    node.select(true);
                });
                return false;
			});
            $("#uncheck_all_anom").bind('click', function() {
                $(layer_3).dynatree("getRoot").visit(function(node){
                    node.select(false);
                });
                return false;
	...
	<style type='text/css'>
		...
		#cont_uncheck_all {padding: 8px 0px 5px 0px; text-align:center;}
		...
		#table_inventory, #table_services, #container_anomalies { border: 1px dotted gray; 
						   border-radius: 0px;
	...
	<table align="center" class='noborder transparent'>
		...
		<td valign="top" class="nobborder" id='form_right'>
			...
			<table class='noborder transparent' width="100%" cellspacing="0" cellpadding="0">
									...
									<select name="inv_prop_ref" id="inv_prop_ref">
										...
										<?php
											...
											foreach ($properties_types as $k => $v)
												echo "<option value='".$v["id"]."'>".(($v["description"] != "") ? gettext($v["description"]) : "&nbsp;")."</option>";
										?>
									</select>
								</td>
							</tr>
				...
				<tr>
					<td class="nobborder left" style="padding-top:10px">
						<a id='availability' class='hide'><img border="0" align="absmiddle" src="../pixmaps/arrow_green.gif"/><?php echo _("Availability")?></a><br />
					</td>
				</tr>
							...
							<tr>
								<td class='noborder'>
									<table id='table_services' class='noborder transparent' width='100%'>
										...
										<tr>
											<td class="nobborder" id="cont_uncheck_all">
												<input type='button' class='lbutton' id='uncheck_all' value='<?php echo _("Uncheck All")?>'/> &nbsp; <input type='button' class='lbutton' id='check_all' value='<?php echo _("Check All")?>'/>
											</td>
										</tr>
										<tr>
                ...
                <tr>
					<td class="nobborder left" style="padding-top:5px">
						<a id='anomaly' class='hide'><img border="0" align="absmiddle" src="../pixmaps/arrow_green.gif"/><?php echo _("Anomalies")?></a>
					</td>
				</tr>

				<tr>
					<td class='noborder'>
						<table width='100%' class='noborder transparent' id='cont_sam_anom'>
						
							<tr>
								<th style="padding:5px">
									<?php echo _("Availability Anomaly Monitoring"); ?>
								</th>
							</tr>
							<tr>
								<td class='noborder'>
                                    <table id='container_anomalies' class='noborder transparent' width='100%'>
                                        <tr><td class="nobborder"><div id='tree_container_3'></div></td></tr>
                                        <tr>
                                            <td class="nobborder" id="cont_uncheck_all">
                                                <input type='button' class='lbutton' id='uncheck_all_anom' value='<?php echo _("Uncheck All")?>'/> &nbsp; <input type='button' class='lbutton' id='check_all_anom' value='<?php echo _("Check All")?>'/>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="nobborder" id='cont_update_selected'>
                                                <input type='button' class='lbutton' id='update_anom_selected' value='<?php echo _("Update Anomalies")?>'/> &nbsp; <input type='button' class='lbutton' id='delete_anom_selected' value='<?php echo _("Delete Anomalies")?>'/>
                                            </td>
                                        </tr>
                                    </table>
								</td>
