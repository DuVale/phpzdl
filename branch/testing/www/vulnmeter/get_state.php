...
if ($_SESSION["scanner"]=="omp") {
    ...
    */
    ...
    /*
    OpenVas doesn't fill .work file
    ...
    */
    ...
    $lineas = explode("\n",`ps -ef | grep "testing" | grep -v "/bin/sh" | grep -v grep`); 

    foreach ($lineas as $linea) if (trim($linea)!="") {
        if (preg_match("/testing (.*?) \((.*)\/(.*?\.nasl)\)/",$linea,$found)) {
            $ips[$found[3]] = $found[1];
        }
    }
    foreach ($ips as $type => $ip) {
        $details[] = "testing $ip ( $type )";
        $run++;
    }
