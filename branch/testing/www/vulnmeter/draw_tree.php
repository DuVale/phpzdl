...
$m_perms  = array ("MenuMonitors", "MenuEvents");
$sm_perms = array ("TrafficCapture", "EventsVulnerabilities");
...
$traffic_capture = intval(GET('traffic_capture'));
        ...
        foreach($hg_hosts as $hosts) 
		{
            if($j>=$from && $j<$to) 
			{
				...
				{   
                    ...
                    $html      .= "{ key:'$host_key', url:'$ip', ".( ($traffic_capture!=1) ? "isLazy:true," : "" )." icon:'../../pixmaps/theme/host.png', title:'$title', tooltip:'$tooltip' },\n";
...
else if (preg_match("/net_(.*)/",$key,$found))
{
	...
	$ips       = "<font style=\"font-size:80%\">(".Net::get_cidrs_summary($conn, $net_name).")</font>";
    ...
    if( $page==1 && $traffic_capture!=1) 
	{
        $title     = "<span style=\"color: #B3B5DD;\">!".$ips." <font style=\"font-weight:normal;font-size:80%\">(".$aux_nname.")</font></span>";
...
else if (preg_match("/netgroup_(.*)/",$key,$found))
{
    ...
    foreach($nets as $net) 
	{
        if($k>=$from && $k<$to) 
		{
			...
			$html.= "{ key:'$net_key', url:'$ips_data', ".( ($traffic_capture!=1) ? "isLazy:true," : "" )." icon:'../../pixmaps/theme/net.png', title:'$title', tooltip:'$tooltip' },\n";
        }
        $k++;
    }
	...
	if ( $num_entities > 0 || ( Session::am_i_admin() && count($entities[0]) > 0) )
	{
		foreach ($entities[0] as $entity)
		{
			...
			echo "{title:'".$title."', noLink: true, tooltip:'$tooltip', key:'".$e_key."', icon:'$icon', expand:true,  name:'$e_name'";
					echochildrens($entities, $entity['id'], $length_name);
			echo "}";
		}
	}
...
function echochildrens($entities, $parent_id, $length_name) {
    
	$length_name = $length_name - 5;
	$length_name = ( $length_name < 20 ) ? 20 : $length_name;
	...
	if ( !empty($children) )
	{
		...
		foreach ($children as $index => $child_id)
		{
			...
			$ch_name     = Util::htmlentities($child['name']);
			...
			if ( $child['parent_id'] == $parent_id )
			{
				...
				echo "{title:'".$title."', tooltip:'$tooltip', noLink: true, url:'".$child_url."', key:'".$child_key."', icon:'$icon', expand:false, name:'$ch_name'";
					echochildrens($entities, $child_id, $length_name);
				echo "}";
			}	
		}
	}
