...
else 
{
	...
	ossim_valid($sensor, OSS_PUNC, OSS_ALPHA, OSS_NULLABLE,                'illegal:' . _("Sensor"));
    ...
    if (!$submitter) 
	{
        ...
        $submitter    = $session_info['name'].'/'.$session_info['login'];
...
<form id='crt' method="POST" action="manageincident.php" enctype="multipart/form-data" target="main">
...
if ($sensor != "") {
	$users_aux = Session::get_users_to_assign($conn);
	$users = array();
	foreach ($users_aux as $u) {
		$allowed_sensors = explode(",", Session::allowedSensors($u->get_login()));
		if (in_array($sensor, $allowed_sensors)) {
			$users[] = $u;
		}
	}
	$entities_aux = Session::get_entities_to_assign($conn);
	$entities = array();
	foreach ($entities_aux as $entity_id=>$ent) {
		$perms = Acl::entityPerms($conn,$entity_id);
		if ($perms['sensors'][$sensor]) { $entities[$entity_id] = $ent; }
	}	
} else {
