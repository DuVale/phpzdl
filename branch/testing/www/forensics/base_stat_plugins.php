...
$sql = "select SQL_CALC_FOUND_ROWS max(acid_event.cid),acid_event.plugin_id,count(distinct acid_event.plugin_sid) as events,count(distinct acid_event.sid) as sensors,plugin.source_type,plugin.name  " . $fromcnt  . ",ossim.plugin " . $where . " AND plugin.id=acid_event.plugin_id GROUP BY acid_event.plugin_id " . $sort_sql[1];
//echo $sql;
...
while (($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt())) {
	...
	$temp = "SELECT acid_event.sid,acid_event.ip_src,acid_event.ip_dst,plugin_sid.name as sig_name,acid_event.timestamp FROM acid_event LEFT JOIN ossim.plugin_sid ON plugin_sid.plugin_id=acid_event.plugin_id AND plugin_sid.sid=acid_event.plugin_sid WHERE acid_event.plugin_id=$plugin_id AND cid=$max_cid LIMIT 1";
