...
if ($submit == gettext("Signature")) {
	...
	if (preg_match("/(\!?)(\d+\.\d+\.\d+\.\d+(\/\d+)?)\s*AND\s*(\!?)(\d+\.\d+\.\d+\.\d+(\/\d+)?)/",$ip_search,$fnd)) {
	    ...
	    for ($i=0;$i<count($ipsf);$i++) {
	        ...
	        $ipfilter[] = array ("",$ipop,$not."=",$ip_aux[0],$ip_aux[1],$ip_aux[2],$ip_aux[3],$ip_search,"",$or,$mask[1]);
	    }
	    
	}
    $_SESSION["ip_addr"] = $_GET["ip_addr"] = $ipfilter;
    $_SESSION["ip_addr_cnt"] = $_GET["ip_addr_cnt"] = count($ipfilter);
	...
	$_SESSION[$var] = $_GET[$var] = array ($ip_search,"both");
}
...
if ($submit == gettext("Query DB") || $submit == gettext("Query+DB") || $submit == gettext("Delete Selected") || $submit == gettext("Delete ALL on Screen") || $submit == gettext("Delete Entire Query") || $qs->isCannedQuery() || ($qs->GetCurrentSort() != "" && $qs->GetCurrentSort() != "none" && $_SERVER["QUERY_STRING"]!="new=1")) {
    ...
    $qs->AddValidActionOp(gettext("DS Group for Selected"));
