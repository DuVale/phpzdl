...
<head>
	...
	function ref_delete (id)
	{
		$.ajax({
			...
			success: function(msg){
				if (msg != "0" && msg != "") {
					if (confirm("<?php echo  Util::js_entities(_("This reference type is linked in some events (at least "))?>"+msg+"<?php echo  Util::js_entities(_("). Are you sure to delete?"))?>")) {
	...
	function formsubmit () {
		...
		else alert ("<?php echo Util::js_entities(_("Must select Data Source/Event Type pair and type a value"))?>");
	}
