...
while (($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt())) {
    ...
    */
    if ($no_ip) qroPrintEntry('<A HREF="' . $BASE_urlpath . '/help/base_app_faq.php#1">' . gettext("unknown") . '</A>');
        ...
        $ip_aux = ($sensors[$currentIP] != "") ? $sensors[$currentIP] : (($hosts[$currentIP] != "") ? $hosts[$currentIP] : $currentIP);
        $div = '<div id="'.$currentIP.';'.$ip_aux.'" class="HostReportMenu" '.getrepbgcolor($prio,1).'>';
		$bdiv = '</div>';
        qroPrintEntry( $div. getrepimg($prio,$rel,$act) . BuildAddressLink($currentIP, 32) . $ip_aux . '</A>&nbsp;' . $country_img . $homelan . $bdiv,'center','','nowrap');
    }
