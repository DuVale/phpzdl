        ...
        *   - Bill Marque (wlmarque@hewitt.com) */
		...
		foreach ($allowed_cols as $colname) {
			...
			//print_r($title);
				...
				$print_title = "";
				if ($title['key'] == "IP_PROTO") $width = "width:75px;";
				elseif (preg_match("/INPUT/",$title['key'])) $width = "width:30px;";
				elseif ($title['key'] == "RISK" || $title['key'] == "RELIABILITY" || $title['key'] == "PRIORITY" || $title['key'] == "ASSET") $width = "width:95px;";
				elseif ($title['key'] == "DATE") $width = "width:150px;";
				...
				if (!preg_match("/INPUT/",$title['key']) && isset($this->qroHeader2[$title['key']])) {
					...
					echo '<TD height="34" CLASS="plfieldhdr" style="'. $width . $border.'line-height:14px;padding-top:1px;background:url(\'../pixmaps/fondo_col.gif\') 50% 50% repeat-x;color:#222222;font-size:12px;font-weight:bold" NOWRAP>&nbsp;' . $print_title . '&nbsp;</TD>' . "\n";
				} else {
					$sort_keys = array_keys($title["value"]);
					...
					echo '<TD height="34" CLASS="plfieldhdr" style="'. $width . $border.'line-height:14px;background:url(\'../pixmaps/fondo_col.gif\') 50% 50% repeat-x;color:#222222;font-size:12px;font-weight:bold" NOWRAP>&nbsp;' . $print_title . '&nbsp;</TD>' . "\n";
				}
			}
			reset($this->qroHeader);
            $field++;
