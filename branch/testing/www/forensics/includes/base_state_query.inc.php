...
class QueryState {
    ...
    */
    ...
    function AddValidActionOp($action_op) {
        if (!(!Session::menu_perms("MenuEvents","EventsForensicsDelete") and (($action_op==gettext("Delete Selected")) or ($action_op==gettext("Delete ALL on Screen")) or ($action_op==gettext("Delete Entire Query")))))
    ...
    function RunAction($submit, $which_page, $db) {
        ...
        if ($this->action != "del_alert") {
        	ActOnSelectedAlerts($this->action, $this->valid_action_list, $submit, $this->valid_action_op_list, $this->action_arg, $which_page, $this->action_chk_lst, $this->action_lst, $show_rows, $this->num_result_rows, $this->action_sql, $this->current_canned_query,$db);
        	 ...
        	 if (!Session::menu_perms("MenuEvents","EventsForensicsDelete") and (($submit==gettext("Delete Selected")) or ($submit==gettext("Delete ALL on Screen")) or ($submit==gettext("Delete Entire Query"))))
    ...
    function PrintAlertActionButtons() {
        ...
        if ($this->valid_action_list == NULL || $this->valid_action_op_list == NULL) return;
        ...
        while ($current_op = each($this->valid_action_op_list)) {
            if ($current_op["value"] == gettext("DS Group for Selected")) { // Exceptional case: execute a javascript function, do not submit
            	echo "    <INPUT TYPE=\"button\" class=\"button\" onclick=\"dsgroup_for_selected()\" VALUE=\"" . $current_op["value"] . "\">\n";
            } else {
        		echo "    <input type=\"submit\" style=\"display:none\" id=\"eqbtn".$bt."\" NAME=\"submit\" VALUE=\"" . $current_op["value"] . "\"/><INPUT TYPE=\"button\" class=\"button\" onclick=\"if (confirm('". Util::js_entities(_("Are you sure?"))."')) $('#eqbtn".$bt."').click()\" VALUE=\"" . $current_op["value"] . "\">\n";
            }
