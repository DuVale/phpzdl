    ...
    */
    ...
    function Description() {
        ...
        if ((isset($this->criteria[0])) && ($this->criteria[0] != " ") && (isset($this->criteria[1])) && ($this->criteria[1] != "")) {
            ...
            else if ($this->criteria[0] == 'LIKE' && $this->criteria[2] == '!=') $tmp_human = ' ' . gettext("does not contain") . ' ';
    ...
    function Description() {
        ...
        if ($this->db->baseGetDBversion() >= 103) {
            if ($this->criteria[1] != " " && $this->criteria[1] != "") {
                if ($this->criteria[1] == null) $tmp = $tmp . gettext("Signature Priority") . ' = ' . '<I>' . gettext("none") . '</I><BR>';
                else $tmp = $tmp . gettext("Signature Priority") . ' ' . Util::htmlentities($this->criteria[0], ENT_COMPAT, "UTF-8") . " " . Util::htmlentities($this->criteria[1], ENT_COMPAT, "UTF-8") . $this->cs->GetClearCriteriaString($this->export_name) . '<BR>';
            }
        }
        return $tmp;
    }
...
class SourceTypeCriteria extends SingleElementCriteria {
    ...
    function SanitizeElement() {
        $this->criteria = CleanVariable($this->criteria, VAR_ALPHA | VAR_SPACE | VAR_PUNC | VAR_FSLASH);
    }
    function PrintForm() {
    }
    function ToSQL() {
...
class ReputationCriteria extends SingleElementCriteria {
    ...
    function Description() {
        ...
        if ( ($this->criteria[0] != " " && $this->criteria[0] != "") || ($this->criteria[1] != " " && $this->criteria[1] != "") ) {
        	$tmp .= _("Reputation").": ";
        	if ($this->criteria[0] != " " && $this->criteria[0] != "")  $tmp .= _("Activity") . ' = (' . $this->criteria[0] .'), ';
...
class NetworkGroupCriteria extends SingleElementCriteria {
    ...
    function Description() {
        $tmp = "";
        if ($this->criteria != " " && $this->criteria != "") $tmp = $tmp . _("Network group") . ' = (' . $this->criteria .')'. $this->cs->GetClearCriteriaString($this->export_name) . '<BR>';
        return $tmp;
    }
...
class IDMUsernameCriteria extends SingleElementCriteria {
    ...
    function Description() {
        $tmp = "";
        $tmp1 = ($this->criteria[1]!="boh") ? " " : " ".ucfirst($this->criteria[1])." ";
    ...
    */
    ...
    function PrintForm() {
		...
		for ($i = 0; $i < $this->criteria_cnt; $i++) {
            ...
            if ($i == $this->criteria_cnt - 1) echo '    <INPUT TYPE="submit" class="button" NAME="submit" VALUE="' . gettext("ADD Addr") . '">';
            echo '<BR>';
        }
    }
    function ToSQL() {
...
class Layer4Criteria extends SingleElementCriteria {
    ...
    function PrintForm() {
        if ($this->criteria != "") echo '<INPUT TYPE="submit" class="button" NAME="submit" VALUE="' . gettext("no layer4") . '"> &nbsp';
    ...
    */
    ...
    function PrintForm() {
        ...
        for ($i = 0; $i < $this->criteria_cnt; $i++) {
            ...
            if ($i == $this->criteria_cnt - 1) echo '    <INPUT TYPE="submit" class="button" NAME="submit" VALUE="' . gettext("ADD Payload") . '">';
            echo '<BR>';
        }
    }
    function ToSQL() {
