			...
			<table cellpadding='0' cellspacing='0' align="center" >
				...
				<tr>
					<td style='text-align: center; padding: 7px; font-size: 10px;' class="noborder">
						...
						<input type="button" value="<?=_("Create New Report")?>" onclick="GB_show('<?=_("Create new report")?>','custom_view_edit.php?edit=1&forcesave=1&savereport_custom_name='+document.getElementById('savereport_custom_name').value,460,700);" class="button">
					</td>
				</tr>
			</table>
		</div>
   ...
   <div style="position:relative">
		<div id="views" style="position:absolute;right:0;top:0;display:none">
			<table cellpadding='0' cellspacing='0' align="center" >
						...
						<tr class='noborder'>
							<?php 
							  if ( $_SESSION['current_cview'] == $name ){
								 ...
								 $opacidad = '';
								 $boton0 = (!$opensource && Session::am_i_admin()) ? "<a style='cursor:pointer;' onclick=\"GB_show('"._('Edit custom view')."','custom_view_edit.php?edit=1&forcesave=1',460,700);\"><img src='../pixmaps/documents-save.png' alt='"._('Save as report module')."' title='"._('Save as report module')."' border='0'/></a>&nbsp;" : "";
								 ...
								 $boton2 = "<a style='cursor:pointer;' onclick=\"GB_show('"._('Edit custom view')."','custom_view_edit.php?edit=1',460,700);\"><img src='../vulnmeter/images/pencil.png' alt='"._('Modify')."' title='"._('Modify')."' border='0'/></a>";
							  }
							  else{
								 $style='';
							...
							<td class="noborder <?=$color?>" style="padding: 0px 90px 3px 5px; text-align: left;"><a style="cursor:pointer;<?=$style?>" onclick="change_view('<?=$name?>');" id="view_<?= $name?>"><span><?=$dname?></span></a></td>
							<td class="noborder <?=$color?>" style="<?=$opacidad?> padding-right:5px;text-align:right"><?=$boton0.$boton1.$boton2;?></td>
							<td class="noborder <?=$color?>" <?php if ($name == "default") { ?>style="<?=$opacidad?>"<?php } ?>><?php if ($name != "default") { ?><a style="cursor:pointer" onclick="if(confirm('<?php echo  Util::js_entities(_("Are you sure?"))?>')) delete_view('<?php echo $name?>')"><img src="../pixmaps/delete.gif" border="0" alt="<?php echo _("Delete") ?>" title="<?php echo _("Delete") ?>"></img></a><?php } ?></td>
						</tr>
						<? } ?>
					</table>
					</td>
				...
				<tr>
					<td style='text-align: center; padding: 7px; font-size: 10px;' class="noborder">
					  <input type="button" value="<?=_("Create New View")?>" onclick="GB_show('<?=_("Create new custom view")?>','custom_view_edit.php',460,700);" class="button">
					</td>
				</tr>
			</table>
		</div>
