...
$fromcnt = (preg_match("/plugin_sid/",$fromcnt)) ? $fromcnt : $fromcnt.",ossim.plugin_sid ";
...
if ($subcat)
	$sql = "select SQL_CALC_FOUND_ROWS max(acid_event.cid),plugin_sid.subcategory_id,count(acid_event.cid) as events,count(distinct acid_event.sid) as sensors  " . $fromcnt  . $where . " AND plugin_sid.category_id=".$_SESSION["category"][0]." GROUP BY plugin_sid.category_id  HAVING plugin_sid.subcategory_id>0 " . $sort_sql[1];
else
	$sql = "select SQL_CALC_FOUND_ROWS max(acid_event.cid),plugin_sid.category_id,count(acid_event.cid) as events,count(distinct acid_event.sid) as sensors  " . $fromcnt  . $where . " GROUP BY plugin_sid.category_id  HAVING plugin_sid.category_id>0 " . $sort_sql[1];
...
while (($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt())) {
	...
	$temp = ($subcat) ? "SELECT CONCAT(c.name,' / ',s.name) as name FROM ossim.category c,ossim.subcategory s WHERE c.id=s.cat_id AND s.id = $category_id AND c.id = ".$_SESSION["category"][0] : "SELECT name FROM ossim.category WHERE id = $category_id";
	$result1 = $db->baseExecute($temp);
	$cname = $result1->baseFetchRow();
	$result1->baseFreeRows();
	if ($cname["name"]=="") continue; # Don't show unknown categories
	$temp = "SELECT acid_event.sid,acid_event.ip_src,acid_event.ip_dst,plugin_sid.name as sig_name,acid_event.timestamp FROM ossim.plugin, acid_event LEFT JOIN ossim.plugin_sid ON plugin_sid.plugin_id=acid_event.plugin_id AND plugin_sid.sid=acid_event.plugin_sid WHERE acid_event.sid!=0 AND cid=$max_cid AND acid_event.plugin_id=plugin.id LIMIT 1";
