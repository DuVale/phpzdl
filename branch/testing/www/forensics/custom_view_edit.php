...
// Edit the Current View
	...
	if ($query1 != "" && $query2 != "" && $columns != "") {
		...
		if ($curid > 0) {
	    	$sql = "UPDATE custom_report_types SET name=\"$name\",type='Custom Security Events',file='SIEM/CustomList.php',inputs='Number of Events:top:text:OSS_DIGIT:25:1000',custom_report_types.sql=\"$query1;$query2;$columns\" WHERE id=$curid";
		} else {
			$sql = "INSERT INTO custom_report_types (id,name,type,file,inputs,custom_report_types.sql) VALUES ($id,\"$name\",'Custom Security Events','SIEM/CustomList.php','Number of Events:top:text:OSS_DIGIT:25:1000',\"$query1;$query2;$columns\")";
		}
		if ($conn->Execute($sql)) {
...
$tags = Event_viewer::get_tags($idm_enabled);
if ($opensource) {
...
<input type="hidden" name="savereport_custom_name" value="<?=$savereport_custom_name?>">
    ...
    <tr><td class="center nobborder">
		...
		<? if ($edit && $_SESSION['current_cview'] != "default") { ?> <input type="button" class="lbuttonoff" onclick="document.fcols.save.value='insert';document.fcols.selected_cols.value=getselectedcombovalue('cols');document.fcols.submit()" value="<?php echo _("Save As")?>" id="saveasbutton" style="color:gray" disabled> <input type="button" class="lbutton" onclick="if(confirm('<?php echo  Util::js_entities(_("Are you sure?"))?>')) { document.fcols.save.value='delete';document.fcols.submit() }" value="<?=_("Delete")?>"><? } ?>
		<?php if (Session::am_i_admin() && $edit && !$opensource) { ?> <input type="button"  class="lbutton" onclick="document.fcols.save.value='report';document.fcols.selected_cols.value=getselectedcombovalue('cols');document.fcols.submit()" value="<?=_("Save as Report Module")?>"><?php } ?>
