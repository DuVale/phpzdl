...
$sql = "select SQL_CALC_FOUND_ROWS max(acid_event.cid),plugin.source_type,count(distinct acid_event.plugin_sid) as events,count(distinct acid_event.sid) as sensors  " . $fromcnt  . ",ossim.plugin " . $where . " AND plugin.id=acid_event.plugin_id GROUP BY plugin.source_type  HAVING plugin.source_type!='' " . $sort_sql[1];
//echo $sql;
...
while (($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt())) {
    ...
    //$urlp = "base_qry_main.php?search=1&sensor=&bsf=Query+DB&search_str=&sip=&ossim_risk_a=+&sourcetype=".urlencode($product_type);
    $urlp = "base_stat_ptypes.php?sort=occur_d&sourcetype=".urlencode($product_type);
    qroPrintEntry('&nbsp;<a href="'.$urlp.'">' . $product_type . '</a>','left',"","nowrap",$bgcolor);
