...
function GetSensorHostName($sid, $db, $sensors) {
    $name = $ip = "";
    $multiple = (preg_match("/\,/", $sid)) ? true : false;
    if ($multiple) $sid = preg_replace("/\s*\,.*/", "", $sid);
    $tmp_sql = "SELECT * FROM sensor WHERE sid='" . $sid . "'";
    ...
    if ($tmp_result) {
        ...
        if ($myrow == "") $name = "Unknown";
            ...
            preg_match('/(\d+\.\d+\.\d+\.\d+)/', $name, $found);
            if( $sensors[$found[1]] != "" ) {
                $name = preg_replace('/\d+\.\d+\.\d+\.\d+/', $sensors[$found[1]], $name);
            }
		}
    }
	else {
...
function GetSourceType($pid,$db) {
    $sourcetype = "";
    $temp_sql = "select source_type from ossim.plugin where id=$pid";
    $tmp_result = $db->baseExecute($temp_sql);
    if ($myrow = $tmp_result->baseFetchRow()) {
        $sourcetype = $myrow["source_type"];
    }
    $tmp_result->baseFreeRows();
    return $sourcetype;
}
...
function GetPluginCategoryName($idcat, $db) {
    $name = $idcat;
    $temp_sql = "SELECT name FROM ossim.category WHERE id=$idcat";
    $tmp_result = $db->baseExecute($temp_sql);
    if ($myrow = $tmp_result->baseFetchRow()) {
...
function GetPluginSubCategoryName($idcat, $db) {
    $name = $idcat[1];
    $temp_sql = "SELECT name FROM ossim.subcategory WHERE id=".$idcat[1]." and cat_id=".$idcat[0];
    $tmp_result = $db->baseExecute($temp_sql);
    if ($myrow = $tmp_result->baseFetchRow()) {
        $name = str_replace("_"," ",$myrow[0]);
    }
...
function GetCategorySubCategory($pid,$sid,$db) {
    ...
    $temp_sql = "SELECT c.name as cname,sc.name as scname FROM ossim.plugin_sid p LEFT JOIN ossim.category c ON p.category_id=c.id LEFT JOIN ossim.subcategory sc ON p.subcategory_id=sc.id AND sc.cat_id=p.category_id WHERE p.plugin_id=$pid and p.sid=$sid";
    $tmp_result = $db->baseExecute($temp_sql);
    if ($myrow = $tmp_result->baseFetchRow()) {
...
function GetPluginGroups($db) {
    $pg = array();
    $temp_sql = "SELECT group_id,name FROM ossim.plugin_group_descr ORDER BY name";
    $tmp_result = $db->baseExecute($temp_sql);
    while ($myrow = $tmp_result->baseFetchRow()) {
        $pg[$myrow["group_id"]] = $myrow["name"];
    }
    $tmp_result->baseFreeRows();
    return $pg;
}
function GetPluginGroupName($pgid, $db) {
    $name = $pgid;
    $temp_sql = "SELECT name FROM ossim.plugin_group_descr WHERE group_id=$pgid";
    $tmp_result = $db->baseExecute($temp_sql);
    if ($myrow = $tmp_result->baseFetchRow()) {
        $name = $myrow[0];
    }
...
function GetPlugins($db) {
	...
	//$plugins = array();
    //$temp_sql = "SELECT distinct plugin_id,name FROM acid_event LEFT JOIN ossim.plugin ON acid_event.plugin_id=plugin.id";
    //$tmp_result = $db->baseExecute($temp_sql);
    //while ($myrow = $tmp_result->baseFetchRow()) {
    ...
    //}
	$plugins = array();
	$temp_sql = "SELECT distinct plugin_id,name FROM ac_alerts_signature LEFT JOIN ossim.plugin ON ac_alerts_signature.plugin_id=plugin.id";
    $tmp_result = $db->baseExecute($temp_sql);
    while ($myrow = $tmp_result->baseFetchRow()) {
        $plg = ($myrow[1]=="") ? $myrow[0] : $myrow[1];
        $plg = preg_replace("/(ossec)-.*/","\\1",$plg);
...
function GetPluginName($pid, $db) {
    ...
    $temp_sql = "SELECT name FROM ossim.plugin WHERE id=$pid";
    $tmp_result = $db->baseExecute($temp_sql);
    if ($myrow = $tmp_result->baseFetchRow()) {
        $name = $myrow[0];
    }
...
function GetOssimHostGroups() {
	...
	$conn = $db->connect();
    $pg = array();
    $groups = Host_group::get_list($conn, "", " ORDER BY name");
...
function GetOssimHostsFromHostGroups($hostgroup) {
	...
	$conn = $db->connect();
    $pg = array();
    $hosts = Host_group::get_hosts($conn, $hostgroup);
    foreach ($hosts as $hg) {
        $pg[] = $hg->get_host_ip();
    }
    $conn->disconnect();
...
function BuildIDMVars($idmvalue, $field, $source="both") {
    return '&idm_'.$field.'%5B1%5D='.$source.'&idm_'.$field.'%5B0%5D='.urlencode($idmvalue);
}
function BuildIDMLink($idmvalue, $field, $source="both") {
    return '<A style="color:navy" HREF="base_qry_main.php?new=2&hmenu=Forensics&smenu=Forensics&num_result_rows=-1&submit=Query+DB&current_view=-1'.BuildIDMVars($idmvalue, $field, $source).'">';
}
...
function PrintAsciiPacketPayload($encoded_payload, $output_type) {
    ...
    return Util::htmlentities(wordwrap($encoded_payload, 144));
}
...
// rep color char
function getrepimg($prio,$rel,$act) {
	if (intval($prio)==0) return "";
	$reptxt = _("IP Priority").": <img src='../forensics/bar2.php?value=$prio&max=9&range=1' border='0' align='absmiddle'  style='width:14mm'><br>"._("IP Reliability").": <img src='../forensics/bar2.php?value=$rel&max=9' border='0' align='absmiddle' style='width:14mm'><br>"._("IP Activity").": <b>$act</b>";
	$lnk = "<a href='javascript:;' class='riskinfo' style='text-decoration:none' txt='".Util::htmlentities($reptxt)."'><img style='margin:0px 2px 2px 0px' align='absmiddle' border='0'";
