...
echo "<FORM METHOD=\"GET\" STYLE='margin:10px' ACTION=\"base_qry_alert.php\">\n";
// Normal view
...
$filename = $username = $password = $userdata1 = $userdata2 = $userdata3 = $userdata4 = $userdata5 = $userdata6 = $userdata7 = $userdata8 = $userdata9 = "<span style='color:gray'><i>"._("empty")."</i></span>";
$idm_data = array();
$payload = "";

/* Event */
//$sql2 = "SELECT signature, timestamp FROM acid_event WHERE sid='" .  filterSql($sid,$db) . "' AND cid='" .  filterSql($cid,$db) . "'";
$sql2 = "SELECT plugin_id, plugin_sid, timestamp, tzone, ip_src, ip_dst, ip_proto, layer4_sport, layer4_dport, ossim_priority, ossim_reliability, ossim_asset_src, ossim_asset_dst, ossim_risk_c, ossim_risk_a, src_username,src_domain,src_hostname,src_mac,dst_username,dst_domain,dst_hostname,dst_mac FROM acid_event WHERE sid='" . filterSql($sid,$db) . "' AND cid='" . filterSql($cid,$db) . "'";
//echo $sql2;
...
if ($plugin_id == "" || $plugin_sid == "") {
    // Try to get info from ossim.event (alarms)
    $result2->baseFreeRows();
	$sql2 = "SELECT * FROM ossim.event WHERE snort_sid='" . filterSql($sid,$db) . "' AND snort_cid='" . filterSql($cid,$db) . "'";
	$result2 = $db->baseExecute($sql2);
	$myrow2 = $result2->baseFetchRow();
	...
	$tzone = $myrow2['tzone'];
	$ip_src = $myrow2['src_ip']; $current_sip = baseLong2IP($ip_src);
	$ip_dst = $myrow2['dst_ip']; $current_dip = baseLong2IP($ip_dst);
    ...
    $filename = Util::htmlentities($myrow2["filename"]); if ($filename=="") $filename="<span style='color:gray'><i>"._("empty")."</i></span>";
    $username = Util::htmlentities($myrow2["username"]); if ($username=="") $username="<span style='color:gray'><i>"._("empty")."</i></span>";
    $password = Util::htmlentities($myrow2["password"]); if ($password=="") $password="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata1 = Util::htmlentities($myrow2["userdata1"]); if ($userdata1=="") $userdata1="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata2 = Util::htmlentities($myrow2["userdata2"]); if ($userdata2=="") $userdata2="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata3 = Util::htmlentities($myrow2["userdata3"]); if ($userdata3=="") $userdata3="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata4 = Util::htmlentities($myrow2["userdata4"]); if ($userdata4=="") $userdata4="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata5 = Util::htmlentities($myrow2["userdata5"]); if ($userdata5=="") $userdata5="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata6 = Util::htmlentities($myrow2["userdata6"]); if ($userdata6=="") $userdata6="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata7 = Util::htmlentities($myrow2["userdata7"]); if ($userdata7=="") $userdata7="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata8 = Util::htmlentities($myrow2["userdata8"]); if ($userdata8=="") $userdata8="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata9 = Util::htmlentities($myrow2["userdata9"]); if ($userdata9=="") $userdata9="<span style='color:gray'><i>"._("empty")."</i></span>";	
    $payload = _("Payload does not exists. Event deleted");
    $context = 0;
    $idm_data['src_username'] = $myrow2['src_username'];
    $idm_data['src_hostname'] = $myrow2['src_hostname'];
    $idm_data['src_domain'] = $myrow2['src_domain'];
    $idm_data['src_mac'] = $myrow2['src_mac'];
    $idm_data['dst_username'] = $myrow2['dst_username'];
    $idm_data['dst_hostname'] = $myrow2['dst_hostname'];
    $idm_data['dst_domain'] = $myrow2['dst_domain'];
    $idm_data['dst_mac'] = $myrow2['dst_mac'];
    foreach ($idm_data as $k => $v) if (empty($v)) $idm_data[$k] = "<span style='color:gray'><i>"._("empty")."</i></span>";
}
...
$encoding = (isset($myrow4["encoding"])) ? $myrow4["encoding"] : 2; # force ascii if sid doesn't found
...
if ($myrow6 = $result6->baseFetchRow()) {
    $filename = Util::htmlentities($myrow6["filename"]); if ($filename=="") $filename="<span style='color:gray'><i>"._("empty")."</i></span>";
    $username = Util::htmlentities($myrow6["username"]); if ($username=="") $username="<span style='color:gray'><i>"._("empty")."</i></span>";
    $password = Util::htmlentities($myrow6["password"]); if ($password=="") $password="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata1 = Util::htmlentities($myrow6["userdata1"]); if ($userdata1=="") $userdata1="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata2 = Util::htmlentities($myrow6["userdata2"]); if ($userdata2=="") $userdata2="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata3 = Util::htmlentities($myrow6["userdata3"]); if ($userdata3=="") $userdata3="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata4 = Util::htmlentities($myrow6["userdata4"]); if ($userdata4=="") $userdata4="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata5 = Util::htmlentities($myrow6["userdata5"]); if ($userdata5=="") $userdata5="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata6 = Util::htmlentities($myrow6["userdata6"]); if ($userdata6=="") $userdata6="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata7 = Util::htmlentities($myrow6["userdata7"]); if ($userdata7=="") $userdata7="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata8 = Util::htmlentities($myrow6["userdata8"]); if ($userdata8=="") $userdata8="<span style='color:gray'><i>"._("empty")."</i></span>";
    $userdata9 = Util::htmlentities($myrow6["userdata9"]); if ($userdata9=="") $userdata9="<span style='color:gray'><i>"._("empty")."</i></span>";
...
// idm_data
if (empty($idm_data)) {
	$sql7 = "select * from idm_data where sid = '" . intval($sid) . "' and cid = '" . intval($cid) . "';";
	$result7 = $db->baseExecute($sql7);
	if ($idm_data = $result7->baseFetchRow()) {
...
echo '
       <TABLE BORDER=0 cellpadding=2 cellspacing=0 class="bborder" WIDTH="100%">
          <TR><TD CLASS="header3" WIDTH=50 ALIGN=CENTER ROWSPAN=6>Meta</TD>
              <TD>
                  <TABLE BORDER=0 CELLPADDING=4 WIDTH="100%">
                    ...
                    <TR><TD CLASS="plfield" nowrap>' . ($sid . " - " . $cid) . '</TD>
                        ...
                        <TD CLASS="plfield"><a href="javascript:;" class="trlnka" id="'.$plugin_id.';'.$plugin_sid.'">';
	$htmlTriggeredSignature=html_entity_decode(htmlspecialchars(str_replace("##", "", BuildSigByPlugin($plugin_id, $plugin_sid, $db))));
	echo $htmlTriggeredSignature.'</a></TD>
                        <TD CLASS="plfield">' . $plugin_name . '</TD>
                        <TD CLASS="plfield">' . $plugin_id . '</TD>
                        <TD CLASS="plfield">' . $plugin_sid . '</TD>
						'.(($_GET['minimal_view'] == "") ? '<TD CLASS="plfield"><a href="javascript:;" onclick="GB_show(\''._("Modify Rel/Prio").'\',\'modify_relprio.php?id='.$plugin_id.'&sid='.$plugin_sid.'\',280,450)" ><img src="../vulnmeter/images/pencil.png" border="0" alt="'._("Modify Rel/Prio").'" title="'._("Modify Rel/Prio").'"></a></td>' : '');
//'<a href="http://cve.mitre.org/cgi-bin/cvename.cgi?name=2009-0033" target="_blank"><img src="manage_references_icon.php?id=5" alt="cve" title="cve" border="0"></a> <a href="http://cve.mitre.org/cgi-bin/cvename.cgi?name=2007-5976" target="_blank"><img src="manage_references_icon.php?id=5" alt="cve" title="cve" border="0"></a> pads: New service detectedArray';
	...
	?>
		<TD CLASS="plfield">
			<a href="javascript:;" title="<?php echo _("Info from OSVDB");?>" onclick="GB_show('Info from OSVDB','osvdb_info.php?<?php echo $arrayData; ?>',450,'90%')"><img src="../vulnmeter/images/osvdb.png" border="0"></a>
		</TD>
		<?php
	}
	//-->
...
// END COMMON DATA
        ...
        if ($_SESSION['_idm']) {
		    echo '<TR><TD>
					<TABLE BORDER=0 CELLPADDING=4 width="100%">
					<TR><TD rowspan="2" class="header2" width="80" ALIGN=CENTER>'._("IDM").'</TD>
					   <TD class="header">' . gettext("Src Username") . '</TD>
					   <TD class="header">' . gettext("Src Domain") . '</TD>
					   <TD class="header">' . gettext("Src Hostname") . '</TD>
					   <TD class="header">' . gettext("Src MAC") . '</TD>
					   <td></td>
					   <TD class="header">' . gettext("Dst Username") . '</TD>
					   <TD class="header">' . gettext("Dst Domain") . '</TD>
					   <TD class="header">' . gettext("Dst Hostname") . '</TD>
					   <TD class="header">' . gettext("Dst MAC") . '</TD>
					</TR>
					<TR>
					  <TD class="plfield" nowrap>' . ((!empty($myrow2["src_username"])) ? $myrow2["src_username"] : "<span style='color:gray'><i>"._("empty")."</i></span>") . '</TD>
					  <TD class="plfield" nowrap>' . ((!empty($myrow2["src_domain"])) ? $myrow2["src_domain"] : "<span style='color:gray'><i>"._("empty")."</i></span>" ). '</TD>
					  <TD class="plfield" nowrap>' . ((!empty($myrow2["src_hostname"])) ? $myrow2["src_hostname"] : "<span style='color:gray'><i>"._("empty")."</i></span>") . '</TD>
					  <TD class="plfield" nowrap>' . ((!empty($myrow2["src_mac"])) ? $myrow2["src_mac"] : "<span style='color:gray'><i>"._("empty")."</i></span>") . '</TD>
					  <td></td>
					  <TD class="plfield" nowrap>' . ((!empty($myrow2["dst_username"])) ? $myrow2["dst_username"] : "<span style='color:gray'><i>"._("empty")."</i></span>") . '</TD>
					  <TD class="plfield" nowrap>' . ((!empty($myrow2["dst_domain"])) ? $myrow2["dst_domain"] : "<span style='color:gray'><i>"._("empty")."</i></span>") . '</TD>
					  <TD class="plfield" nowrap>' . ((!empty($myrow2["dst_hostname"])) ? $myrow2["dst_hostname"] : "<span style='color:gray'><i>"._("empty")."</i></span>") . '</TD>
					  <TD class="plfield" nowrap>' . ((!empty($myrow2["dst_mac"])) ? $myrow2["dst_mac"] : "<span style='color:gray'><i>"._("empty")."</i></span>") . '</TD>
					</TR>
					</TABLE>
			     </TD</TR>';
        }
		...
		/* Reputation */
		if (!empty($idm_data)) {
			$src_img = getrepimg($idm_data["rep_prio_src"],$idm_data["rep_rel_src"],$idm_data["rep_act_src"]);
			$dst_img = getrepimg($idm_data["rep_prio_dst"],$idm_data["rep_rel_dst"],$idm_data["rep_act_dst"]);
			$src_bgcolor = getrepbgcolor($idm_data["rep_prio_src"],1);
			$dst_bgcolor = getrepbgcolor($idm_data["rep_prio_dst"],1);
		    echo '<TR><TD>
					<TABLE BORDER=0 CELLPADDING=4 width="100%">
					<TR><TD rowspan="2" class="header2" width="80" ALIGN=CENTER>'._("Reputation").'</TD>
					   <TD class="header">' . gettext("Source Address") . '</TD>
					   <TD class="header">' . gettext("Priority") . '</TD>
					   <TD class="header">' . gettext("Reliability") . '</TD>
					   <TD class="header">' . gettext("Activity") . '</TD>
					   <td></td>
					   <TD class="header">' . gettext("Destination Address") . '</TD>
					   <TD class="header">' . gettext("Priority") . '</TD>
					   <TD class="header">' . gettext("Reliability") . '</TD>
					...
					<TR>
					  <TD class="plfield" '.$src_bgcolor.' nowrap>' . $src_img . $current_sip . '</TD>
					  <TD class="plfield" '.$src_bgcolor.' nowrap>' . $idm_data["rep_prio_src"] . '</TD>
					  <TD class="plfield" '.$src_bgcolor.' nowrap>' . $idm_data["rep_rel_src"] . '</TD>
					  <TD class="plfield" '.$src_bgcolor.' nowrap>' . $idm_data["rep_act_src"] . '</TD>
					  <td></td>
					  <TD class="plfield" '.$dst_bgcolor.' nowrap>' . $dst_img . $current_dip. '</TD>
					  <TD class="plfield" '.$dst_bgcolor.' nowrap>' . $idm_data["rep_prio_dst"] . '</TD>
					  <TD class="plfield" '.$dst_bgcolor.' nowrap>' . $idm_data["rep_rel_dst"] . '</TD>
					  <TD class="plfield" '.$dst_bgcolor.' nowrap>' . $idm_data["rep_act_dst"] . '</TD>
					</TR>
					</TABLE>
			     </TD</TR>';
...
if (in_array($plugin_id, $snort_ids)) {
	if ($plugin_id==1001) {
		...
		if (trim($result)=="" || count($found)<=1) {
			...
			$rule = $found[2];
			echo "<TR><TD class='plfield' style='text-align:left'><b>Rule:</b> ".Util::htmlentities(str_replace(",",", ",$rule))."</TD></TR>\n";
			$more = explode(";",$found[3]);
			foreach ($more as $dat) {
				$val = explode(":",$dat);
				if ($val[0]!="") echo "<TR><TD class='plfield' style='text-align:left;padding-left:25px'><b>".Util::htmlentities(trim($val[0])).":</b> ".Util::htmlentities($val[1])."</TD></TR>\n";
			}
		}		
	    echo '</TABLE>';
	}
...
else echo "<script>if (typeof(load_tree)=='function') load_tree();</script>";
echo "<br></body></html>";
?>
