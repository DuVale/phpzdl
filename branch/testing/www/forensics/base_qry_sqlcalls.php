...
$qro->AddTitle("SENSOR");
$qro->AddTitle("SRC_USERNAME");
$qro->AddTitle("DST_USERNAME");
$qro->AddTitle("SRC_DOMAIN");
$qro->AddTitle("DST_DOMAIN");
...
if (!$printing_ag) {
	...
	if (!$opensource && Session::am_i_admin()) PrintReportView();
...
while (($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt())) {
    ...
    if ($need_extradata && !array_key_exists("username",$myrow)) {
	    ...
	    while ($row_ed = $rs_ed->baseFetchRow()) {
	    	foreach ($row_ed as $k => $v) $myrow[$k] = Util::htmlentities($v);
	    }
	    $rs_ed->baseFreeRows();
	}
    //
    // Reputation info
    //
	$rs_id = $qs->ExecuteOutputQueryNoCanned("SELECT * FROM idm_data WHERE sid=".$myrow["sid"]." AND cid=".$myrow["cid"], $db);
    while ($row_id = $rs_id->baseFetchRow()) {
    	foreach ($row_id as $k => $v) $myrow[strtoupper($k)] = Util::htmlentities($v);
    }
    $rs_id->baseFreeRows();
    //
    // SID, CID, PLUGIN_*
    ...
    if ( Session::menu_perms("MenuIncidents", "IncidentsOpen") )
        $link_incident = "<a class='trlnk greybox' title='"._('New Event ticket')."' href=\"../incidents/newincident.php?nohmenu=1&" . "ref=Event&" . "title=" . urlencode($nidespues) . "&" . "priority=1&" . "src_ips=".$current_sip."&" . "event_start=".$myrow['timestamp']."&" . "event_end=".$myrow['timestamp']."&" . "src_ports=".$myrow["layer4_sport"]."&" . "dst_ips=".$current_dip."&" . "dst_ports=".$myrow["layer4_dport"]."&hmenu=Tickets&smenu=Tickets\"><img style='vertical-align:text-top;' src='../pixmaps/script--pencil.png' width='12' alt='"._('New Event ticket')."' border='0'/></a>";
    
    // 1- Checkbox
    qroPrintEntry($solera.' '.$link_incident.' <INPUT class="trlnks" TYPE="checkbox" pid="'.$myrow['plugin_id'].'" psid="'.$myrow['plugin_sid'].'" NAME="action_chk_lst[' . $i . ']" VALUE="' . htmlspecialchars($tmp_rowid) . '">',"","","","style='border-left:1px solid white;border-top:1px solid white; text-align:center;' nowrap");
    echo '    <INPUT TYPE="hidden" NAME="action_lst['.$i.']" VALUE="'.htmlspecialchars($tmp_rowid).'">';
    // 2- Signature    
    $temp = "$antes <A class='trlnka' id='".$myrow["plugin_id"].";".$myrow["plugin_sid"]."' HREF='base_qry_alert.php?submit=" . rawurlencode($tmp_rowid) . "&amp;sort_order=";
    ...
    */
    ...
    if ($current_sip32 != "") {
        ...
        $div = '<div id="'.$current_sip.';'.$ip_aux.'" id2="'.$current_sip.';'.$current_dip.'" class="HostReportMenu">';
		$bdiv = '</div>';
        ...
        // IDM?
        if ($myrow["src_username"]!="" || $myrow["src_hostname"]!="") {
        	$idmtxt = _("IDM Username").": <b>".$myrow["src_username"]."</b><br>"._("IDM Hostname").": <b>".$myrow["src_hostname"]."</b><br>"._("IDM Domain").": <b>".$myrow["src_domain"]."</b><br>"._("IDM MAC").": <b>".$myrow["src_mac"]."</b><br>"._("IDM IP").": <b>$current_sip</b>";
        	$sip_aux = $myrow["src_username"]."@".$myrow["src_hostname"];
        	$sip_where = '&idm_username%5B1%5D=src&idm_username%5B0%5D='.urlencode($myrow["src_username"]).'&idm_hostname%5B1%5D=src&idm_hostname%5B0%5D='.urlencode($myrow["src_hostname"]);
        	$sip_lnk = '<A class="trlnk idminfo" txt="'.Util::htmlentities($idmtxt,ENT_QUOTES).'" style="color:navy;text-decoration:none" HREF="base_qry_main.php?new=2&hmenu=Forensics&smenu=Forensics&num_result_rows=-1&submit=Query+DB&current_view=-1'.$sip_where.'">'.$sip_aux.'</A>';
        } else {
        	$sip_lnk = '<A class="trlnk" alt="'.$current_sip.'" title="'.$current_sip.'" HREF="base_qry_main.php?new=2&hmenu=Forensics&smenu=Forensics&num_result_rows=-1&submit=Query+DB&current_view=-1&ip_addr_cnt=1&sort_order=time_d&ip_addr%5B0%5D%5B0%5D=+&ip_addr%5B0%5D%5B1%5D=ip_src&ip_addr%5B0%5D%5B2%5D=%3D&ip_addr%5B0%5D%5B3%5D='.$current_sip.'&ip_addr%5B0%5D%5B8%5D=+">'.$sip_aux.'</A><FONT SIZE="-1">' . $current_sport . '</FONT>';
        }
		$cell_data['IP_PORTSRC'] = $div.getrepimg($myrow["REP_PRIO_SRC"],$myrow["REP_REL_SRC"],$myrow["REP_ACT_SRC"]) . $sip_lnk . $country_img . $homelan . $bdiv;
		$cell_bgcolor['IP_PORTSRC'] = getrepbgcolor($myrow["REP_PRIO_SRC"]);
        $cell_pdfdata['IP_PORTSRC'] = $sip_aux.$current_sport.$slnk;
		$cell_data['IP_SRC'] = getrepimg($myrow["REP_PRIO_SRC"],$myrow["REP_REL_SRC"],$myrow["REP_ACT_SRC"]) . $current_sip . $country_img . $homelan;
        $cell_bgcolor['IP_SRC'] = getrepbgcolor($myrow["REP_PRIO_SRC"]);
    ...
    if ($current_dip32 != "") {
        ...
        $div = '<div id="'.$current_dip.';'.$ip_aux.'" id2="'.$current_sip.';'.$current_dip.'" class="HostReportMenu">';
		$bdiv = '</div>';
        ...
        // IDM?
        if ($myrow["dst_username"]!="" || $myrow["dst_hostname"]!="") {
        	$idmtxt = _("IDM Username").": <b>".$myrow["dst_username"]."</b><br>"._("IDM Hostname").": <b>".$myrow["dst_hostname"]."</b><br>"._("IDM Domain").": <b>".$myrow["dst_domain"]."</b><br>"._("IDM MAC").": <b>".$myrow["dst_mac"]."</b><br>"._("IDM IP").": <b>$current_dip</b>";
        	$dip_aux = $myrow["dst_username"]."@".$myrow["dst_hostname"];
        	$dip_where = '&idm_username%5B1%5D=dst&idm_username%5B0%5D='.urlencode($myrow["dst_username"]).'&idm_hostname%5B1%5D=dst&idm_hostname%5B0%5D='.urlencode($myrow["dst_hostname"]);
        	$dip_lnk = '<A class="trlnk idminfo" txt="'.Util::htmlentities($idmtxt,ENT_QUOTES).'" style="color:navy;text-decoration:none" HREF="base_qry_main.php?new=2&hmenu=Forensics&smenu=Forensics&num_result_rows=-1&submit=Query+DB&current_view=-1'.$dip_where.'">'.$dip_aux.'</A>';
        } else {
        	$dip_lnk = '<A class="trlnk" alt="'.$current_dip.'" title="'.$current_dip.'" HREF="base_qry_main.php?new=2&hmenu=Forensics&smenu=Forensics&num_result_rows=-1&submit=Query+DB&current_view=-1&ip_addr_cnt=1&sort_order=time_d&ip_addr%5B0%5D%5B0%5D=+&ip_addr%5B0%5D%5B1%5D=ip_src&ip_addr%5B0%5D%5B2%5D=%3D&ip_addr%5B0%5D%5B3%5D='.$current_sip.'&ip_addr%5B0%5D%5B8%5D=+">'.$dip_aux.'</A><FONT SIZE="-1">' . $current_dport . '</FONT>';
        }        
		$cell_data['IP_PORTDST'] = $div.getrepimg($myrow["REP_PRIO_DST"],$myrow["REP_PRIO_DST"],$myrow["REP_ACT_DST"]) . $dip_lnk . $country_img . $homelan . $bdiv;
        $cell_bgcolor['IP_PORTDST'] = getrepbgcolor($myrow["REP_PRIO_DST"]);
        $cell_pdfdata['IP_PORTDST'] = $dip_aux.$current_dport.$dlnk;
		$cell_data['IP_DST'] = getrepimg($myrow["REP_PRIO_DST"],$myrow["REP_PRIO_DST"],$myrow["REP_ACT_DST"]) . $current_dip . $country_img . $homelan;
        $cell_bgcolor['IP_DST'] = getrepbgcolor($myrow["REP_PRIO_DST"]);
   ...
   /*if ($current_dip != "255.255.255.255") {
	...
	$risk_detail = str_replace("10mm","14mm","Asset S->D: ".$cell_pdfdata['ASSET']."<br>Priority: ".$cell_pdfdata['PRIORITY']."<br>Reliability: ".$cell_pdfdata['RELIABILITY']."<br>Risk A->C: <img src='".$current_url."/forensics/bar2.php?value=" . $current_oriskc . "&value2=" . $current_oriska . "&max=9&range=1' border='0' align='absmiddle' style='width:10mm'>");
	...
	// X- ExtraData
	$cell_data['USERNAME'] = Util::wordwrap($myrow['username'],25," ",true);
	$cell_data['PASSWORD'] = Util::wordwrap($myrow['password'],25," ",true);
	$cell_data['FILENAME'] = Util::wordwrap($myrow['filename'],25," ",true);
	$cell_data['PAYLOAD'] = Util::wordwrap($myrow['data_payload'],25," ",true);
	for ($u = 1; $u < 10; $u++)
		$cell_data['USERDATA'.$u] = ($i<9) ? Util::wordwrap($myrow['userdata'.$u],25," ",true) : $myrow['userdata'.$u];

	// IDM-Reputation Data
	$cell_data['SRC_USERNAME'] = Util::htmlentities($myrow['src_username']);
	$cell_align['SRC_USERNAME'] = "center";
	$cell_data['DST_USERNAME'] = Util::htmlentities($myrow['dst_username']);
	$cell_align['DST_USERNAME'] = "center";
	$cell_data['SRC_DOMAIN'] = Util::htmlentities($myrow['src_domain']);
	$cell_align['SRC_DOMAIN'] = "center";
	$cell_data['DST_DOMAIN'] = Util::htmlentities($myrow['dst_domain']);
	$cell_align['DST_DOMAIN'] = "center";
    ...
    foreach ($_SESSION['views'][$_SESSION['current_cview']]['cols'] as $colname) {
        ...
        if ($cell_tooltip[$colname]!="") 
        	...
        	qroPrintEntry($cell_data[$colname], $cell_align[$colname],"",$cell_more[$colname],$cell_bgcolor[$colname]);        	
