...
while (($myrow = $result->baseFetchRow()) && ($i < $qs->GetDisplayRowCnt())) {
    ...
    if ($sip && $dip) {
        ...
        $sip_aux = ($sensors[$ip_sip] != "") ? $sensors[$ip_sip] : (($hosts[$ip_sip] != "") ? $hosts[$ip_sip] : $ip_sip);
        $div1 = '<div id="'.$ip_sip.';'.$sip_aux.'" class="HostReportMenu">'; $bdiv1 = '</div>';
        ...
        $dip_aux = ($sensors[$ip_dip] != "") ? $sensors[$ip_dip] : (($hosts[$ip_dip] != "") ? $hosts[$ip_dip] : $ip_dip);
		$div2 = '<div id="'.$ip_dip.';'.$dip_aux.'" class="HostReportMenu">'; $bdiv2 = '</div>';        
        if ($fqdn=="yes") qroPrintEntry('<FONT>' . $sip_fqdn . '</FONT>');
        qroPrintEntry($div1 .BuildAddressLink($ip_sip , 32) . $sip_aux . '</A>' . $s_country_img . $homelan_sip . $bdiv1, "", "", "nowrap");
        qroPrintEntry('<img src="images/dash.png" border="0">');
        qroPrintEntry($div2. BuildAddressLink($ip_dip , 32) . $dip_aux . '</A>' . $d_country_img . $homelan_dip . $bdiv2, "", "", "nowrap");
