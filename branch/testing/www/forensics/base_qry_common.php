...
function PrintCriteria($caller) {
    ...
    $save_criteria.= '&nbsp;&nbsp;</TD>';
    $save_criteria.= '<TD>';
    if (!$cs->criteria['ip_addr']->isEmpty() || !$cs->criteria['ip_field']->isEmpty() || !$cs->criteria['networkgroup']->isEmpty() || !$cs->criteria['idm_username']->isEmpty() || !$cs->criteria['idm_hostname']->isEmpty() || !$cs->criteria['idm_domain']->isEmpty()  || !$cs->criteria['rep']->isEmpty()) {
					...
					<TD style="border:1px solid #CACACA">
						<table cellpadding=0 cellspacing=0 border=0 WIDTH="100%">
							<tr>
								...
								<th style="border-right:1px solid #CACACA;border-bottom:1px solid #CACACA;background-color:#eeeeee">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?=($_SESSION["_idm"]) ? _("IDM") : _("IP")?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
...
function GetPluginListBySourceType($sourcetype) {
    ...
    if ($sourcetype==_("Unknown type")) 
    	$sql = "SELECT id FROM ossim.plugin WHERE source_type='' OR source_type is NULL"; 
    else
...
function ProcessCriteria() {
    ...
    $tmp_meta = "";
    /* Sensor */
    if ($sensor != "" && $sensor != " ") $tmp_meta = $tmp_meta . " AND acid_event.sid $sensor_op ( ".preg_replace("/^\!/","",$sensor)." )";
    else {
		$cs->criteria['sensor']->Set("");
	}
	// Filter by user perms if no criteria (this is a bug, must be ALWAYS)
    ...
    if ($idm_username[0]!="") {
    	if ($idm_username[1]=="both") {
    		$tmpcrit = "(acid_event.src_username='".str_replace("'","\'",$idm_username[0])."' OR acid_event.dst_username='".str_replace("'","\'",$idm_username[0])."')";
    	} else {
    		$tmpcrit = "acid_event.".$idm_username[1]."_username='".str_replace("'","\'",$idm_username[0])."'";
    	}
    	$tmp_meta .= " AND $tmpcrit";
    }
    if ($idm_hostname[0]!="") {
    	if ($idm_hostname[1]=="both") {
    		$tmpcrit = "(acid_event.src_hostname='".str_replace("'","\'",$idm_hostname[0])."' OR acid_event.dst_hostname='".str_replace("'","\'",$idm_hostname[0])."')";
    	} else {
    		$tmpcrit = "acid_event.".$idm_hostname[1]."_hostname='".str_replace("'","\'",$idm_hostname[0])."'";
    	}
    	$tmp_meta .= " AND $tmpcrit";
    }
    if ($idm_domain[0]!="") {
    	if ($idm_domain[1]=="both") {
    		$tmpcrit = "(acid_event.src_domain='".str_replace("'","\'",$idm_domain[0])."' OR acid_event.dst_domain='".str_replace("'","\'",$idm_domain[0])."')";
    	} else {
    		$tmpcrit = "acid_event.".$idm_domain[1]."_domain='".str_replace("'","\'",$idm_domain[0])."'";
    	}
    	$tmp_meta .= " AND $tmpcrit";
    }        
    /* Reputation */
    ...
    if ($rep_data) {
    	...
    	$tmp_meta .= " AND acid_event.sid=idm_data.sid AND acid_event.cid=idm_data.cid";
	}    
	if (trim($rep[0])!="") { # Activity
    	if ($rep[0]!="ANY") {
    		$tmpcrit = "(idm_data.rep_act_src='".str_replace("'","\'",$rep[0])."' OR idm_data.rep_act_dst='".str_replace("'","\'",$rep[0])."')";
    	} else {
    		$tmpcrit = "(idm_data.rep_act_src!='' OR idm_data.rep_act_dst!='')";
    	}
    	$tmp_meta .= " AND $tmpcrit";
    }
	if (trim($rep[1])!="") { # Severity
		switch ($rep[1]) {
			case "High": 
				$tmpcrit = "(idm_data.rep_prio_src>6 OR idm_data.rep_prio_dst>6)";
				break;

			case "Medium": 
				$tmpcrit = "(idm_data.rep_prio_src in (3,4,5,6) OR idm_data.rep_prio_dst in (3,4,5,6))";
				break;

			case "Low": 
				$tmpcrit = "(idm_data.rep_prio_src in (0,1,2) OR idm_data.rep_prio_dst in (0,1,2))";
				break;
			
			default:
				$tmpcrit = "(idm_data.rep_prio_src>0 OR idm_data.rep_prio_dst>0)";
		}
    	$tmp_meta .= " AND $tmpcrit";
    }
    /* Source Type */
    ...
    */
    ...
    if ($domain != "") {
        ...
        $criteria_sql = $criteria_sql . " AND (".implode(" OR ",$cidrs).")";
    }
    /* IP Addresses */
    $tmp2 = "";
