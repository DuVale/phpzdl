...
else
{
	...
	switch ($action)
	{
				...
				if ( is_array ($agents) )
				{
					...
					if (count($agents) == 1 )
					{
						$header = "<tr>
									...
									<th>"._("Name")."</th>
									<th>"._("IP/CIDR")."</th>";
						if ($_SESSION["_idm"]) $header .= "<th>"._("Current IP")."</th>
									<th>"._("Current User")."</th>
									<th>"._("Current Domain")."</th>";
						$header .= "<th>"._("Status")."</th>
					...
					if ( is_array($agent_field))
					{
						...
						// IDM Data
						if ($_SESSION["_idm"]) {
							$agent_idm_data = (!empty($idm[$agent_field[1]])) ? $idm[$agent_field[1]] : array("user"=>"-","ip"=>"-","domain"=>"-");
						}
						echo "1###"._("Agent added sucessfully")."###".$agent_field[0]."###".$header.
							"<tr id='cont_agent_".$agent_field[0]."'>
								  ...
								  <td>".$agent_field[2]."</td>";
						if ($_SESSION["_idm"]) {
							echo "<td>".$agent_idm_data["ip"]."</td>
							 	  <td>".$agent_idm_data["user"]."</td>
								  <td>".$agent_idm_data["domain"]."</td>";
						}
						echo "    <td>".$agent_field[3]."</td>
		...
		case "modified_files":
			...
			exec("sudo /var/ossec/bin/syscheck_control -s -i $id", $result, $ret);	
		
			if ( $ret !== 0 )
				echo "error###"._("Error to extract modified files for agent")." (1)";
			else
			{
				if ( $result[0] == "" ) 
				...
				if ( count($result) > 0 )
				{
					echo "1###<div style='font-weight: bold; font-size: 11px; padding: 10px 0px;'>"._($header)."</div>";
				
					echo "<table class='table_files' id='tf'>";
						...
						echo "<tbody>";
							foreach ($result as $line) 
							{
								...
								if ( !empty($data) )
								{
									if ( empty($dates[$data[0]]) )
									{
										$dates[$data[0]] = $data[0];
										$days  			 = $days + 1;
										$color 			 = ( $days%2 == 0) ? 'background:#FFFFFF' :'background:#EEEEEE';
									}
									
									echo "<tr>
											<td class='cf_date' style='$color'>".$data[0]."</td>
											<td class='cf_path' style='$color'>".$data[1]."</td>
											<td class='cf_ocurrences' style='$color'>".$data[2]."</td>
										  </tr>";
								}
							}
						echo "</tbody>";
				...
				{
					echo "1###<div class='ossim_info'>"._($header)." <span style='font-weight: bold;'>"._("No results")."</span></div>";
				}
			}
		...
		case "modified_reg_files":
			...
			exec("sudo /var/ossec/bin/syscheck_control -r -s -i $id", $result, $ret);	
						
			
			if ( $ret !== 0 )
				echo "error###"._("Error to extract modified registry files for agent")." (1)";
			else
			{
				if ( $result[0] == "" ) 
					$result = array_slice($result,1);
		...
		case "rootcheck":
			...
			exec("sudo /var/ossec/bin/rootcheck_control -s -i $id", $result, $ret);	
						
			
			if ( $ret !== 0 )
				echo "error###"._("Error to extract policy and auditing events for agent")." (1)";
			else
			{
				if ( $result[0] == "" ) 
					$result = array_slice($result,1);
												
				exec("sudo /var/ossec/bin/rootcheck_control -i $id | head -2", $header, $ret);	
				
				if ( count($result) > 0 )
				{
					echo "1###<div style='font-weight: bold; font-size: 11px; padding: 10px 0px;'>"._($header[1])."</div>";
						...
						echo "<tbody>";
							foreach ($result as $line) 
							{
								...
								if ( !empty($data) )
								{
									...
									echo "<tr>
											<td class='cf_type' style='$color'>".$data[0]."</td>
											<td class='cf_date' style='$color'>".$data[1]."</td>
											<td class='cf_date' style='$color'>".$data[2]."</td>
											<td style='$color;' class='cf_path'>".$data[3]."</td>
										  </tr>";
								}
							}
						echo "</tbody>";
				...
				{
					echo "1###<div class='ossim_info'>"._($header[1])." <span style='font-weight: bold;'>"._("No results")."</span></div>";
				}
			}
