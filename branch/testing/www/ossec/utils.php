...
function get_actions ($agent)
{
	...
	{
		$key    = "<a id='_key##".$agent[0]."'><img src='$path/key--arrow.png' align='absmiddle' alt='"._("Extract Key")."' title='"._("Extract Key")."'/></a>";
		$mfiles = "<a id='_file##".$agent[0]."'><img src='$path/file.png' align='absmiddle' alt='"._("Modified Files")."' title='"._("Modified Files")."'/></a>";
		$mreg   = "<a id='_reg##".$agent[0]."'><img src='$path/registry.gif' align='absmiddle' alt='"._("Modified Reg. Entries")."' title='"._("Modified Reg. Entries")."'/></a>";
		$rchk   = "<a id='_rchk##".$agent[0]."'><img src='$path/radar.gif' align='absmiddle' alt='"._("Rootcheck")."' title='"._("Rootcheck")."'/></a>";
		$delete = "<a id='_del##".$agent[0]."'><img src='$path/delete.gif' align='absmiddle' alt='"._("Delete Agent")."' title='"._("Delete Agent")."'/></a>";
	}
	else
	{
		$key    = "<span class='unbind'><img src='$path/key--arrow.png' align='absmiddle' alt='"._("Extract Key")."' title='"._("Extract Key")."'/></span>";
		$mfiles = "<span class='unbind'><img src='$path/file.png' align='absmiddle' alt='"._("Modified Files")."' title='"._("Modified Files")."'/></span>";
		$mreg   = "<span class='unbind'><img src='$path/registry.gif' align='absmiddle' alt='"._("Modified Reg. Entries")."' title='"._("Modified Reg. Entries")."'/></span>";
		$rchk   = "<span class='unbind'><img src='$path/radar.gif' align='absmiddle' alt='"._("Rootcheck")."' title='"._("Rootcheck")."'/></span>";
		$delete = "<span class='unbind'><img src='$path/delete.gif' align='absmiddle' alt='"._("Delete Agent")."' title='"._("Delete Agent")."'/></span>";
	}
		
	$restart = "<a id='_restart##".$agent[0]."'><img src='$path/clock.png' align='absmiddle' alt='"._("Restart Agent")."' title='"._("Restart Agent")."'/></a>";
	$check   = "<a id='_check##".$agent[0]."'><img src='$path/tick-circle.png' align='absmiddle' alt='"._("Integrity/rootkit checking")."' title='"._("Integrity/rootkit checking")."'/></a>";
		
	return $restart.$check.$key.$mfiles.$mreg.$rchk.$delete;
}
...
function SIEM_trends_hids($agent_ip,$agent_name,$use_sensor=true)
{
	...
	// Agent ip filter
	if (!$use_sensor && Host::ip2ulong($agent_ip)>0) {
        // use $agent_ip like src_ip
    	$agent_where = Host::ip2ulong($agent_ip); 

    	$sqlgraph = "SELECT COUNT(acid_event.sid) as num_events, day(convert_tz(timestamp,'+00:00','$tzc')) as intervalo, monthname(convert_tz(timestamp,'+00:00','$tzc')) as suf FROM snort.acid_event LEFT JOIN ossim.plugin ON acid_event.plugin_id=plugin.id WHERE ip_src=$agent_where AND timestamp BETWEEN '".gmdate("Y-m-d 00:00:00",gmdate("U")-604800)."' AND '".gmdate("Y-m-d 23:59:59")."' $plugins_sql $sensor_where GROUP BY suf,intervalo ORDER BY suf,intervalo";
    } else {
	    // use $agent_ip like sensor and get sids
    	if ($agent_ip=="0.0.0.0/0") $agent_ip = preg_replace("/_\d+_.*/","",$agent_name);
...
function get_idm_data() {
	// IDM Info			
	require_once ('ossim_conf.inc');
	$conf        = $GLOBALS["CONF"];
	$_SESSION["_idm"] = $idm_enabled = $conf->get_conf("enable_idm", FALSE);
	$idm_backup  = "/tmp/idm_backup";
	$idm = array();
	if ($idm_enabled && file_exists($idm_backup)) {
		$idm_data = @json_decode(@join("",@file($idm_backup)));
		if (is_array($idm_data)) {
			// Format after json decode:
			// 84.218.167.226;<MSSP disabled>
			// 1320924883
			// idm-event ip="84.218.167.226" sensor="20" username="cGVkcm8=" hostname="jamon york" domain="RUVVVQ==" mac="A9:72:6D:4A:29:1A"
			for ($i=0;$i<count($idm_data);$i++) {
				$line = $idm_data[$i];
				if (preg_match('/ip="(.*?)" sensor="(.*?)" (.*)/',$line,$fnd)) {
					
					$date = $idm_data[$i-1]; // unixtime from previous line
				    $ip = $fnd[1];
				    $sensor = $fnd[2];
				    $username = $hostname = $domain = $mac = "";
				    if (preg_match('/username="(.*?)"/',$fnd[3],$fnd1)) $username = @base64_decode($fnd1[1]);
				    if (preg_match('/hostname="(.*?)"/',$fnd[3],$fnd1)) $hostname = $fnd1[1];
				    if (preg_match('/domain="(.*?)"/',$fnd[3],$fnd1))   $domain   = @base64_decode($fnd1[1]);
				    if (preg_match('/mac="(.*?)"/',$fnd[3],$fnd1))      $mac      = $fnd1[1];
				    
					if (!isset($idm[$hostname]["date"]) || $date>$idm[$hostname]["date"])
						$idm[$hostname] = array("user"=>$username, "hostname"=>$hostname, "ip"=>$ip, "domain"=>$domain, "mac"=>$mac, "date"=>$date);
				}
			}
		}
	}
...
function get_ossec_data($type,$agent,$data) {
    $date = 0;
    $agent = preg_replace("/[;']/","",$agent);
    if ($type=="Syscheck" && $data=="Unknown") {
    	$tmp = explode("\n",`sudo /usr/share/ossim/scripts/ossec_check.sh syscheck '$agent'`);
    	foreach ($tmp as $line) if (preg_match("/\!(\d+)\!(\d+) Starting syscheck scan/",$line,$fnd)) {
    		if ($fnd[1]>$date) $date = $fnd[1];
    	}
    	if ($date>0) $data = gmdate("D M j H:i:s Y",$date);
    }
    if ($type=="Rootcheck" && $data=="Unknown") {
    	$tmp = explode("\n",`sudo /usr/share/ossim/scripts/ossec_check.sh rootcheck '$agent'`);
    	foreach ($tmp as $line) if (preg_match("/\!(\d+)\!(\d+) Starting rootcheck scan/",$line,$fnd)) {
    		if ($fnd[1]>$date) $date = $fnd[1];
    	}        
    	if ($date>0) $data = gmdate("D M j H:i:s Y",$date);
    }    
