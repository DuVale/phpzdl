          ...
          if (event->textfields[i] != NULL)
            {
              ...
              gint field_size = strlen(event->textfields[i]);
              base64_params[i].base64data =g_base64_encode(event->textfields[i],field_size);
              g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "%s:%d b64 %s=\"%s\"", __FILE__, __LINE__, sim_text_field_get_name(i), base64_params[i].base64data);
            }
          else
            {
          ...
          if (base64_params[inx].base64data)
            {
              ...
              base64_params[inx].base64data = NULL;
            }
