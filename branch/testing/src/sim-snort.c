  ...
  do
    {
      ...
      case SIM_COMMAND_SYMBOL_SNORT_EVENT_PACKET_TYPE:
        ...
        break;
#if 0	/* We need this for 2.x  sensors */
	case SIM_COMMAND_SYMBOL_SNORT_EVENT_DATE:
		g_scanner_get_next_token (scanner); /* = */
		g_scanner_get_next_token (scanner); /* Discard */
		if (scanner->token != G_TOKEN_STRING)
			r = FALSE;
		break;
	case SIM_COMMAND_SYMBOL_SNORT_EVENT_DATE_STRING:
		g_scanner_get_next_token (scanner); /* = */
		g_scanner_get_next_token (scanner); /* Discar data */
		if (scanner->token != G_TOKEN_STRING)
			r = FALSE;
		break;
#endif 
...
sim_command_snort_event_scan(SimCommand *command, GScanner *scanner)
{
  ...
  //    g_free(real_time_str);
	if (r){
		if (command->data.event.date == 0)
			command->data.event.date = time (NULL);
...
update_snort_database(SimEvent *event)
{
  ...
  {
    r = FALSE;
    query = g_strdup_printf("INSERT INTO iphdr (sid,cid,ip_src,ip_dst,ip_ver,ip_hlen,ip_tos,ip_len,ip_id,ip_flags,ip_off,ip_ttl,ip_proto,ip_csum) VALUES (%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u)", sid, cid, ntohl(packet->sim_iphdr.ip_src), /* byte host order */
    ntohl(packet->sim_iphdr.ip_dst), packet->sim_iphdr.ip_v, packet->sim_iphdr.ip_hl, packet->sim_iphdr.ip_tos, packet->sim_iphdr.ip_len, packet->sim_iphdr.ip_id, packet->sim_iphdr.ip_off & 0x7, (packet->sim_iphdr.ip_off & 0xf8) >> 3, packet->sim_iphdr.ip_ttl, packet->sim_iphdr.ip_p, packet->sim_iphdr.ip_sum);
    ...
    if (packet->sim_iphdr.ip_hl > 5)
    {
      ...
      for (k = 0; k < packet->sim_iphdr.nOptions; k++)
      {
        ...
        {
          query = g_strdup_printf("INSERT INTO opt (sid,cid,optid,opt_proto,opt_code,opt_len,opt_data) VALUES(%u,%u,%u,%u,%u,%u,'%s')", sid, cid, j, 0, packet->sim_iphdr.options[i], 0, "");
          j++;
          i++;
          sim_database_execute_no_query(ossim.dbsnort, query);
          g_free(query);
        ...
        else
        {
          ...
          query = g_strdup_printf("INSERT INTO opt (sid,cid,optid,opt_proto,opt_code,opt_len,opt_data) VALUES	(%u,%u,%u,%u,%u,%u,'%s')", sid, cid, j, 0, optcode, optlen, hexdata);
          sim_database_execute_no_query(ossim.dbsnort, query);
          g_free(query);
          query = NULL;
          i = i + 2 + optlen;
    ...
    case IPPROTO_TCP:
      ...
      query = g_strdup_printf("INSERT INTO tcphdr (sid,cid,tcp_sport,tcp_dport,tcp_seq,tcp_ack,tcp_off,tcp_res,tcp_flags,tcp_win,tcp_csum,tcp_urp) VALUES (%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u)", sid, cid, packet->hdr.sim_tcphdr.th_sport, packet->hdr.sim_tcphdr.th_dport, packet->hdr.sim_tcphdr.th_seq, packet->hdr.sim_tcphdr.th_ack, packet->hdr.sim_tcphdr.th_off, packet->hdr.sim_tcphdr.th_x2, packet->hdr.sim_tcphdr.th_flags, packet->hdr.sim_tcphdr.th_win, packet->hdr.sim_tcphdr.th_sum,
      ...
      if (packet->hdr.sim_tcphdr.th_off > 5)
      {
        ...
        {
          if (packet->hdr.sim_tcphdr.th_opt[i] == 0 || packet->hdr.sim_tcphdr.th_opt[i] == 1)
          {
            //FIXME: Only one should be needed (opt from ip or opt from tcp)
            query = g_strdup_printf("INSERT IGNORE INTO opt (sid,cid,optid,opt_proto,opt_code,opt_len,opt_data) VALUES(%u,%u,%u,%u,%u,%u,'%s')", sid, cid, j, 6, packet->sim_iphdr.options[i], 0, "");
            j++;
            i++;
            sim_database_execute_no_query(ossim.dbsnort, query);
            g_free(query);
          ...
          else
          {
            ...
            query = g_strdup_printf("INSERT IGNORE INTO opt (sid,cid,optid,opt_proto,opt_code,opt_len,opt_data) VALUES	(%u,%u,%u,%u,%u,%u,'%s')", sid, cid, j, 6, optcode, optlen, hexdata);
            sim_database_execute_no_query(ossim.dbsnort, query);
            g_free(query);
            query = NULL;
            i = i + 2 + optlen;
      ...
      break;
    case IPPROTO_UDP:
      query = g_strdup_printf("INSERT INTO udphdr (sid,cid,udp_sport,udp_dport,udp_len,udp_csum) VALUES (%u,%u,%u,%u,%u,%u) ", sid, cid, packet->hdr.sim_udphdr.uh_sport, packet->hdr.sim_udphdr.uh_dport, packet->hdr.sim_udphdr.uh_ulen, packet->hdr.sim_udphdr.uh_sum);
      ...
      break;
    case IPPROTO_ICMP:
      query = g_strdup_printf("INSERT INTO icmphdr (sid,cid,icmp_type,icmp_code,icmp_csum,icmp_id,icmp_seq) VALUES (%u,%u,%u,%u,%u,%u,%u) ", sid, cid, packet->hdr.sim_icmphdr.icmp_type, packet->hdr.sim_icmphdr.icmp_code, packet->hdr.sim_icmphdr.icmp_cksum, packet->hdr.sim_icmphdr.un.echo.id, packet->hdr.sim_icmphdr.un.echo.sequence);
      sim_database_execute_no_query(ossim.dbsnort, query);
