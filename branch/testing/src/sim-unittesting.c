...
static gint sim_unittesting_signals[LAST_SIGNAL] = { 0 };
 ...
 */
SimUnittesting*
sim_unittesting_new (gchar *filedata)
{
  SimUnittesting *unittesting;

  g_return_val_if_fail (filedata, NULL);

  unittesting = SIM_UNITTESTING (g_object_new (SIM_TYPE_UNITTESTING, NULL));

  unittesting->_priv->unittests = g_list_alloc();
  if (unittesting->_priv->unittests == NULL) {
ossim_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Uops... problems loading unittests\n");
    return NULL;
  }
...
sim_unittesting_append(SimUnittesting *engine, gchar *name, gboolean (*func)(void), guint expected) {
  ...
  if (test == NULL) {
ossim_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Error appending unittest %s to the engine. Could not allocate memory\n", name);
      return;
  }
