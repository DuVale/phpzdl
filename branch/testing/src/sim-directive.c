...
sim_directive_impl_finalize (GObject  *gobject)
{
  ...
  if (!directive->_priv->loaded_from_file)
		...
		switch (var->type)
		{
			case SIM_RULE_VAR_SRC_IA:
				    ...
				    {										
					    case SIM_RULE_VAR_SRC_IA:
										if (var->negated)
									    sim_rule_prepend_src_inet_not (rule, sim_inet_new_from_ginetaddr (ia));
										else
									    sim_rule_prepend_src_inet (rule, sim_inet_new_from_ginetaddr (ia));
							      break;
					    case SIM_RULE_VAR_DST_IA:
										if (var->negated)
											sim_rule_prepend_dst_inet_not (rule, sim_inet_new_from_ginetaddr (ia));
										else
											sim_rule_prepend_dst_inet (rule, sim_inet_new_from_ginetaddr (ia));
							      break;
					    default:
							      break;
						}
...
-------------*/
      ...
      case SIM_RULE_VAR_SENSOR:
        ...
        if (var->negated)
					sim_rule_add_sensor_not (rule, sim_sensor_new_from_hostname(aux));
				else
					sim_rule_add_sensor (rule, sim_sensor_new_from_hostname(aux));
        break;
...
sim_directive_backlog_get_update_clause (SimDirective *directive)
{
  ...
  strftime(timestamp, TIMEBUF_SIZE, "%Y-%m-%d %H:%M:%S", gmtime(&curtime));
  query = g_strdup_printf ("UPDATE backlog SET matched = %d, timestamp = '%s' "
			   " WHERE id = %u",
			   directive->_priv->matched,
         timestamp,
			   directive->_priv->backlog_id);
...
void static sim_directive_delete_database_backlog(SimDirective *backlog){
	...
	if (dm)
	{
      ...
      sim_directive_set_alarm_as_removable(backlog_id);
...
void sim_directive_purge_db_backlogs(SimDatabase *db)
{
	...
	query = "DELETE FROM backlog WHERE id NOT IN (SELECT DISTINCT(backlog_id) FROM alarm)";
  sim_database_execute_no_query (db, query);
	...
	query = "DELETE FROM backlog_event WHERE backlog_id NOT IN (SELECT DISTINCT(backlog_id) FROM alarm)";
  sim_database_execute_no_query (db, query);

	// Deleting rows in event
	g_log (G_LOG_DOMAIN,G_LOG_LEVEL_DEBUG,"%s: Deleting rows from event table without backlog", __FUNCTION__);
	query = "DELETE FROM event WHERE alarm = 0 AND id NOT IN (SELECT DISTINCT(event_id) FROM backlog_event)";
  sim_database_execute_no_query (db, query);

  // Deleting rows from backlog_event without event
  g_log (G_LOG_DOMAIN,G_LOG_LEVEL_DEBUG,"%s: Deleting rows from backlog_event table without event", __FUNCTION__);
  query = "DELETE FROM backlog_event WHERE event_id NOT IN (SELECT DISTINCT(id) FROM event)";
  sim_database_execute_no_query (db, query);

  // Deleting rows from backlog without backlog_event
  g_log (G_LOG_DOMAIN,G_LOG_LEVEL_DEBUG,"%s: Deleting rows from backlog table without backlog_event", __FUNCTION__);
  query = "DELETE FROM backlog WHERE id NOT IN (SELECT DISTINCT(backlog_id) FROM backlog_event)";
  sim_database_execute_no_query (db, query);

  // Updating alarm table. Previous alarms stored are now removable
  g_log (G_LOG_DOMAIN,G_LOG_LEVEL_DEBUG,"%s: Updating previous alarms", __FUNCTION__);
  query = "UPDATE alarm set removable = TRUE";
  sim_database_execute_no_query (db, query);
 ...
 */
void
sim_directive_set_alarm_as_removable(guint32 backlog_id)
{
  gchar *query;

  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "%s: Updating removable field in alarm table", __func__);
  query = g_strdup_printf("UPDATE alarm SET removable = 1 WHERE backlog_id = %d", backlog_id);
