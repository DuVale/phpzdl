...
sim_server_new(SimConfig *config)
{
  ...
  {
    g_message ("Loading reputation from file: %s", config->reputation.filename);
    server->_priv->reputation = sim_reputation_new (config->reputation.filename);
  } 
  else
    g_message ("%s: no reputation data available", __func__);
...
sim_server_append_session(SimServer *server, SimSession *session)
{
  ...
  g_atomic_int_inc (&server->_priv->session_counter);
...
sim_server_remove_session(SimServer *server, SimSession *session)
{
  ...
  g_atomic_int_dec_and_test (&server->_priv->session_counter);
...
gint sim_server_get_session_count(SimServer *server)
{
  gint len;

  // This may be inaccurate.
  if ((len = g_atomic_int_get (&server->_priv->session_counter)) < 0)
  {
    g_atomic_int_set (&server->_priv->session_counter, 0);
    return (0);
  }
  else
    return (len);
...
{
  guint nactive_sessions = 0;
  g_return_val_if_fail (SIM_IS_SERVER (server), 0);
  g_mutex_lock(server->_priv->sessions_mutex);
  nactive_sessions = sim_server_get_session_count_active_ul(server);
  g_mutex_unlock (server->_priv->sessions_mutex);
  return nactive_sessions;
}
guint sim_server_get_session_count_active_ul(SimServer *server)
{
  guint nactive_sessions = 0;
  GList *list;
  g_return_val_if_fail (SIM_IS_SERVER (server), 0);
