...
SimRadixKey *sim_radix_key_new(void) {
  ...
  if (!key) {
		g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "ERROR with malloc!\n");
    return NULL;
  }
...
//g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Different bit at Cnt %"PRIu8" Keyiter:%"PRIu8", iterbit:%d keybit:%d\n", cnt, keyiter, GET_BIT(iter->key->keydata[keyiter / 8], keyiter % 8), GET_BIT(key->keydata[cnt / 8], cnt % 8));
// g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "iterlen:%d keylen:%d\n", iter->key->keylen, key->keylen);
        ...
        if ( !k) {
					g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Failed to create key!\n");
          return NULL;
        }
        ...
        if ( !n) {
					g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Failed to create node (splitting iter node)!\n");
          return NULL;
        }
...
// g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Equal\n");
      }
    }
...
// g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "NO PATH TO LEFT, Need to append!\n");
          break;
        }
...
// g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "NO PATH TO RIGHT, Need to append!\n");
          break;
        }
...
// g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "We should not be here\n");
    }
...
g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "ERROR with malloc!\n");
      ...
      return node;
		...
		g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Error on radix (1)\n");
  } else {
		g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Error on radix (2)\n");
  }
	g_debug (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Error on radix (3)\n");
...
SimRadix *sim_radix_clone(SimRadix *tree)
{
...
int sim_radix_test11() {
  int result = 1;
  SimRadix *radix = sim_radix_new(free, pr, update, NULL);
  SimRadixKey *key1 = sim_radix_key_create(strdup("Hola"), 4 * 8);
  SimRadixKey *key2 = sim_radix_key_create(strdup("Hoca"), 4 * 8);
  SimRadixKey *key3 = sim_radix_key_create(strdup("Hocb"), 4 * 8);
  SimRadixKey *key4 = sim_radix_key_create(strdup("Hpcb"), 2 * 8);
  SimRadixKey *key5 = sim_radix_key_create(strdup("Hppp"), 4 * 8);
