...
{
  SimConfig *config;
  guint sim_total_events_popped; //needed for sim_scheduler_show_stats() to know the number of events that the organizer processes
};
...
sim_organizer_instance_init(SimOrganizer *organizer)
{
  ...
  organizer->_priv->sim_total_events_popped = 0;
}
...
sim_organizer_run(SimOrganizer *organizer)
{
  ...
  while (TRUE)
  {
    ...
    if (event->plugin_id != SIM_PLUGIN_ID_TYPE_DIRECTIVE_EVENT) //only count the real events, not the alarms (wich are events with the alarm flag activated)
    {
    ...
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_organizer_run: event->policy: %x, bool: %d", event->policy, ev_act);
    if (event->alarm || ev_act)
    {
      g_object_ref(event);
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_organizer_run: Inserting it into queue to send it to frameworkd");
...
/* FIXME: after the changes above, there is a lot of things here that won't work. */
    ...
    { //For example, a user may want to do cross-correlation between 2 new plugins, say 25000 and 25001.
      ...
      event->is_reliability_set = TRUE;
      event->correlation |= EVENT_MATCH_CROSS_CORR;
            g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "%s: Match! plugin id: %d. Succesfull attack. new reliability: %d",  __func__, plugin_id, event->reliability);
  ...
  if (policy)
    {
      ...
      {
        if ((aux = sim_plugin_sid_get_priority(plugin_sid)) == -1) //if -1 (return value), priority doesn't exists
        {
      ...
      g_debug ("sim_organizer_reprioritize: Policy Match. new priority: %d", event->priority);
    }
  else //set the priority from plugin DB (if not prioritized early in other server down in architecture).
    {
      ...
      {
        if ((aux = sim_plugin_sid_get_priority (plugin_sid)) == -1)
        {
          g_message("Error: Unable to fetch priority for plugin id %d, plugin sid %d", event->plugin_id, event->plugin_sid);
          return 0;
        }
        ...
        g_debug ("sim_organizer_reprioritize: Policy Doesn't match, priority: %d",event->priority);
      }
    }
...
//            new_event->sensor = gnet_inetaddr_get_canonical_name(ia);
          ...
          else if ((ia = sim_rule_get_sensor(rule_root)))
          {
            new_event->sensor = gnet_inetaddr_get_canonical_name(ia);
          }
          else
          {
            g_warning("Event without sensor: event->id: %d",event->id);
          }
      ...
      stickys = g_list_prepend(stickys, GINT_TO_POINTER(id));
 ...
 */
gint
sim_organizer_snort_sensor_get_sid(SimDatabase *db_snort, SimEvent *event, gchar *plugin_name)
{
  GdaDataModel *dm;
  GdaValue *value;
  GdaValue *value2;
  ...
  gchar *query;
  gchar *insert;
  gchar *hostname = NULL;//Do not free
  ...
  if (dm)
  {
    ...
    {
      if (plugin_name)
        insert = g_strdup_printf("INSERT INTO sensor (hostname, interface, encoding, last_cid, sensor) VALUES ('%s-%s', '%s', 2, 0, '%s')", hostname, plugin_name, event->interface, event->sensor);
      else
        insert = g_strdup_printf("INSERT INTO sensor (hostname, interface, detail, encoding, last_cid, sensor) VALUES ('%s', '%s', 1, 0, 0, '%s')", hostname, event->interface, event->sensor);
...
sim_organizer_snort_event_get_max_cid(SimDatabase *db_snort, gint sid)
{
  ...
  /* CID */
  query = g_strdup_printf("SELECT max(cid) FROM acid_event WHERE sid = %d", sid);
  dm = sim_database_execute_single_command(db_snort, query);
  if (dm)
    {
          ...
          if (!gda_value_is_null(value))
            {
              ...
              g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_organizer_snort_event_get_max_cid: %u Query: %s",  last_cid, query);
            }
        }
 ...
 */
void
sim_organizer_snort_event_update_acid_event(SimDatabase *db_snort, SimEvent *event, gint sid, gulong cid)
{
  ...
  query
      = g_strdup_printf(
          ...
          sid, cid, timestamp, (event->src_ia) ? sim_inetaddr_ntohl(
  ...
  g_free(query);
  sim_organizer_snort_event_update_acid_event_ac(db_snort, event, sid, cid, timestamp);
  sim_container_increments_events_count (ossim.container);
}
 ...
 */
void
sim_organizer_snort_event_update_acid_event_ac(SimDatabase *db_snort,  SimEvent *event, gint sid, gulong cid, gchar *timestamp)
{
  g_return_if_fail(db_snort);
  g_return_if_fail(SIM_IS_DATABASE (db_snort));
  gchar *query;
...
void
sim_organizer_snort_extra_data_insert(SimDatabase *db_snort, SimEvent *event, gint sid, gulong cid)
{
  ...
  if (event->log)
    populated_data = event->log;
  else
    populated_data = event->data;

  if (populated_data)
  {
    e_data = g_new0(gchar, strlen(populated_data) * 2 + 1);
    gda_connection_escape_string(conn, populated_data, e_data);
  }
  ...
  else
  {
    ...
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "update_snort_database: no snort payload %s", e_data);
    g_string_free(st, TRUE);
  }
  st = g_string_new("");
  for (i = 0; i < N_TEXT_FIELDS; i++)
...
sim_organizer_snort_idm_data_insert(SimDatabase *db_snort, SimEvent *event)
{
  GdaConnection *conn;
  gint storage_type;
  gchar *src_ip = NULL, *dst_ip = NULL;
  gchar *e_rep_act_src = NULL, *e_rep_act_dst = NULL;
  gchar *query, *values;
  guint sid = event->snort_sid;
  guint cid = event->snort_cid;
  ...
  if (event->rep_act_src)
  {
    ...
    gda_connection_escape_string (conn, event->rep_act_src, e_rep_act_src);
  }
  if (event->rep_act_dst)
  {
    e_rep_act_dst = g_new0 (gchar, strlen(event->rep_act_dst) * 2 + 1);
    gda_connection_escape_string (conn, event->rep_act_dst, e_rep_act_dst);
  }

  if (event->rep_prio_src)
    src_ip = gnet_inetaddr_get_canonical_name (event->src_ia);
  if (event->rep_prio_dst)
    dst_ip = gnet_inetaddr_get_canonical_name (event->dst_ia); 

  values = g_strdup_printf("(%u, %u,'%s','%s',%u,%u,%u,%u,'%s','%s')",
      sid, cid,
      src_ip ? src_ip : "",
      dst_ip ? dst_ip : "",
      event->rep_prio_src,
  ...
  query = g_strdup_printf ("INSERT IGNORE INTO idm_data (sid, cid, rep_ip_src, rep_ip_dst, rep_prio_src, rep_prio_dst, rep_rel_src, rep_rel_dst, rep_act_src, rep_act_dst) VALUES %s", values);
  sim_database_execute_no_query(db_snort, query);
    ...
    SimEvent *event, gint sid)
{
  ...
  if (event->src_ia)
    g_string_append_printf(where, " AND ip_src = %lu", sim_inetaddr_ntohl(
        event->src_ia));
  if (event->dst_ia)
    g_string_append_printf(where, " AND ip_dst = %lu", sim_inetaddr_ntohl(
        event->dst_ia));
  ...
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "%s: Query: %s", __func__, select->str);
      ...
      for (row = 0; row < gda_data_model_get_n_rows(dm); row++)
        {
          ...
          g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "%s: cid: %d", __func__, cid);
...
sim_organizer_snort(SimOrganizer *organizer, SimEvent *event)
{
...
#if 0
  // Deprecated: the info is already in each column not is necessary to rebuild data

  gchar *new_data_aux = NULL;
  ...
  event->data = g_strdup(new_data_aux);
  ...
  /* Events SNORT */
  if ((event->plugin_id >= SIM_PLUGIN_ID_SNORT_MIN) && (event->plugin_id < SIM_PLUGIN_ID_SNORT_MAX))
  {
    guint snort_sid;
    guint snort_cid;
  ...
  else /* Other Events */
  {
    ...
    sid = sim_organizer_snort_sensor_get_sid(ossim.dbsnort, event, sim_plugin_get_name(plugin));
    ...
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_organizer_snort pluginid: %d , pluginsid: %d", event->plugin_id, event->plugin_sid);
    ...
    switch (event->plugin_id)
    {
      ...
      case SIM_EVENT_HOST_MAC_EVENT: //arpwatch
        ...
        {
          sim_container_db_insert_host_mac_ul(ossim.container, ossim.dbossim, event->src_ia, timestamp, event->data_storage[0], //new mac
      ...
      case SIM_EVENT_HOST_OS_EVENT: //P0f, OS event
        if ((event->plugin_sid == EVENT_NEW) || (event->plugin_sid == EVENT_CHANGE))
        {
          sim_container_db_insert_host_os_ul(ossim.container, ossim.dbossim, event->src_ia, timestamp, event->sensor, event->interface, event->data_storage[0]); //OS
        }
        g_strfreev(event->data_storage);
        break;
    ...
    case SIM_EVENT_HOST_SERVICE_EVENT: //pads, service event
      if ((event->plugin_sid == EVENT_NEW) || (event->plugin_sid == EVENT_CHANGE))
      {
        ...
        {
            sim_container_db_insert_host_service_ul(ossim.container, ossim.dbossim, event->src_ia, timestamp, atoi(event->data_storage[0]), //port
    ...
    case SIM_EVENT_HOST_IDS_EVENT: //prelude, HIDS event
      if (event->data_storage)
      {
        //cid & sid needed for extra_data (username, userdata1.....)
          sim_container_db_insert_host_ids_event_ul(ossim.container, ossim.dbossim, ossim.dbsnort, event, timestamp, sid, cid);
...
sim_organizer_rrd(SimOrganizer *organizer, SimEvent *event)
{
  ...
  if (event->plugin_id != SIM_PLUGIN_ID_TYPE_RDD_ANOMALY_EVENT) // Return if not rrd_anomaly
    return;
  ...
  if (name)
  {
    ...
    insert = g_strdup_printf("INSERT INTO rrd_anomalies(ip, what, anomaly_time, anomaly_range) VALUES ('%s', '%s', '%s', '0')", name, plugin_sid_name, timestamp);
    sim_database_execute_no_query(ossim.dbossim, insert);
    if(plugin_sid_name)
    {
      g_free(plugin_sid_name);
      plugin_sid_name = NULL;
    }
    if(insert)
    {
      g_free(insert);
...
{
  organizer->_priv->sim_total_events_popped++;
}

guint sim_organizer_get_total_events(SimOrganizer *organizer)
{
  return organizer->_priv->sim_total_events_popped;
}
