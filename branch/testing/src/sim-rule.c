...
struct _SimRulePrivate {
  ...
  gboolean    src_home_net_not;
	GList				*dst_inets_not;
  gboolean    dst_home_net_not;
  ...
  gboolean    src_home_net;
  GList				*dst_inets;
  gboolean    dst_home_net;
...
*/
  ...
  rule->_priv->src_home_net_not = FALSE;
	// !dst ips 
  ...
  rule->_priv->dst_home_net_not = FALSE; 
 ...
 */
void
sim_rule_prepend_src_inet (SimRule    *rule,
												  SimInet    *inet)
{
  g_return_if_fail (rule);
  g_return_if_fail (SIM_IS_RULE (rule));
  ...
  rule->_priv->src_inets = g_list_prepend (rule->_priv->src_inets, inet);
}
 ...
 */
void
sim_rule_prepend_dst_inet (SimRule    *rule,
												  SimInet    *inet)
{
  g_return_if_fail (rule);
  g_return_if_fail (SIM_IS_RULE (rule));
  ...
  rule->_priv->dst_inets = g_list_prepend (rule->_priv->dst_inets, inet);
}
												 ...
												 SimSensor  *sensor)
{
  ...
  guint	hash = gnet_inetaddr_hash (sim_sensor_get_ia (sensor));
	...
	if(rule->_priv->sensors == NULL)
		rule->_priv->sensors = g_hash_table_new_full (g_direct_hash, g_direct_equal, NULL, g_object_unref);

	g_hash_table_insert (rule->_priv->sensors, GUINT_TO_POINTER(hash), sensor);
}
													...
													SimSensor  *sensor)
{
  ...
  g_return_if_fail (SIM_IS_SENSOR (sensor));
	...
	guint hash = gnet_inetaddr_hash (sim_sensor_get_ia (sensor));
	g_hash_table_remove (rule->_priv->sensors, GUINT_TO_POINTER(hash));
}
 ...
 */
void 
sim_rule_prepend_src_inet_not (SimRule *rule, 
															SimInet *src_inet) 
{
	g_return_if_fail (rule); 
	g_return_if_fail (SIM_IS_RULE (rule)); 
	...
	rule->_priv->src_inets_not = g_list_prepend (rule->_priv->src_inets_not, src_inet); 
}
 ...
 */
void 
sim_rule_prepend_dst_inet_not (SimRule *rule, 
															SimInet *dst_inet) 
{
	g_return_if_fail (rule); 
	g_return_if_fail (SIM_IS_RULE (rule)); 
	...
	rule->_priv->dst_inets_not = g_list_prepend (rule->_priv->dst_inets_not, dst_inet); 
}
...
*/
 ...
 // src ips not 
	...
	{ 
		SimInet *inet = (SimInet *) list->data; 
		new_rule->_priv->src_inets_not = g_list_prepend (new_rule->_priv->src_inets_not, sim_inet_clone (inet)); 
		list = list->next; 
	} 
  ...
  // dst ips not 
	...
	{ 
		SimInet *inet = (SimInet *) list->data; 
		new_rule->_priv->dst_inets_not = g_list_prepend (new_rule->_priv->dst_inets_not, sim_inet_clone (inet)); 
		list = list->next; 
	} 
...
// HOME_NET related fields
  new_rule->_priv->src_home_net =  rule->_priv->src_home_net;
  new_rule->_priv->dst_home_net =  rule->_priv->dst_home_net;
  new_rule->_priv->src_home_net_not =  rule->_priv->src_home_net_not;
  new_rule->_priv->dst_home_net_not =  rule->_priv->dst_home_net_not;
 ...
 *
 */
gboolean _find_match_sensors(gpointer key, gpointer value, gpointer user_data)
{
	SimSensor *cmp_sensor = (SimSensor *) value; //each rule can handle multiple sensors
	SimEvent	*event = (SimEvent *) user_data;
	gchar *tmp = gnet_inetaddr_get_canonical_name (sim_sensor_get_ia(cmp_sensor));	//get the dotted decimal name.
...
gboolean _find_match_not_sensors(gpointer key, gpointer value, gpointer user_data)
{
	GInetAddr *sensor_ia = (GInetAddr *) user_data;
  SimSensor *cmp_sensor = (SimSensor *) value;
...
//  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_rule_match_by_event: printing event: %s",lala);
  ...
  if (rule->_priv->sensors_not)
  {
    GInetAddr *sensor_ia = gnet_inetaddr_new_nonblock (event->sensor, 0);
	 	...
	 	if(g_hash_table_find (rule->_priv->sensors_not, _find_match_not_sensors, sensor_ia))
    {
      gnet_inetaddr_unref (sensor_ia);
      return FALSE;
    }
    gnet_inetaddr_unref (sensor_ia);
...
/*
  ...
  if (rule->_priv->sensors)
  {
		if (!g_hash_table_find(rule->_priv->sensors, _find_match_sensors, event))
		  return FALSE;
	}
  ...
  if ((rule->_priv->occurrence > 1) && (rule->_priv->sticky_different))
  {
    ...
    switch (rule->_priv->sticky_different)
        ...
        if (g_hash_table_lookup (rule->_priv->sticky_int, GINT_TO_POINTER(val)))
					return FALSE;
        g_hash_table_insert (rule->_priv->sticky_int, GINT_TO_POINTER(val), GINT_TO_POINTER (GENERIC_VALUE));
				break;
...
{	
  ...
  if (ip_src && ip_dst)
  {
    ...
    rule->_priv->sensor = (event->sensor) ? gnet_inetaddr_new_nonblock (event->sensor, 0) : NULL;
		for (i = 0 ; i < N_TEXT_FIELDS; i++){
          rule->_priv->eventvalue[i] = event->isTextMatched[i]  ? g_strdup (event->textfields[i]) : g_strdup (SIM_WILDCARD_EMPTY);
		}
/*
  ...
  if (rule->_priv->sensors)
	{
		g_hash_table_iter_init(&iter, rule->_priv->sensors);
		if(g_hash_table_iter_next (&iter, &key, &value))
			rule->_priv->sensor =  gnet_inetaddr_clone (value);//FIXME: wrrrronggggg
	}
}
...
sim_rule_print (SimRule      *rule)
{
  ...
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "plugin_sid=%d ", g_hash_table_size (rule->_priv->plugin_sids));
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "src_inets=%d ", g_list_length (rule->_priv->src_inets));
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "\nplugin_sids_not=%d ", g_hash_table_size (rule->_priv->plugin_sids_not));
  list = rule->_priv->src_inets;
  while (list)
    {
  ...
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "src_ports=%d ", g_hash_table_size (rule->_priv->src_ports));
 	g_hash_table_iter_init(&iter, rule->_priv->src_ports);
  while (g_hash_table_iter_next(&iter, &key, &value))
  {
  ...
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "dst_ports=%d ", g_hash_table_size (rule->_priv->dst_ports));
	g_hash_table_iter_init(&iter, rule->_priv->dst_ports);
  while (g_hash_table_iter_next(&iter, &key, &value))
  {
    gint port = GPOINTER_TO_INT (key);
  ...
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sensors=%d ", g_hash_table_size (rule->_priv->sensors));
	g_hash_table_iter_init(&iter, rule->_priv->sensors);
  while (g_hash_table_iter_next(&iter, &key, &value))
  {
	...
	g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "protocols=%d ", g_hash_table_size (rule->_priv->protocols));
	g_hash_table_iter_init(&iter, rule->_priv->protocols);
  while (g_hash_table_iter_next(&iter, &key, &value))
    g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, " %d ", GPOINTER_TO_INT (key));
 ...
 g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "src_port=%d ", rule->_priv->src_port);
  ...
  while (list)
	...
	g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "src_ports_not=%d ", g_hash_table_size (rule->_priv->src_ports_not));
	g_hash_table_iter_init(&iter, rule->_priv->src_ports_not);
  while (g_hash_table_iter_next(&iter, &key, &value))
    g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, " %d ", GPOINTER_TO_INT (key));

	g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "dst_ports_not=%d ", g_hash_table_size (rule->_priv->dst_ports_not));
	g_hash_table_iter_init(&iter, rule->_priv->dst_ports_not);
  while (g_hash_table_iter_next(&iter, &key, &value))
    g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, " %d ", GPOINTER_TO_INT (key));

	g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "protocols_not=%d ", g_hash_table_size (rule->_priv->protocols_not));
	g_hash_table_iter_init(&iter, rule->_priv->protocols_not);
  while (g_hash_table_iter_next(&iter, &key, &value))
    g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, " %d ", GPOINTER_TO_INT (key));

  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "plugin_sids_not=%d ", g_hash_table_size (rule->_priv->plugin_sids_not));
  g_hash_table_iter_init(&iter, rule->_priv->plugin_sids_not);
  while (g_hash_table_iter_next(&iter, &key, &value))
    g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, " %d ", GPOINTER_TO_INT (key));

	g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sensors_not=%d ", g_hash_table_size (rule->_priv->sensors_not));
  g_hash_table_iter_init(&iter, rule->_priv->sensors_not);
  while (g_hash_table_iter_next(&iter, &key, &value))
  {
    SimSensor *sensor = (SimSensor *) value;
 ...
 *
 */
void sim_rule_set_src_home_net (SimRule *rule, gboolean yes_no) 
{ 
  rule->_priv->src_home_net = yes_no; 
} 
 
/*
 *
 */
void sim_rule_set_src_home_net_not (SimRule *rule, gboolean yes_no) 
{ 
  rule->_priv->src_home_net_not = yes_no; 
}

/*
 *
 */
void sim_rule_set_dst_home_net (SimRule *rule, gboolean yes_no) 
{ 
  rule->_priv->dst_home_net = yes_no; 
}

/*
 *
 */
void sim_rule_set_dst_home_net_not (SimRule *rule, gboolean yes_no) 
{ 
  rule->_priv->dst_home_net_not = yes_no; 
}

gboolean sim_rule_match_src_ip (SimRule *rule, SimEvent *event)
{
  gboolean  match = TRUE;
  GList     *list;
  ...
  // src_ip in HOME_NET
  if (rule->_priv->src_home_net)
  {
    if(!event->src_home_net_searched)
    {
      event->src_home_net_searched = TRUE;

    	SimInet *inet = sim_inet_new_from_ginetaddr (event->src_ia);

      list = sim_container_get_home_net(ossim.container);
  		while (list) 
	  	{ 
			  SimInet *cmp_inet = (SimInet *) list->data; 
    ...
    return event->src_home_net;
  }

 
  // src_ip not in HOME_NET
  if (rule->_priv->src_home_net_not)
  {
    if(!event->src_home_net_searched)
    {
      event->src_home_net_searched = TRUE;
...
gboolean sim_rule_match_dst_ip (SimRule *rule, SimEvent *event)
{
  gboolean  match = TRUE;
  GList     *list;
  ...
  {
    SimInet *inet = sim_inet_new_from_ginetaddr (event->dst_ia); //take the event dst ip nd check if it belongs to the rule	
    list = rule->_priv->dst_inets;
    while (list)
		{
		  SimInet *cmp_inet = (SimInet *) list->data; //each rule can handle multiple dst's
  ...
  // dst_ip in HOME_NET
  if (rule->_priv->dst_home_net)
  {
    if(!event->dst_home_net_searched)
    {
      event->dst_home_net_searched = TRUE;
    ...
    return event->dst_home_net;
  }

 
  // dst_ip not in HOME_NET
  if (rule->_priv->dst_home_net_not)
  {
    if(!event->dst_home_net_searched)
    {
      event->dst_home_net_searched = TRUE;
