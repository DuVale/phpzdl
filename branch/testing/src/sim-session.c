...
sim_session_impl_finalize(GObject *gobject)
{
  ...
  else if (sim_session_is_web(session))
    g_message("Session Web: REMOVED");
  else if (sim_session_is_master_server(session))
    g_message("Session Master server: REMOVED");
  else if (sim_session_is_children_server(session))
  ...
  if (!object_type)
    {
      static const GTypeInfo type_info =
        { sizeof(SimSessionClass), NULL, NULL,
            (GClassInitFunc) sim_session_class_init, NULL, NULL, /* class data */
            sizeof(SimSession), 0, /* number of pre-allocs */
            (GInstanceInitFunc) sim_session_instance_init, NULL /* value table */
        };

      g_type_init();

      object_type = g_type_register_static(G_TYPE_OBJECT, "SimSession",
          &type_info, 0);
    }
 ...
 */
SimSession*
sim_session_new(GObject *object, SimConfig *config, GTcpSocket *socket)
{
  ...
  session->_priv->reputation = sim_server_get_reputation (session->_priv->server);
      ...
      if (aux)
        {
          ...
          g_log(
              G_LOG_DOMAIN,
              G_LOG_LEVEL_DEBUG,
              "sim_session_new Remote address is loopback, applying new address: %s ",
              ip_temp);
      ...
      else
        {
          g_log(
              G_LOG_DOMAIN,
              G_LOG_LEVEL_DEBUG,
              "sim_session_new: Warning: we will maintain the 127.0.0.1 address. Please check your /etc/hosts file to include the real IP");
        }
    }

  session->_priv->io = gnet_tcp_socket_get_io_channel(session->_priv->socket);
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
      "sim_session_new session->_priv->io: %x", session->_priv->io);
  ...
  if (!session->_priv->io) //FIXME: Why does this happens?
    {
      ...
      g_log(
          G_LOG_DOMAIN,
          G_LOG_LEVEL_DEBUG,
          "sim_session_new Error: channel with IP %s has been closed (NULL value)",
          ip_temp);
      g_free(ip_temp);
 ...
 */
void
sim_session_set_hostname(SimSession *session, gchar *hostname)
{
 ...
 */
static void
sim_session_cmd_connect(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  ...
  if (command->data.connect.hostname && strcmp(command->data.connect.hostname,
      "None"))
    {
  ...
  if (command->data.connect.type == SIM_SESSION_TYPE_SENSOR)
    if (command->data.connect.tzone != 0) //update tzone for each sensor
      {
        gchar *query = g_strdup_printf("UPDATE sensor SET tzone=\'%4.2f\' WHERE name=\'%s\'", command->data.connect.tzone, sim_sensor_get_name(sensor));
        g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_session_cmd_connect: Setting tzone: %s", query);
        sim_database_execute_no_query(ossim.dbossim, query);
        g_free(query);
      }
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
      "sim_session_cmd_connect: hostname: %s",
      sim_session_get_hostname(session));
#if 0
  if (session->type == SIM_SESSION_TYPE_SENSOR)
    {
      if (command->data.connect.version == NULL)
        {
          g_message(
              "Sensor agent doesn't send version info. Closing connection");
      ...
      if (!sim_sensor_set_agent_version(sensor, command->data.connect.version))
        {
          ...
          g_message(
              "Bad version '%s' from agent at ip '%s'. Closing connection",
              command->data.connect.version, c != NULL ? c
              : "Unknown ip address");
          if (c)
          g_free(c);
          sim_session_close(session);
      ...
      if (command->data.connect.version != NULL)
        {
          ...
          gchar *ip = gnet_inetaddr_get_canonical_name(session->_priv->ia);
          gchar
              *query = g_strdup_printf("INSERT INTO sensor_properties (ip,version) VALUES (\"%s\",\"%s\") ON DUPLICATE KEY UPDATE version = \"%s\"", ip, command->data.connect.version, command->data.connect.version);
 ...
 */
static void
sim_session_cmd_session_append_plugin(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  ...
  plugin = sim_container_get_plugin_by_id(ossim.container,
  ...
  if (plugin)
    {
      ...
      session->_priv->plugin_states = g_list_append(
          session->_priv->plugin_states, plugin_state);
      /*
       cmd = sim_command_new_from_type (SIM_COMMAND_TYPE_OK);
       cmd->id = command->id;
          ...
          while (directives)
            {
              ...
              if (sim_rule_get_plugin_id(rule)
                  == command->data.session_append_plugin.id)
                {
 ...
 */
static void
sim_session_cmd_session_remove_plugin(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  SimPlugin *plugin = NULL;
  ...
  plugin = sim_container_get_plugin_by_id(ossim.container,
      command->data.session_remove_plugin.id);
  if (plugin)
    {
 ...
 */
static void
sim_session_cmd_server_get_sensors(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  ...
  if (sim_session_is_master_server(session) || // a little identity check
    ...
    {
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_server_get_sensors Inside");
      ...
      if ((!command->data.server_get_sensors.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.server_get_sensors.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_server_get_sensors: %s, %s", sim_server_get_name(
              server), command->data.server_get_sensors.servername);
      ...
      while (list) //list of the sessions connected to the server
        {
          ...
          if (for_this_server) //execute the command in this server
            {
              g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                  "sim_session_cmd_server_get_sensors Inside 2");
              if (sim_session_is_sensor(sess))
                {
                  ...
                  cmd->data.sensor.host = gnet_inetaddr_get_canonical_name(
                      sess->_priv->ia);
                  cmd->data.sensor.state = TRUE; //FIXME: check this and why is it used. Not filled in sim_command_sensor_scan() by now.
                  cmd->data.sensor.servername = g_strdup(sim_server_get_name(
                      server));
            ...
            {
              if (sim_session_is_children_server(sess) && !g_ascii_strcasecmp(
                  command->data.server_get_sensors.servername,
                  sim_session_get_hostname(sess)))
                {
 ...
 */
static void
sim_session_cmd_sensor(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  gboolean for_this_server;
  ...
  if (sim_session_is_children_server(session))
    {
      ...
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_session_cmd_sensor: %s, %s",
          sim_server_get_name(server), command->data.sensor.servername);
      ...
      while (list) //list of the sessions connected to the server
        {
          ...
          if (sim_session_is_master_server(sess) || sim_session_is_web(sess))
            {
 ...
 */
static void
sim_session_cmd_server_get_servers(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  gboolean for_this_server;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_server_get_servers Inside");
      SimServer *server = session->_priv->server;
      ...
      if ((!command->data.server_get_servers.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.server_get_servers.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_server_get_servers: %s, %s", sim_server_get_name(
              server), command->data.server_get_servers.servername);
      ...
      while (list) //list of the sessions connected to the server
        {
          ...
          if (for_this_server) //execute the command in this server
            {
              g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                  "sim_session_cmd_server_get_servers Inside 2");
              if (sim_session_is_children_server(sess))
                {
                  ...
                  cmd->data.server.host = gnet_inetaddr_get_canonical_name(
                      sess->_priv->ia);
                  cmd->data.server.servername = g_strdup(
                      sim_session_get_hostname(sess));
 ...
 */
static void
sim_session_cmd_server(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  gboolean for_this_server;
  ...
  if (sim_session_is_children_server(session))
    {
      ...
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_session_cmd_server: %s, %s",
          sim_server_get_name(server), command->data.server.servername);
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      if ((!command->data.server_get_sensor_plugins.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.server_get_sensor_plugins.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_server_get_sensor_plugins: %s, %s",
      ...
      while (list) //list of the sessions connected to the server
        {
          ...
          g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
              "sim_session_cmd_server_get_sensor_plugins: Session : %x", sess);
              ...
              if (sim_session_is_sensor(sess))
                {
                  ...
                  while (plugin_states)
                    {
                      SimPluginState *plugin_state =
                          (SimPluginState *) plugin_states->data;
                      SimPlugin *plugin = sim_plugin_state_get_plugin(
                          plugin_state);

                      cmd = sim_command_new_from_type(
                          SIM_COMMAND_TYPE_SENSOR_PLUGIN);
                      cmd->data.sensor_plugin.plugin_id = sim_plugin_get_id(
                          plugin);
                      cmd->data.sensor_plugin.sensor
                          = gnet_inetaddr_get_canonical_name(sess->_priv->ia); //if this is not defined
                      cmd->data.sensor_plugin.state
                          = sim_plugin_state_get_state(plugin_state);
                      cmd->data.sensor_plugin.enabled
 ...
 */
static void
sim_session_cmd_server_set_data_role(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  GList *sessions;
  ...
  if (sim_session_is_master_server(session)) //check if the remote server has rights to send data to this server
    {
      ...
      g_log(
          G_LOG_DOMAIN,
          G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_server_set_data_role: servername: %s; set to server: %s",
          sim_server_get_name(server),
      ...
      if (!g_ascii_strcasecmp(sim_server_get_name(server),
          command->data.server_set_data_role.servername))
        {
              ...
              if (is_server)
                {
                  ...
                  if (!g_ascii_strcasecmp(hostname,
                      command->data.server_set_data_role.servername))
                    {
                      ...
                      cmd->data.server_set_data_role.servername = g_strdup(
                          command->data.server_set_data_role.servername);
  ...
  else
    {
      ...
      g_message(
          "Error: Warning, %s is trying to send server role without rights!",
          ip_temp);
      g_free(ip_temp);
 ...
 */
static void
sim_session_cmd_sensor_plugin_start(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  GInetAddr *ia;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      if ((!command->data.sensor_plugin_start.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.sensor_plugin_start.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_sensor_plugin_start: %s, %s", sim_server_get_name(
              server), command->data.sensor_plugin_start.servername);

      ia = gnet_inetaddr_new_nonblock(command->data.sensor_plugin_start.sensor,
          0); //FIXME: Remember to check this as soon as event arrive!!
...
static void
sim_session_cmd_sensor_plugin_stop(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  GInetAddr *ia;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_server_get_sensors: %s, %s", sim_server_get_name(
              server), command->data.sensor_plugin_stop.servername);
      ...
      if ((!command->data.sensor_plugin_stop.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.sensor_plugin_stop.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      ia = gnet_inetaddr_new_nonblock(command->data.sensor_plugin_stop.sensor,
          0);
      list = sim_server_get_sessions(server);
      while (list) //list of the sessions connected to the server
        {
 ...
 */
static void
sim_session_cmd_sensor_plugin_enable(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  GInetAddr *ia;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      SimServer *server = session->_priv->server;
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_sensor_plugin_enable: %s, %s", sim_server_get_name(
              server), command->data.sensor_plugin_enable.servername);
      ...
      if ((!command->data.sensor_plugin_enable.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.sensor_plugin_enable.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      ia = gnet_inetaddr_new_nonblock(
          command->data.sensor_plugin_enable.sensor, 0);
      list = sim_server_get_sessions(server);
      while (list) //list of the sessions connected to the server
        {
          SimSession *sess = (SimSession *) list->data;
...
static void
sim_session_cmd_sensor_plugin_disable(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  GInetAddr *ia;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      SimServer *server = session->_priv->server;
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_sensor_plugin_disable: %s, %s", sim_server_get_name(
              server), command->data.sensor_plugin_disable.servername);
      ...
      if ((!command->data.sensor_plugin_disable.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.sensor_plugin_disable.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      ia = gnet_inetaddr_new_nonblock(
          command->data.sensor_plugin_disable.sensor, 0);
      list = sim_server_get_sessions(server);
      while (list) //list of the sessions connected to the server
        {
          SimSession *sess = (SimSession *) list->data;
 ...
 */
static void
sim_session_cmd_plugin_state_started(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *sessions;
  GList *list;
...
static void
sim_session_cmd_plugin_state_unknown(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *sessions;
  GList *list;
 ...
 */
static void
sim_session_cmd_plugin_state_stopped(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *sessions;
  GList *list;
 ...
 */
static void
sim_session_cmd_plugin_enabled(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *sessions;
  GList *list;
 ...
 */
static void
sim_session_cmd_plugin_disabled(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *sessions;
  GList *list;
 ...
 */
static void
sim_session_cmd_event(SimSession *session, SimCommand *command)
{
  ...
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_session_read: init..");
  ...
  if (session->_priv->reputation)
    sim_reputation_match_event (session->_priv->reputation, event);
 ...
 */
static void
sim_session_cmd_reload_plugins(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  gboolean for_this_server;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      if ((!command->data.reload_plugins.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.reload_plugins.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_reload_plugins: %s, %s",
          sim_server_get_name(server), command->data.reload_plugins.servername);
 ...
 */
static void
sim_session_cmd_reload_sensors(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  gboolean for_this_server;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      if ((!command->data.reload_sensors.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.reload_sensors.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_reload_sensors: %s, %s",
 ...
 */
static void
sim_session_cmd_reload_hosts(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  gboolean for_this_server;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      if ((!command->data.reload_hosts.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.reload_hosts.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_reload_hosts: %s, %s", sim_server_get_name(server),
          command->data.reload_hosts.servername);
 ...
 */
static void
sim_session_cmd_reload_nets(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  gboolean for_this_server;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      if ((!command->data.reload_nets.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.reload_nets.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_reload_nets: %s, %s", sim_server_get_name(server),
          command->data.reload_nets.servername);
 ...
 */
static void
sim_session_cmd_reload_policies(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  gboolean for_this_server;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      if ((!command->data.reload_policies.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.reload_policies.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_reload_policies: %s, %s",
          sim_server_get_name(server), command->data.reload_policies.servername);
 ...
 */
static void
sim_session_cmd_reload_directives(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  GList *list;
  gboolean for_this_server;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      if ((!command->data.reload_directives.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.reload_directives.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_reload_directives: %s, %s", sim_server_get_name(
              server), command->data.reload_directives.servername);
      ...
      if (for_this_server) //execute the command in this server
        {
          ...
          sim_container_load_directives_from_file(ossim.container,
              ossim.dbossim, SIM_XML_DIRECTIVE_FILE);
        }
      else //resend the command buffer to the children servers whose name match.
        {
          list = sim_server_get_sessions(server);
 ...
 */
static void
sim_session_cmd_reload_all(SimSession *session, SimCommand *command)
{
  SimCommand *cmd;
  SimConfig *config;
  GList *list;
  ...
  if (sim_session_is_master_server(session) || sim_session_is_web(session))
    {
      ...
      if ((!command->data.reload_all.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          ...
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(server),
              command->data.reload_all.servername)))
        for_this_server = TRUE;
      else
        for_this_server = FALSE;

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_reload_all: %s, %s", sim_server_get_name(server),
          command->data.reload_all.servername);
      ...
      if (for_this_server) //execute the command in this server
        {
          ...
          if ((config->directive.filename) && (g_file_test(
            ...
            sim_container_load_directives_from_file(ossim.container,
                ossim.dbossim, config->directive.filename);
 ...
 */
void
sim_session_cmd_host_os_event(SimSession *session, SimCommand *command)
{
  ...
  if (ia = gnet_inetaddr_new_nonblock(command->data.host_os_event.host, 0))
    {
      ...
      os = sim_container_db_get_host_os_ul(ossim.container, ossim.dbossim, ia,
          sensor);
      event = sim_event_new();
      ...
      if (command->data.host_os_event.os)
        {
          ...
          command->data.host_os_event.os = g_strdup(
              os_event_split ? os_event_split[0] : NULL);
      ...
      if (!os) //the new event is inserted into db.
      ...
      else
        {
          ...
          if (!g_ascii_strcasecmp(os, command->data.host_os_event.os))
            ...
            // we insert the event, but it's in database at this moment.
      ...
      else if (command->data.host_os_event.date_str)
        {
          if (strptime(command->data.host_os_event.date_str,
              "%Y-%m-%d %H:%M:%S", &tm))
            {
      ...
      if (command->data.host_os_event.date_str)
        {
          //g_message("command os date_str:%s", command->data.host_os_event.date_str);
          event->time_str = g_strdup(command->data.host_os_event.date_str);
        }
      ...
      if ((sim_container_get_host_by_ia(ossim.container, event->src_ia) == NULL)
          && (sim_container_get_nets_has_ia(ossim.container, event->src_ia)
              == NULL))
        return;

      event->dst_ia = gnet_inetaddr_new_nonblock("0.0.0.0", 0);

      event->data = g_strdup_printf("%s --> %s", (os) ? os
          : command->data.host_os_event.os, command->data.host_os_event.os);

      //this is used to pass the event data to sim-organizer, so it can insert it into database
      event->data_storage = g_new(gchar*, 2);
      event->data_storage[0] = g_strdup(
          (command->data.host_os_event.os) ? command->data.host_os_event.os
              : "");
    ...
    g_message("Error: Data sent from agent; host OS event wrong src IP %s",
        command->data.host_os_event.host);
 ...
 */
static void
sim_session_cmd_host_mac_event(SimSession *session, SimCommand *command)
{
  SimConfig *config;
  SimEvent *event;
  GInetAddr *ia = NULL;
  ...
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
      "sim_session_cmd_host_mac_event: command->data.host_mac_event.mac: %s",
  ...
  if (ia = gnet_inetaddr_new_nonblock(command->data.host_mac_event.host, 0))
    {
      ...
      if (!mac) //if the ia-sensor pair doesn't obtains a mac in the database, inserts the new one.
        {
          event->plugin_sid = EVENT_NEW;
          g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
              "sim_session_cmd_host_mac_event: EVENT_NEW");
        }
      else if (!g_ascii_strcasecmp(mac, command->data.host_mac_event.mac)) //the mac IS the same (0 = exact match)
        {
          event->plugin_sid = EVENT_SAME;
          g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
              "sim_session_cmd_host_mac_event: EVENT_SAME");
        }
      else //the mac is different
        {
          g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
              "sim_session_cmd_host_mac_event: EVENT_CHANGE");
      ...
      else if (command->data.host_mac_event.date_str)
        if (strptime(command->data.host_mac_event.date_str,
            "%Y-%m-%d %H:%M:%S", &tm))
          event->time = mktime(&tm);
      ...
      if (command->data.host_mac_event.date_str)
        {
          g_message("command host_mac_event date_str:%s",
              command->data.host_mac_event.date_str);
          event->time_str = g_strdup(command->data.host_mac_event.date_str);
        }

      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_host_mac_event: event->time: %d", event->time);
      ...
      if (ip_temp)
      {
        ...
        {
          event->src_ia = gnet_inetaddr_new_nonblock("0.0.0.0", 0);
          g_message("Error: Data sent from agent; host MAC event wrong IP %s",
              command->data.host_mac_event.host);
        }
      ...
      event->data
          = g_strdup_printf(
              "%s|%s --> %s|%s",
              (mac) ? mac : command->data.host_mac_event.mac,
              (vendor) ? vendor : "",
              command->data.host_mac_event.mac,
      ...
      event->data_storage[0] = g_strdup(
          (command->data.host_mac_event.mac) ? command->data.host_mac_event.mac
              : "");
      event->data_storage[1]
          = g_strdup(
              (command->data.host_mac_event.vendor) ? command->data.host_mac_event.vendor
                  : "");
      ...
      event->textfields[SimTextFieldUserdata1] = g_strdup(
          command->data.host_mac_event.mac); //needed for correlation
      ...
      sim_container_push_event(ossim.container, event);
      sim_container_set_sensor_event_number(ossim.container,
          SIM_EVENT_HOST_MAC_EVENT, sensor);
  ...
  if (command->data.host_mac_event.date_str)
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
        "command->data.host_mac_event.date: %s",
        command->data.host_mac_event.date_str);
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
      "sim_session_cmd_host_mac_event: TYPE: %d", event->plugin_sid);
 ...
 */
static void
sim_session_cmd_host_service_event(SimSession *session, SimCommand *command)
{
  SimConfig *config;
  SimEvent *event;
  GInetAddr *ia = NULL;
  ...
  if (ia = gnet_inetaddr_new_nonblock(command->data.host_service_event.host, 0))
      ...
      if (!g_ascii_strcasecmp(command->data.host_service_event.service, "ARP"))
        {
          ...
          cmd->data.host_mac_event.date_str = g_strdup(
              command->data.host_service_event.date_str);
          cmd->data.host_mac_event.host = g_strdup(
              command->data.host_service_event.host);
          cmd->data.host_mac_event.mac = g_strdup(
              command->data.host_service_event.application);
          cmd->data.host_mac_event.sensor = g_strdup(
              command->data.host_service_event.sensor);
          cmd->data.host_mac_event.interface = g_strdup(
              command->data.host_service_event.interface);
          ...
          g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
              "sim_session_cmd_host_service_event date: %d",
              cmd->data.host_mac_event.date);
          g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
              "sim_session_cmd_host_service_event date_str: %s",
              cmd->data.host_mac_event.date_str);
      ...
      else //ok, this is not a MAC change event, its a service change event
        {
          ...
          application_and_service = sim_container_db_get_host_service_ul(
              ossim.container, ossim.dbossim, ia, port, protocol, sensor);
          ...
          else if (!g_ascii_strcasecmp(application,
              command->data.host_service_event.application)) //service is the same
            {
              if (!g_ascii_strcasecmp(service,
                  command->data.host_service_event.service))
                event->plugin_sid = EVENT_SAME;
              else
                event->plugin_sid = EVENT_CHANGE;
            }
          ...
          else if (command->data.host_service_event.date_str)
            if (strptime(command->data.host_service_event.date_str,
                "%Y-%m-%d %H:%M:%S", &tm))
              event->time = mktime(&tm);
          if (!event->time)
            event->time = time(NULL);
          ...
          if (ip_temp)
          {
          ...
          else
            {
              event->src_ia = gnet_inetaddr_new_nonblock("0.0.0.0", 0);
              g_message(
                  "Error: Data sent from agent; host Service event wrong IP %s",
                  command->data.host_service_event.host);
            }


          //we want to process only the hosts defined in Policy->hosts or inside a network from policy->networks
          if ((sim_container_get_host_by_ia(ossim.container, event->src_ia)
              == NULL) && (sim_container_get_nets_has_ia(ossim.container,
              event->src_ia) == NULL))
            return;

          event->dst_ia = gnet_inetaddr_new_nonblock("0.0.0.0", 0);
          event->data = g_strdup_printf("%d/%d - %s/%s", port, protocol,
              command->data.host_service_event.service,
          ...
          event->data_storage[2] = g_strdup(
              command->data.host_service_event.service);
          event->data_storage[3] = g_strdup((application) ? application
              : command->data.host_service_event.application);
          ...
          sim_container_push_event(ossim.container, event);
          sim_container_set_sensor_event_number(ossim.container,
              SIM_EVENT_HOST_SERVICE_EVENT, sensor);
    ...
    g_message("Error: Data sent from agent; host MAC or OS event wrong IP %s",
        command->data.host_service_event.host);
 ...
 */
static void
sim_session_cmd_host_ids_event(SimSession *session, SimCommand *command)
{
  SimConfig *config;
  SimEvent *event;
  GInetAddr *ia = NULL;
  ...
  if (ia = gnet_inetaddr_new_nonblock(command->data.host_ids_event.host, 0))
    {
      ...
      else if (command->data.host_ids_event.date_str)
        {
          if (strptime(command->data.host_ids_event.date_str,
              "%Y-%m-%d %H:%M:%S", &tm))
            {
              event->time = mktime(&tm);
            }
      ...
      if (command->data.host_ids_event.date_str)
        {
          g_message("command host_ids_event date_str:%s",
              command->data.host_ids_event.date_str);
          event->time_str = g_strdup(command->data.host_ids_event.date_str);
        }
      ...
      if (ip_temp)
      {
      ...
      else
        {
          ...
          g_message(
              "Error: Data sent from agent; host Service event wrong IP %s",
              command->data.host_ids_event.host);
        }
      ...
      event->data_storage[1]
          = g_strdup(command->data.host_ids_event.event_type);
      ...
      event->data_storage[4]
          = g_strdup(command->data.host_ids_event.extra_data);
      ...
      if (command->data.host_ids_event.userdata3)
        ...
        event->textfields[SimTextFieldUserdata3] = g_strdup(
            command->data.host_ids_event.userdata3);
      if (command->data.host_ids_event.userdata4)
        ...
        event->textfields[SimTextFieldUserdata4] = g_strdup(
            command->data.host_ids_event.userdata4);
      if (command->data.host_ids_event.userdata5)
        ...
        event->textfields[SimTextFieldUserdata5] = g_strdup(
            command->data.host_ids_event.userdata5);
      if (command->data.host_ids_event.userdata6)
        ...
        event->textfields[SimTextFieldUserdata6] = g_strdup(
            command->data.host_ids_event.userdata6);
      if (command->data.host_ids_event.userdata7)
        ...
        event->textfields[SimTextFieldUserdata7] = g_strdup(
            command->data.host_ids_event.userdata7);
      if (command->data.host_ids_event.userdata8)
        ...
        event->textfields[SimTextFieldUserdata8] = g_strdup(
            command->data.host_ids_event.userdata8);
      if (command->data.host_ids_event.userdata9)
        ...
        event->textfields[SimTextFieldUserdata9] = g_strdup(
            command->data.host_ids_event.userdata9);
      ...
      sim_container_push_event(ossim.container, event);
      sim_container_set_sensor_event_number(ossim.container,
          SIM_EVENT_HOST_IDS_EVENT, sensor);

    }
  else
    g_message("Error: Data sent from agent; error from host ids event, IP: %s",
        command->data.host_ids_event.host);
}
 ...
 */
void
sim_session_cmd_database_query(SimSession *session, SimCommand *command)
{
  GdaDataModel *dm;
  SimCommand *cmd;
  ...
  if (sim_session_is_children_server(session) || sim_session_is_sensor(session))
    {
      ...
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_database_query: %s, %s",
          sim_server_get_name(server), command->data.database_query.servername);
      ...
      if (for_this_server) //execute the command in this server.
        {
          ...
          cmd->data.database_answer.database_element_type
              = command->data.database_query.database_element_type;
          cmd->data.database_answer.servername = g_strdup(
          ...
          case SIM_DB_ELEMENT_TYPE_PLUGINS:
				    ...
				    table = sim_container_get_plugins (ossim.container);

            g_hash_table_iter_init (&iter, table);
          	while (g_hash_table_iter_next (&iter, &key, &value))
            {
					    SimPlugin *plugin = (SimPlugin *) value;
              ...
              sim_base64_encode(plugin_description,
                  strlen(plugin_description), base64_stored, BUFFER_SIZE,
                  &base64_len);

              //we will use this separation character: |
              //id, type, name, description
              cmd->data.database_answer.answer = g_strdup_printf(
                  "%d|%d|%s|%s", sim_plugin_get_id(plugin), plugin->type,
          ...
          case SIM_DB_ELEMENT_TYPE_PLUGIN_SIDS:
				    ...
				    table = sim_container_get_plugin_sids (ossim.container);
            g_hash_table_iter_init (&iter, table);
          	while (g_hash_table_iter_next (&iter, &key, &value))
            {
					    SimPluginSid *plugin_sid = (SimPluginSid *) value;
              gchar *plugin_sid_name = sim_plugin_sid_get_name(plugin_sid);

              sim_base64_encode(plugin_sid_name, sim_strnlen(plugin_sid_name,
                  BUFFER_SIZE), base64_stored, BUFFER_SIZE, &base64_len);

              //plugin_id, sid, reliability, priority, name.
              cmd->data.database_answer.answer = g_strdup_printf(
                  "%d|%d|%d|%d|%s", sim_plugin_sid_get_plugin_id(plugin_sid),
              ...
              if (!sim_session_write(session, cmd))
                g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                    "Error: sim_session_cmd_database_query NO Session");
              g_free(cmd->data.database_answer.answer);
              memset(base64_stored, 0, BUFFER_SIZE);
            }
            break;
            ...
            while (list)
              {
                ...
                //name, ip, port
                cmd->data.database_answer.answer = g_strdup_printf("%s|%s|%d",
                    sim_sensor_get_name(sensor), ip,
            ...
            while (list)
              {
                ...
                //name, ip, asset
                cmd->data.database_answer.answer = g_strdup_printf("%s|%s|%d",
                    sim_host_get_name(host), ip, sim_host_get_asset(host));
                g_free(ip);
                sim_session_write(session, cmd);
                g_free(cmd->data.database_answer.answer);
                list = list->next;
            ...
            while (list)
              {
                ...
                cmd->data.database_answer.answer = g_strdup_printf("%s|%s|%d",
                    sim_net_get_name(net), sim_net_get_ips(net),
            ...
            while (list)
              {
                ...
                /*
                 ...
                 !g_ascii_strcasecmp (server_name, SIM_WILDCARD_ANY))
                 {
                 found = TRUE;
                 break;
                 }
                    ...
                    while (list_targets)
                      {
                        ...
                        if (!g_ascii_strcasecmp(target_name,
                            cmd->data.database_answer.servername) || //to simplify things, we store sensor name in servername variable
                            !g_ascii_strcasecmp(target_name,
                                SIM_WILDCARD_ANY))
                          {
                            found = TRUE;
                            break;
                ...
                cmd->data.database_answer.answer = g_strdup_printf(
                    "%d|%d|%d|%d|%d|%d|%d", SIM_POLICY_ELEMENT_TYPE_GENERAL,
                    sim_policy_get_id(policy), sim_policy_get_priority(policy),
                    sim_policy_get_begin_hour(policy), sim_policy_get_end_hour(
                        policy), sim_policy_get_begin_day(policy),
                ...
                if (role)
                  {
                    cmd->data.database_answer.answer = g_strdup_printf(
                        ...
                        sim_policy_get_id(policy), role->correlate,
                        role->cross_correlate, role->store, role->qualify,
                        role->resend_event, role->resend_alarm);
                    sim_session_write(session, cmd);
                    g_free(cmd->data.database_answer.answer);
                  }
                cmd->data.database_answer.answer = NULL;
                ...
                while (src)
                  {
                    ...
                    if (src->next == NULL)
                      ...
                      s = g_strdup_printf("%s%s%s", s ? s : "", aux,
                          SIM_DELIMITER_LIST); //if there are more src's we have to separate it with a ","
                ...
                if (s)
                  {
                    cmd->data.database_answer.answer = g_strdup_printf(
                        "%d|%d|%s", SIM_POLICY_ELEMENT_TYPE_SRC,
                        sim_policy_get_id(policy), s);
                    sim_session_write(session, cmd);
                    g_free(cmd->data.database_answer.answer);
                    g_free(s);
                  }
                ...
                if (s)
                  {
                    cmd->data.database_answer.answer = g_strdup_printf(
                        "%d|%d|%s", SIM_POLICY_ELEMENT_TYPE_DST,
                        sim_policy_get_id(policy), s);
                    sim_session_write(session, cmd);
                    g_free(cmd->data.database_answer.answer);
                    g_free(s);
                ...
                while (list2)
                  {
                    ...
                    aux = g_strdup_printf("%d%s%d", pp->port,
                        SIM_DELIMITER_RANGE, pp->protocol);
                    s2 = s;
                    ...
                    if (list2->next == NULL)
                      ...
                      s = g_strdup_printf("%s%s%s", s ? s : "", aux,
                          SIM_DELIMITER_LIST);//if next not null, we'll need a ","
                ...
                if (s)
                  {
                    cmd->data.database_answer.answer = g_strdup_printf(
                        "%d|%d|%s", SIM_POLICY_ELEMENT_TYPE_PORTS,
                        sim_policy_get_id(policy), s);
                    sim_session_write(session, cmd);
                    g_free(cmd->data.database_answer.answer);
                    g_free(s);
                ...
                while (list2)
                  {
                    ...
                    if (list2->next == NULL)
                      ...
                      s = g_strdup_printf("%s%s%s", s ? s : "", sensor,
                          SIM_DELIMITER_LIST);//if next not null, we'll need a ","
                ...
                if (s)
                  {
                    cmd->data.database_answer.answer = g_strdup_printf(
                        "%d|%d|%s", SIM_POLICY_ELEMENT_TYPE_SENSORS,
                        sim_policy_get_id(policy), s);
                    sim_session_write(session, cmd);
                    g_free(cmd->data.database_answer.answer);
                    g_free(s);
                ...
                while (list2)
                  {
                    ...
                    Plugin_PluginSid *plugin_group =
                        (Plugin_PluginSid *) list2->data;

                    aux = g_strdup_printf("%d%s", plugin_group->plugin_id,
                        SIM_DELIMITER_RANGE);
                    ...
                    while (sids)
                      {
                        ...
                        s2 = s;
                        if (sids->next == NULL)
                          s = g_strdup_printf("%s%d", s ? s : "",
                              *aux_plugin_sid);
                        else
                          s = g_strdup_printf("%s%d%s", s ? s : "",
                              *aux_plugin_sid, SIM_DELIMITER_LIST);//if next not null, we'll need a ","
                    ...
                    if (s)
                      {
                        cmd->data.database_answer.answer = g_strdup_printf(
                            ...
                            sim_policy_get_id(policy), aux, //plugin_id- (ie. "1001-")
                            s); //plugin_sid's
                        sim_session_write(session, cmd);
                        g_free(cmd->data.database_answer.answer);
                        g_free(aux);
                ...
                while (list2)
                  {
                    ...
                    if (list2->next == NULL)
                      ...
                      s = g_strdup_printf("%s%s%s", s ? s : "", target_aux,
                          SIM_DELIMITER_LIST);//if next not null, we'll need a ","
                ...
                if (s)
                  {
                    cmd->data.database_answer.answer = g_strdup_printf(
                        "%d|%d|%s", SIM_POLICY_ELEMENT_TYPE_TARGETS,
                        sim_policy_get_id(policy), s);
                    sim_session_write(session, cmd);
                    g_free(cmd->data.database_answer.answer);
                    g_free(s);
            ...
            while (list)
              {
                ...
                //ip, c, a
                cmd->data.database_answer.answer = g_strdup_printf("%s|%f|%f",
                    ip, sim_host_level_get_c(host_level), sim_host_level_get_a(
                        host_level));
                g_free(ip);
                sim_session_write(session, cmd);
                g_free(cmd->data.database_answer.answer);
            ...
            while (list)
              {
                ...
                //name, c, a
                cmd->data.database_answer.answer = g_strdup_printf("%s|%f|%f",
                    sim_net_level_get_name(net_level), sim_net_level_get_c(
                        net_level), sim_net_level_get_a(net_level));
                sim_session_write(session, cmd);
                g_free(cmd->data.database_answer.answer);
                list = list->next;
              }
          ...
          case SIM_DB_ELEMENT_TYPE_SERVER_ROLE:
            ...
            // correlate, cross_correlate, store, qualify, resend_event, resend_alarm
            cmd->data.database_answer.answer = g_strdup_printf(
                "%d|%d|%d|%d|%d|%d", role->correlate, role->cross_correlate,
                role->store, role->qualify, role->resend_event,
                role->resend_alarm);
            sim_session_write(session, cmd);
            g_free(cmd->data.database_answer.answer);
            break;
          ...
          while (list2) //list of the sessions connected to the server
            {
              ...
              if (sim_session_is_master_server(sess) && !g_ascii_strcasecmp(
                  rserver->name, sim_session_get_hostname(sess)))
                {
                  sim_session_write_from_buffer(sess, command->buffer);
                  break;
                }
 ...
 */
void
sim_session_cmd_database_answer(SimSession *session, SimCommand *command)
{
  GdaDataModel *dm;
  SimCommand *cmd;
  GList *list, *list2;
  ...
  if (sim_session_is_master_server(session))
  {
      ...
      if ((!command->data.database_answer.servername) || //FIXME: If the server name isn't specified, for backwards compatibility
          //we will assume that this is the dst server. This should be removed in
          //a near future. All the commands from frameworkd should issue a server name.
          (!g_ascii_strcasecmp(sim_server_get_name(ossim.server),
      ...
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_cmd_database_answer: %s, %s", sim_server_get_name(
              ossim.server), command->data.database_answer.servername);
          ...
          if (for_this_server) //execute the command in this server
            {
              ...
              switch (command->data.database_answer.database_element_type)
                {
              g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                  "sim_session_cmd_database_answer: string: %s",
                  sim_command_get_string(command));
              //I don't want to use the GScanner here. As this will be a lot of info to send, I think may be better to use
              //static fields in static places.
            case SIM_DB_ELEMENT_TYPE_PLUGINS:
              plugin = sim_plugin_new();
              values = g_strsplit(command->data.database_answer.answer,
                  SIM_DELIMITER_PIPE, 0);
              ...
              //id, type, name, description
              //
              sim_base64_decode(values[2], sim_strnlen(values[2], BUFFER_SIZE),
                  base64_stored, &base64_len); //the data comes here in base64, so we decode it
              ...
              sim_container_add_plugin (ossim.container, plugin);
            ...
            case SIM_DB_ELEMENT_TYPE_PLUGIN_SIDS:
              ...
              sim_base64_decode(values[4], sim_strnlen(values[4], BUFFER_SIZE),
                  base64_stored, &base64_len);
              ...
              sim_plugin_sid_set_name(plugin_sid, g_strdup(base64_stored));
					    ...
					    sim_container_add_plugin_sid (ossim.container, plugin_sid);
              g_strfreev(values);
              memset(base64_stored, 0, BUFFER_SIZE);
              break;
            ...
            case SIM_DB_ELEMENT_TYPE_POLICIES:
              values = g_strsplit(command->data.database_answer.answer,
                  SIM_DELIMITER_PIPE, 0);
              SimPolicy *pol = NULL;
              if ((!sim_string_is_number(values[0], FALSE))
                  || (!sim_string_is_number(values[1], FALSE)))
              ...
              g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                  "sim_session_cmd_database_answer list2: %x", list2);
              while (list2) //find the policy that matches with id.
                {
                  pol = (SimPolicy *) list2->data;
                  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                      "sim_session_cmd_database_answer pol 1: %x", pol);
              ...
              switch (atoi(values[0]))
                //In all the policy msgs, the first field is the kind of message.
                {
              case SIM_POLICY_ELEMENT_TYPE_GENERAL:
                ...
                g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                    "sim_session_cmd_database_answer pol 2: %x", policy);
                break;
              case SIM_POLICY_ELEMENT_TYPE_ROLE:
                ...
                g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                    "sim_session_cmd_database_answer pol 3: %x", pol);
                break;
                ...
                for (i = 0; values2[i] != NULL; i++)
                  {
                    ...
                    SimPortProtocol *pp = sim_port_protocol_new(
                        atoi(values3[0]), atoi(values3[1]));
            ...
            case SIM_DB_ELEMENT_TYPE_HOST_LEVELS:
              ...
              //ip, c, a
              host_level = sim_host_level_new(ia, g_ascii_strtod(values[1],
                  NULL), g_ascii_strtod(values[2], NULL));
            ...
            case SIM_DB_ELEMENT_TYPE_NET_LEVELS:
              ...
              //name, c, a
              net_level = sim_net_level_new(values[0], g_ascii_strtod(
                  values[1], NULL), g_ascii_strtod(values[2], NULL));
 ...
 */
static void
sim_session_cmd_ok(SimSession *session, SimCommand *command)
{
g_return_if_fail(session != NULL);
g_return_if_fail(SIM_IS_SESSION (session));
g_return_if_fail(command != NULL);
 ...
 */
static void
sim_session_cmd_error(SimSession *session, SimCommand *command)
{
g_return_if_fail(session != NULL);
g_return_if_fail(SIM_IS_SESSION (session));
g_return_if_fail(command != NULL);
 ...
 */
static void sim_session_cmd_reload_post_correlation_sids (SimSession *session, SimCommand *command)
{
  g_return_if_fail(session != NULL);
  g_return_if_fail(SIM_IS_SESSION (session));
  g_return_if_fail(command != NULL);
  ...
  {
    values = g_strsplit (command->data.reload_post_correlation_sids.sids, SIM_DELIMITER_ELEMENT_LIST, 0); //separate each of the individual plugin_sid chunk (id:name:priority:reliability) delimited with ";"
    for (i = 0; values[i] != NULL; i++)
    {
      ...
      g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_session_cmd_reload_post_correlation_sids: %s", query);
      sim_database_execute_no_query (ossim.dbossim, query);
      sim_container_add_plugin_sid (ossim.container, plugin_sid);
...
sim_session_read(SimSession *session)
{
  ...
  while ((!session->_priv->close) && (error = gnet_io_channel_readline(session->_priv->io, buffer, BUFFER_SIZE, &n)) == G_IO_ERROR_NONE && (n > 0))
  {
    ...
    if (sim_session_get_is_initial(session)) //is this the session started in sim_container_new();?
    {
      ...
      else if (cmd->type == SIM_COMMAND_TYPE_OK) //
      {
        ...
        if (session->_priv->fully_stablished == FALSE
        ) //we only need to do the mutex the first time, when we are not sure that the
    ...
    case SIM_COMMAND_TYPE_EVENT: //from sensor / server children
      ...
      if ((!config->server.role->correlate) && (!config->server.role->cross_correlate) && (!config->server.role->store) && (!config->server.role->qualify) && (!config->server.role->resend_alarm))
      {
 ...
 */
gint
sim_session_write(SimSession *session, SimCommand *command)
{
GIOError error;
gchar *str;
gsize n;
 ...
 */
guint
sim_session_write_from_buffer(SimSession *session, gchar *buffer)
{
GIOError error;
 ...
 */
gboolean
sim_session_has_plugin_id(SimSession *session, gint plugin_id)
{
GList *list;
gboolean found = FALSE;
...
while (list)
  {
    ...
    SimPlugin *plugin = sim_container_get_plugin_by_id(ossim.container,
        plugin_id);
...
g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
    "sim_session_close: closing session: %x", session);
 ...
 */
void
sim_session_set_is_initial(SimSession *session, gboolean tf)
{
  g_return_if_fail(session);
  g_return_if_fail(SIM_IS_SESSION (session));
...
void
sim_session_set_sensor(SimSession *session, SimSensor *sensor)
{
g_return_if_fail(session != NULL);
...
void
sim_session_set_event_scan_fn(SimSession *session, gboolean
...
gboolean (*
  sim_session_get_event_scan_fn(SimSession *session))(SimCommand *,GScanner*)
  {
    g_return_if_fail(session != NULL);
    g_return_if_fail(SIM_IS_SESSION (session));
...
sim_session_config_agent(SimSession *session, SimCommand *command)
{
  ...
  session->_priv->agent_scan_fn = sim_command_get_agent_scan(command); //wich parser whould we use? Assign it to the function pointer
}
