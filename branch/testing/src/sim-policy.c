									...
									guint							plugin_sid)
{
  ...
  g_return_val_if_fail (sensor, FALSE);
	...
	while (list)	
	{
		...
		if (!g_ascii_strcasecmp (target, server_aux) ||
				!g_ascii_strcasecmp (target, SIM_WILDCARD_ANY))
		{
			found = TRUE;
			break;
		}
  ...
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_policy_match: BeginDayHour: %d, EndDayHour:%d DATE:%d",policy->_priv->begin_dayhour,policy->_priv->end_dayhour);
