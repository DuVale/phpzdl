																				   ...
																				   xmlNodePtr        node)
{
  ...
  value = (char*)xmlGetProp(node,(xmlChar*) PROPERTY_ID);
  id = atoi(value);
  xmlFree(value);
  value = (char *) xmlGetProp(node,(xmlChar*) PROPERTY_NAME);
  name = g_strdup_printf("directive_event: %s",value);
  xmlFree(value);
  ...
  for (i = 0; values[i] != NULL; i++)
  {
    ...
    if (strstr (token_value, SIM_DELIMITER_LEVEL))            //if this token doesn't refers to the 1st rule level...
      ...
      if (field_neg)
		...
		if (!strcasecmp (token_value, SIM_WILDCARD_ANY)) 
		{
  ...
  for (i = 0; values[i] != NULL; i++)
  {
    ...
    if (strstr (token_value, SIM_DELIMITER_LEVEL))								//if this isn't the first level...
	  ...
	  if (!strcmp (token_value, SIM_HOME_NET_CONST))						//usually, "HOME_NET"
		{
      if(addr_neg)
        sim_rule_set_src_home_net_not(rule, TRUE);
      else 
        sim_rule_set_src_home_net(rule, TRUE);
      ...
      */
      ...
      while (nets)
      {
          ...
          SimInet *inet = (SimInet *) inets->data;
	          ...
	          sim_rule_prepend_src_inet_not (rule, inet);
					else
						sim_rule_prepend_src_inet (rule, inet);
          inets = inets->next;
        }
        g_list_free(inets);
        nets = nets->next;
  ...
  for (i = 0; values[i] != NULL; i++)
  {
    ...
    if (strstr (token_value, SIM_DELIMITER_LEVEL))
  	...
  	if (!strcmp (token_value, SIM_HOME_NET_CONST))
		{
      if(addr_neg)
        sim_rule_set_dst_home_net_not(rule, TRUE);
      else
        sim_rule_set_dst_home_net(rule, TRUE);
      ...
      */
      ...
      while(nets)
      {
        ...
        while(inets)
          ...
          SimInet *inet = (SimInet *) inets->data;
          if (addr_neg)
            sim_rule_prepend_dst_inet_not (rule, inet);
          else
            sim_rule_prepend_dst_inet (rule, inet);
          inets = inets->next;
        }
        g_list_free(inets);
        nets = nets->next;
