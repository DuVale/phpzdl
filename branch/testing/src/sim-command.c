...
static gboolean sim_command_reload_post_correlation_sids (SimCommand *command, GScanner *scanner);
static gboolean
...
static gboolean
sim_command_error_scan(SimCommand *command, GScanner *scanner);
static gboolean
...
static const struct
{
...
} agent_parsers_table[] =
  {
    ...
    { "2.3", &sim_command_event_scan_base64 },
    { "3", &sim_command_event_scan_base64 },
    { NULL, NULL } };
...
sim_command_impl_finalize(GObject *gobject)
{
  ...
  switch (cmd->type)
  {
    case SIM_COMMAND_TYPE_CONNECT:
      ...
      if (cmd->data.connect.version)
        g_free(cmd->data.connect.version);
      break;
    case SIM_COMMAND_TYPE_SERVER:
      if (cmd->data.server.servername)
        g_free(cmd->data.server.servername);
      if (cmd->data.server.host)
        g_free(cmd->data.server.host);
      break;
    ...
    case SIM_COMMAND_TYPE_SERVER_GET_SERVERS:
      if (cmd->data.server_get_servers.servername)
        g_free(cmd->data.server_get_servers.servername);
      break;
    ...
    case SIM_COMMAND_TYPE_RELOAD_PLUGINS: //
      if (cmd->data.reload_plugins.servername)
        g_free(cmd->data.reload_plugins.servername);
      break;
    case SIM_COMMAND_TYPE_RELOAD_SENSORS: //
      if (cmd->data.reload_sensors.servername)
        g_free(cmd->data.reload_sensors.servername);
      break;
    case SIM_COMMAND_TYPE_RELOAD_HOSTS: //
      if (cmd->data.reload_hosts.servername)
        g_free(cmd->data.reload_hosts.servername);
      break;
    case SIM_COMMAND_TYPE_RELOAD_NETS: //
      if (cmd->data.reload_nets.servername)
        g_free(cmd->data.reload_nets.servername);
      break;
    case SIM_COMMAND_TYPE_RELOAD_POLICIES: //
      if (cmd->data.reload_policies.servername)
        g_free(cmd->data.reload_policies.servername);
      break;
    case SIM_COMMAND_TYPE_RELOAD_DIRECTIVES: //
      if (cmd->data.reload_directives.servername)
        g_free(cmd->data.reload_directives.servername);
      break;
    case SIM_COMMAND_TYPE_RELOAD_ALL: //
      if (cmd->data.reload_all.servername)
        g_free(cmd->data.reload_all.servername);
      break;
    ...
    case SIM_COMMAND_TYPE_SESSION_REMOVE_PLUGIN: //
      ...
      break;
    case SIM_COMMAND_TYPE_PLUGIN_STATE_STARTED: //
      break;
    case SIM_COMMAND_TYPE_PLUGIN_STATE_UNKNOWN: //
      break;
    case SIM_COMMAND_TYPE_PLUGIN_STATE_STOPPED: //
      break;
    ...
    case SIM_COMMAND_TYPE_HOST_OS_EVENT: //
      ...
      if (cmd->data.host_os_event.date_str)
    ...
    case SIM_COMMAND_TYPE_HOST_MAC_EVENT: //
      ...
      if (cmd->data.host_mac_event.date_str)
        g_free(cmd->data.host_mac_event.date_str);

      if (cmd->data.host_mac_event.log)
        g_free(cmd->data.host_mac_event.log);
      break;
    ...
    case SIM_COMMAND_TYPE_HOST_SERVICE_EVENT: //
      ...
      if (cmd->data.host_service_event.date_str)
    ...
    case SIM_COMMAND_TYPE_HOST_IDS_EVENT: //
      if (cmd->data.host_ids_event.date_str)
      ...
      if (cmd->data.host_ids_event.interface)
        g_free(cmd->data.host_ids_event.interface);
      if (cmd->data.host_ids_event.log)
        g_free(cmd->data.host_ids_event.log);

      g_free (cmd->data.host_ids_event.rep_act_src);
      g_free (cmd->data.host_ids_event.rep_act_dst);
    ...
    case SIM_COMMAND_TYPE_DATABASE_QUERY: //
      if (cmd->data.database_query.servername)
        g_free(cmd->data.database_query.servername);
      break;
    case SIM_COMMAND_TYPE_DATABASE_ANSWER: //
      if (cmd->data.database_answer.servername)
        g_free(cmd->data.database_answer.servername);
      if (cmd->data.database_answer.answer)
...
sim_command_instance_init(SimCommand *command)
{
  ...
  command->data.server_get_sensors.servername = NULL;
  command->data.server_get_servers.servername = NULL;
  command->data.server_get_sensor_plugins.servername = NULL;
  command->data.server_set_data_role.servername = NULL;
  command->data.sensor.host = NULL;
  command->data.sensor.servername = NULL;
  command->data.sensor_plugin.servername = NULL;
  command->data.sensor_plugin.sensor = NULL;
  command->data.sensor_plugin_start.servername = NULL;
  command->data.sensor_plugin_start.sensor = NULL;
  command->data.sensor_plugin_stop.servername = NULL;
  command->data.sensor_plugin_stop.sensor = NULL;
  command->data.sensor_plugin_enable.servername = NULL;
  command->data.sensor_plugin_enable.sensor = NULL;
  command->data.sensor_plugin_disable.servername = NULL;
  command->data.sensor_plugin_disable.sensor = NULL;
  command->data.reload_plugins.servername = NULL;
  command->data.reload_sensors.servername = NULL;
  command->data.reload_hosts.servername = NULL;
  command->data.reload_nets.servername = NULL;
  command->data.reload_policies.servername = NULL;
  command->data.reload_directives.servername = NULL;
  command->data.reload_all.servername = NULL;
  ...
  command->data.watch_rule.str = NULL;
  ...
  command->data.host_ids_event.interface = NULL;
  ...
  command->data.database_query.servername = NULL;
  command->data.database_answer.servername = NULL;
  command->data.database_answer.answer = NULL;
  command->snort_event.gzipdata = NULL;
...
sim_command_start_scanner()
{
  ...
  /* Added post-correlation symbols */
...
sim_command_scan(SimCommand *command, const gchar *buffer, SimSession *session)
{
  ...
  do
  {
    ...
    case SIM_COMMAND_SYMBOL_RELOAD_POST_CORRELATION_SIDS:
      if (!sim_command_reload_post_correlation_sids (command, scanner))
        OK=FALSE;
      break;
    ...
    case SIM_COMMAND_SYMBOL_ERROR:
      if (!sim_command_error_scan(command, scanner))
        OK = FALSE;
      break;
  ...
  do
    {
      ...
      case SIM_COMMAND_SYMBOL_LOG:
        ...
        command->data.event.log = g_base64_decode(scanner->value.v_string,
            &base64len);
				sim_util_substite_problematic_chars ( command->data.event.log);
        break;
      ...
      case SIM_COMMAND_SYMBOL_ALARM:
		...
		case SIM_COMMAND_SYMBOL_REP_ACT_SRC:
			...
			command->data.event.rep_act_src = g_base64_decode (scanner->value.v_string,&base64len);
			break;
		...
		case SIM_COMMAND_SYMBOL_REP_ACT_DST:
			...
			command->data.event.rep_act_dst = g_base64_decode (scanner->value.v_string,&base64len);
			break;
  ...
  do
    {
      ...
      case SIM_COMMAND_SYMBOL_ALARM:
		...
		case SIM_COMMAND_SYMBOL_REP_ACT_SRC:
			...
			command->data.event.rep_act_src = g_strdup (scanner->value.v_string);
			break;
		...
		case SIM_COMMAND_SYMBOL_REP_ACT_DST:
			...
			command->data.event.rep_act_dst = g_strdup (scanner->value.v_string);
			break;
  ...
  do
  {
    ...
    switch (scanner->token)
    {
      ...
      default:
        if (scanner->token == G_TOKEN_EOF)
          break;
        g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_command_reload_post_correlation_sids: error symbol unknown.");
      return FALSE;
    }
  }
  while(scanner->token != G_TOKEN_EOF);
  ...
  do
    {
      ...
      default:
        ...
        g_scanner_get_next_token(scanner); /* = */
        g_scanner_get_next_token(scanner); /* value */
        g_message("sim_command_host_mac_event_scan: error symbol unknown :%s",scanner->value.v_string);
        return FALSE;
        }
    }
  while (scanner->token != G_TOKEN_EOF);
      ...
      case SIM_COMMAND_SYMBOL_PLUGIN_SID:
		...
		case SIM_COMMAND_SYMBOL_REP_REL_DST:
 		...
 		case SIM_COMMAND_SYMBOL_REP_ACT_SRC:
			...
			command->data.host_ids_event.rep_act_src = g_strdup (scanner->value.v_string);
			break;
 		...
 		case SIM_COMMAND_SYMBOL_REP_ACT_DST:
			...
			command->data.host_ids_event.rep_act_dst = g_strdup (scanner->value.v_string);
			break;
  ...
  do
    {
      ...
      case SIM_COMMAND_SYMBOL_ID:
        ...
        if (sim_string_is_number(scanner->value.v_string, 0))
          ...
          {
            g_message(
                "Error: ERROR event incorrect. Please check the id issued from the remote machine: %s",
                scanner->value.v_string);
            return FALSE;
          }
        break;
      ...
      default:
        if (scanner->token == G_TOKEN_EOF)
          break;
        g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
            "sim_command_error_scan: error symbol unknown");
        return FALSE;
        }
    }
  while (scanner->token != G_TOKEN_EOF);
...
sim_command_get_event(SimCommand *command)
{
  ...
  {
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_command_get_event event->type = None");
    return NULL;
  }
  ...
  {
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_command_get_event command->data.event.date= NULL");
    return NULL;
  }
  ...
  if (!event->src_ia)
  {
    ...
    if (!event->src_ia)
    {
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_command_get_event: event->src_ia = NULL, command->data.event.src_ip: %s command->data.sensor.host:%s ?? Maybe not valid command?", command->data.event.src_ip, command->data.sensor.host);
      g_object_unref(event);
      return NULL;
    }
    else
  ...
  if (command->data.event.rep_act_src)
    event->rep_act_src = g_strdup (command->data.event.rep_act_src);
  if (command->data.event.rep_act_dst)
    event->rep_act_dst = g_strdup (command->data.event.rep_act_dst);
  ...
  {
    event->textfields[SimTextFieldUserdata1] = g_strdup("Please check the src ip of this plugin");
  ...
  {
    event->textfields[SimTextFieldUserdata2] = g_strdup("Please check the src ip of this plugin");
    insert_warning_message = FALSE;
  }
  ...
  {
    event->textfields[SimTextFieldUserdata3] = g_strdup("Please check the src ip of this plugin");
    insert_warning_message = FALSE;
  }
  ...
  {
    event->textfields[SimTextFieldUserdata4] = g_strdup("Please check the src ip of this plugin");
    insert_warning_message = FALSE;
  }
  ...
  {
    event->textfields[SimTextFieldUserdata5] = g_strdup("Please check the src ip of this plugin");
    insert_warning_message = FALSE;
  }
  ...
  {
    event->textfields[SimTextFieldUserdata6] = g_strdup("Please check the src ip of this plugin");
    insert_warning_message = FALSE;
  }
  ...
  {
    event->textfields[SimTextFieldUserdata7] = g_strdup("Please check the src ip of this plugin");
    insert_warning_message = FALSE;
  }
  ...
  {
    event->textfields[SimTextFieldUserdata8] = g_strdup("Please check the src ip of this plugin");
    insert_warning_message = FALSE;
  }
  ...
  {
    event->textfields[SimTextFieldUserdata9] = g_strdup("Please check the src ip of this plugin");
    insert_warning_message = FALSE;
  }
  ...
  if (command->type == SIM_COMMAND_TYPE_SNORT_EVENT)
  {
    ...
    simpluginsid = sim_container_get_plugin_sid_by_pky(ossim.container, event->plugin_id, event->plugin_sid);
    if (simpluginsid)
      event->log = g_strdup_printf("%s, src:%u dst:%u", sim_plugin_sid_get_name(simpluginsid), command->data.event.src_ip, command->data.event.dst_ip);
    else
    {
      event->log = g_strdup_printf("Event unknown, please insert plugin_id: %d and plugin_sid: %d into DB. src:%u dst:%u", event->plugin_id, event->plugin_sid, command->data.event.src_ip, command->data.event.dst_ip);
      g_message( "Event unknown, please insert plugin_id: %d and plugin_sid: %d into DB", event->plugin_id, event->plugin_sid);
    }
  }
  return event;
}
      ...
      if (command->data.connect.version != NULL)
        {
          while (agent_parsers_table[i].pf != NULL)
            {
              if (g_str_has_prefix (command->data.connect.version,agent_parsers_table[i].version))
                {
