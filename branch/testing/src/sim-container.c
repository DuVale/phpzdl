...
void sim_container_remove_disabled_directives_ul (SimContainer *container,
...
sim_container_impl_finalize(GObject *gobject)
{
  ...
  sim_container_free_home_net (container);
...
sim_container_db_load_plugins_ul(SimContainer *container, SimDatabase *database)
{
  ...
  container->_priv->plugins = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, g_object_unref);
          ...
          for (row = 0; row < gda_data_model_get_n_rows(dm); row++)
            {
              plugin = sim_plugin_new_from_dm(dm, row);
  				    if(plugin)
	  				    g_hash_table_insert(container->_priv->plugins, GINT_TO_POINTER(sim_plugin_get_id(plugin)), plugin);
            }
 ...
 */
void
sim_container_add_plugin_ul (SimContainer  *container,
                             SimPlugin     *plugin)
{
  g_return_if_fail(SIM_IS_CONTAINER (container));
...
sim_container_get_plugins_ul(SimContainer *container)
{
  ...
  return container->_priv->plugins;
}
...
sim_container_free_plugins_ul(SimContainer *container)
{
  ...
  g_hash_table_destroy(container->_priv->plugins);
  container->_priv->plugins = NULL;
}
																	 ...
																	 gint           id)
{
  ...
  if(container->_priv->plugins)
    return g_hash_table_lookup(container->_priv->plugins, GINT_TO_POINTER(id));
  else
    return NULL;
}
 ...
 */
void
sim_container_add_plugin (SimContainer  *container,
												  SimPlugin       *plugin)
{
  g_return_if_fail(container);
  g_return_if_fail(SIM_IS_CONTAINER (container));
  ...
  G_LOCK(s_mutex_plugins);
  sim_container_add_plugin_ul (container, plugin);
  G_UNLOCK(s_mutex_plugins);
}
...
sim_container_get_plugins(SimContainer *container)
{
  ...
  G_LOCK(s_mutex_plugins);
  table = sim_container_get_plugins_ul (container);
  G_UNLOCK(s_mutex_plugins);
    ...
    SimDatabase *database)
{
  ...
  gint          plugin_id, sid, nsids, tmp_sids, last_sid;
  ...
  gchar		* select_sids = "select plugin_id, sid, reliability, priority, name FROM plugin_sid where plugin_id BETWEEN %d AND %d %s";
  gchar		* select_sids_group = "select plugin_id, sid, reliability, priority, name FROM plugin_sid where plugin_id = %d AND sid > %d ORDER BY 2 LIMIT %d";
  gboolean      flag_first;
  gchar         *not_group = NULL;

  g_return_if_fail(SIM_IS_CONTAINER (container));
  g_return_if_fail(SIM_IS_DATABASE (database));

  container->_priv->plugin_sids = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)g_object_unref);

  //g_message("%s", select_grp);
  ...
  if (dm_grp)
  {
	...
	plugin_end = gda_value_get_integer (value);
      ...
      // If a plugin has a lot of plugin sids associated, then we will process them separately
      value = (GdaValue *) gda_data_model_get_value_at (dm_grp, 1, row_grp);
      tmp_sids = gda_value_get_bigint (value);
      if(tmp_sids > MAX_NSIDS_GROUP)
      {
        if(not_group)
        {
          gchar *aux = g_strdup_printf("%s, %d", not_group, plugin_end);
          g_free(not_group);
          not_group = aux;
        }
        else
          not_group = g_strdup_printf("%d", plugin_end);
          
        //Select SIDS_GROUP rows
        last_sid = 0;
        do
        {
          gchar *query = g_strdup_printf(select_sids_group, plugin_end, last_sid, MAX_NSIDS_GROUP);
          //g_message("%s",query);
            ...
            for (row_sids = 0; row_sids < count_sids; row_sids++)
	          {
	            ...
	            if (plugin_sid)
	            {
                g_hash_table_insert(container->_priv->plugin_sids,
                                    GUINT_TO_POINTER(sim_plugin_sid_get_cantor_key(plugin_sid)),
                                    plugin_sid);
                last_sid = sim_plugin_sid_get_sid(plugin_sid);
	            }
      	    }
        ...
        g_log (G_LOG_DOMAIN,G_LOG_LEVEL_DEBUG, "Plugin %d loaded. %d sids", plugin_end, tmp_sids);
        continue;
      }

      nsids += tmp_sids;
      if(nsids > MAX_NSIDS_GROUP || row_grp == count_grp - 1)
      {
        flag_first = TRUE;
        if(not_group)
        {
          gchar *aux = g_strdup_printf(" AND plugin_id NOT IN (%s)", not_group);
          g_free(not_group);
          not_group = aux;
        }
        else
	...
	gchar *query = g_strdup_printf(select_sids, plugin_ini, plugin_end, not_group);
        //g_message("%s",query);
	dm_sids = sim_database_execute_single_command (database, query);
	if (dm_sids)
	{
	  ...
	  g_object_unref(dm_sids);
    g_log (G_LOG_DOMAIN,G_LOG_LEVEL_DEBUG,"Plugins %d - %d loaded. %d sids", plugin_ini, plugin_end, nsids);
	}
  else
  ...
  if (sim_database_execute_no_query (database, ins_demo_events) < 0)
	...
	/* Debug code*/
	GList *k;
	for(k = g_hash_table_get_keys (container->_priv->plugins);k!=NULL;k = k->next){
		g_log (G_LOG_DOMAIN,G_LOG_LEVEL_DEBUG,"%s: Hash table key:%s",__FUNCTION__,(gchar*)k->data);
	}
#endif
 ...
 */
void
sim_container_add_plugin_sid_ul (SimContainer  *container,
...
sim_container_get_plugin_sids_ul(SimContainer *container)
{
  ...
  return container->_priv->plugin_sids;
}
...
sim_container_free_plugin_sids_ul(SimContainer *container)
{
  ...
  g_return_if_fail(SIM_IS_CONTAINER (container));
	...
	g_hash_table_destroy(container->_priv->plugin_sids);
}
                                        ...
                                        gint sid)
{
  ...
  g_return_val_if_fail(sid > 0, NULL);
	...
	if(container->_priv->plugin_sids)
	{
 		g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "%s: looking for id:%d and sid:%d",__func__, plugin_id, sid);

    guint cantor_key = CANTOR_KEY(plugin_id, sid);
		plugin_sid = g_hash_table_lookup(container->_priv->plugin_sids, GUINT_TO_POINTER(cantor_key));
	}
																				...
																				const gchar   *name)
{
  ...
  g_return_val_if_fail(name, NULL);
	...
	g_hash_table_iter_init (&iter, container->_priv->plugin_sids);
	while (g_hash_table_iter_next (&iter, &key, &value))
  {
    plugin_sid = (SimPluginSid *) value;
 ...
 */
void
sim_container_add_plugin_sid (SimContainer  *container,
                              SimPluginSid *plugin_sid)
{
  g_return_if_fail(container);
  g_return_if_fail(SIM_IS_CONTAINER (container));
  ...
  G_LOCK(s_mutex_plugin_sids);
  sim_container_add_plugin_sid_ul (container, plugin_sid);
  G_UNLOCK(s_mutex_plugin_sids);
}
...
sim_container_get_plugin_sids(SimContainer *container)
{
  ...
  G_LOCK(s_mutex_plugin_sids);
  table = sim_container_get_plugin_sids_ul (container);
  G_UNLOCK(s_mutex_plugin_sids);
...
sim_container_db_load_host_plugin_sids_ul (SimContainer * container, SimDatabase * database)
{
  ...
  gint            reference_sid;
	...
	host_plugin_sids = g_hash_table_new_full(g_str_hash, g_str_equal, (GDestroyNotify) g_free, (GDestroyNotify) g_list_free);
...
{
  GdaConnection *conn;
  char *sensor_name, *e_sensor_name;
  ...
  conn = sim_database_get_conn (database);

  if (sensor_name = sim_sensor_get_name (sensor))
  {
    e_sensor_name = g_new0 (gchar, strlen (sensor_name) * 2 + 1);
    gda_connection_escape_string (conn, sensor_name, e_sensor_name);
  }
  else
    e_sensor_name = g_strdup ("");
  ...
  query
      = g_strdup_printf(
          ...
          event_number.host_ids_events, e_sensor_name);

  g_free (e_sensor_name);
  ...
  if (dm)
  {
    for (row = 0; row < gda_data_model_get_n_rows(dm); row++)
    {
      ...
      while (list)
      {
        ...
        container->_priv->home_net = g_list_prepend(container->_priv->home_net, sim_inet_clone(inet));

        list = list->next;
      } // while inet

      container->_priv->nets = g_list_prepend(container->_priv->nets, net);
    }
              ...
              for (row2 = 0; row2 < gda_data_model_get_n_rows(dm2); row2++)
                {
                  ...
                  if (g_strstr_len(str_port, strlen(str_port),
                      SIM_WILDCARD_ANY) || g_strstr_len(str_port, strlen(
                      str_port), "any"))
                    {
                      port_num = 0;
                    }
              ...
              for (row2 = 0; row2 < gda_data_model_get_n_rows(dm2); row2++)
                {
                  ...
                  if (g_strstr_len(sensor, strlen(sensor),
                      SIM_WILDCARD_ANY) || g_strstr_len(sensor, strlen(
                      sensor), "any"))
                    {
                      g_free(sensor);
              ...
              for (row2 = 0; row2 < gda_data_model_get_n_rows(dm2); row2++)
                {
                  ...
                  if (!g_ascii_strcasecmp(target_name, SIM_WILDCARD_ANY))
                    {
...
/**
 *  sim_container_remove_disabled_directives_ul
 *
 ...
 *  and removes the directives loaded with that directive_id
 *
 *  file format:
 *     <directive_id>;<directive filenane>
 */
void
sim_container_remove_disabled_directives_ul (SimContainer *container,
                                             const gchar  *filename)
{
  g_return_if_fail (SIM_IS_CONTAINER (container));
  ...
  if (g_file_get_contents (filename, &contents, &length, &error))
  {
    ...
    while (all_lines[i] != NULL)
    {
      ...
      // The directive Id is the first filed
      if (line[0] != NULL)
      {
  ...
  {
    g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Error Opening disabled directives file %s", filename);
  }
  ...
  // Check if there are disabled directives
  if (disabled == NULL)
    return;

  // Find disabled directives and remove them from the list
  GList *list = container->_priv->directives;
  GList *node = list;
  while (node)
	{
		SimDirective *directive = (SimDirective *) node->data;
    guint64 id = sim_directive_get_id (directive);

    if (g_list_find (disabled, (gpointer) id))
    {
      GList *next_node = g_list_next (node);
      g_object_unref (directive);
      list = g_list_delete_link (list, node);
      node = next_node;

      g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Directive Disabled directive_id=%d", id);
    }
    else
    {
    ...
    SimDatabase *db_ossim, const gchar *filename)
{
  ...
  sim_container_remove_disabled_directives_ul (container, "/etc/ossim/server/disabled_directives.data"); //FIXME filename in config?
...
sim_container_pop_event(SimContainer *container)
{
  ...
  g_atomic_int_dec_and_test(&container->_priv->events_len);
...
sim_container_debug_print_plugins(SimContainer *container)
{
  ...
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_container_debug_print_plugins");
	...
	g_hash_table_iter_init (&iter, container->_priv->plugins);
	while (g_hash_table_iter_next (&iter, (gpointer)&key,(gpointer)&value))
    sim_plugin_debug_print (value);
}
...
sim_container_debug_print_plugin_sids(SimContainer *container)
{
  ...
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,  "sim_container_debug_print_plugin_sids");
	...
	g_hash_table_iter_init (&iter, container->_priv->plugin_sids);
	while (g_hash_table_iter_next (&iter, (gpointer)&key,(gpointer)&value))
    sim_plugin_sid_debug_print (value);
}
...
gint sim_container_get_nbacklogs(SimContainer *container)
{
  gint nbacklogs = 0;
  g_return_val_if_fail (SIM_IS_CONTAINER (container), 0);
  g_mutex_lock (ossim.mutex_backlogs);
  nbacklogs = sim_container_get_nbacklogs_ul(container);
  g_mutex_unlock (ossim.mutex_backlogs);
  return nbacklogs;
}
gint sim_container_get_nbacklogs_ul(SimContainer *container)
{
  g_return_val_if_fail (SIM_IS_CONTAINER (container), 0);
  return g_list_length (container->_priv->backlogs);
}
...
void sim_container_db_load_events_count(SimContainer * container)
{
  ...
  gchar *query;
  gint nevents = 0;
  query = g_strdup("Select count(*) from acid_event");
  dm = sim_database_execute_single_command(ossim.dbsnort, query);
  if(dm)
  {
    value = (GdaValue*)gda_data_model_get_value_at(dm,0,0);
    container->_priv->events_count = gda_value_get_bigint (value);
 ...
 */
void
sim_container_increments_events_count (SimContainer * container)
{
  g_atomic_int_inc (&container->_priv->events_count);
  return;
}
 ...
 */
GList*
sim_container_get_home_net(SimContainer *container)
{
  g_return_if_fail(container);
  g_return_if_fail(SIM_IS_CONTAINER (container));

  return container->_priv->home_net;
}
 ...
 */
void
sim_container_free_home_net(SimContainer *container)
{
  GList *list;
  ...
  g_list_free(container->_priv->home_net);
  container->_priv->home_net = NULL;
}
