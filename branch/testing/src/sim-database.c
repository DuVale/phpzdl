...
static gint sim_database_signals[LAST_SIGNAL] =
  { 0 };
  ...
  if (!object_type)
    {
      static const GTypeInfo type_info =
        { sizeof(SimDatabaseClass), NULL, NULL,
            (GClassInitFunc) sim_database_class_init, NULL, NULL, /* class data */
            sizeof(SimDatabase), 0, /* number of pre-allocs */
            (GInstanceInitFunc) sim_database_instance_init, NULL /* value table */
        };

      g_type_init();

      object_type = g_type_register_static(G_TYPE_OBJECT, "SimDatabase",
          &type_info, 0);
    }
  ...
  if (config->local_DB)
    {
      ...
      db->_priv->conn = gda_client_open_connection_from_string(
          db->_priv->client, db->_priv->provider, db->_priv->dsn,
      ...
      for (i = 0; i < g_list_length(errors); i++)
        {
          ...
          g_message("ERROR %u: %s", gda_error_get_number(error),
      ...
      while (!GDA_IS_CONNECTION(database->_priv->conn) || !gda_connection_is_open(database->_priv->conn)) //if database connection is not open, try to open it.
        {
          ...
          database->_priv->conn = gda_client_open_connection_from_string(  database->_priv->client, database->_priv->provider,  database->_priv->dsn, GDA_CONNECTION_OPTIONS_DONT_SHARE);
      ...
      if (ret < 0)
        {
          ...
          for (i = 0; i < g_list_length(errors); i++)
            {
              error = (GdaError *) g_list_nth_data(errors, i);
              g_message("ERROR %s %u: %s", buffer, gda_error_get_number(error),
                  gda_error_get_description(error));
 ...
 */
GdaDataModel*
sim_database_execute_single_command(SimDatabase *database, const gchar *buffer)
{
