...
* - html_vul_scan()
* - ossim_valid()
* - ossim_make_valid() --> for delete all character not allower
* - ossim_set_error()
* - ossim_error()
* - valid_ip()
* - valid_cidr()
...
define('OSS_IP_ADDR_NET', 'func:valid_ip_3'); // check new host ip is valid with allowed sensor or allowed network
...
define('OSS_PASSWORD', OSS_DIGIT . OSS_ALPHA . OSS_PUNC_EXT);
		...
		foreach($parms as $p) {
			...
			if (preg_match('/^regex:(.+)$/', $p, $m)) {
				if (!preg_match('/^' . $m[1] . '$/', $v)) {
					...
					return ossim_set_error($error_msg . " (the string '<b>$v</b>' is not in a valid format)");
				}
				continue;
			}
			//
...
function ossim_make_valid() {
	...
	// {
		...
		$clean_v = $v;
        $match   = '/[^'.$val_str.']/';
        $clean_v = preg_replace($match,"",$clean_v);
...
function valid_user($user)
{
    ...
    {
        ossim_set_error(_("Invalid login")."<br/>Entered login: '<b>".Util::htmlentities($user)."</b>'");
    }

    $pattern = "/^[0-9]*$/";
    if ( preg_match($pattern, $user, $match) )
    {
        ossim_set_error(_("Invalid login")."<br/>Login can not be just numerical: '<b>".Util::htmlentities($user)."</b>'");
    }
...
function valid_cidr($cidrs)
{
	...
	{
		$v = trim($v);
		if ( preg_match('/^(\d+)\.(\d+)\.(\d+)\.(\d+)\/(\d+)$/', $v, $m) && $m[1] <= 255 && $m[2] <= 255 && $m[3] <= 255 && $m[4] <= 255 && $m[5] > 0 && $m[5] <= 32 ) 
			continue;
		else
		{
...
function valid_ip($ip) {
	...
	if ( !is_array($ip) )
	{
		if ( $ip != "0.0.0.0" && $ip != "255.255.255.255" && preg_match('/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/', $ip, $m) && $m[1] <= 255 && $m[2] <= 255 && $m[3] <= 255 && $m[4] <= 255 )
			return true;
	...
	else
	{
		foreach ($ip as $k => $v)
		{
			if ( $v != "0.0.0.0" && $v != "255.255.255.255" && preg_match('/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/', $v, $m) && $m[1] <= 255 && $m[2] <= 255 && $m[3] <= 255 && $m[4] <= 255 )
				continue;
			else
			{
...
// check new host ip is valid with allowed sensor or allowed network

function valid_ip_3($ip) {
...
require_once "classes/Net.inc";
	...
	{
		if ( $ip != "0.0.0.0" && $ip != "255.255.255.255" && preg_match('/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/', $ip, $m) && $m[1] <= 255 && $m[2] <= 255 && $m[3] <= 255 && $m[4] <= 255 ){
            if (Net::isIpInNet($ip,Session::allowedNets($user)))
                return true;
            else{
                if (Session::am_i_admin())
                    return true;
                ossim_set_error(_("Invalid IP address. Not valid with allowed sensor or allowed network") . "<br/>Entered ip: '<b>$ip</b>'");
                return false;
            }
...
function valid_hostname($hostname)
{
    ...
    if( strlen($hostname) > 255 ) {
        ossim_set_error(_("Invalid Host name, it's too long"));
        return false;
    }
    else if( preg_match('/^[a-zA-Z0-9\-]{1,255}$/',$hostname) && !preg_match('/\./',$hostname)) {
        return true;
    }
    ...
    foreach($labels as $label) {
        // Validate Host name label
        if ( !preg_match('/^[a-zA-Z0-9\-]{1,63}$/',$label) ) {
            if(strlen($label)>63) {
                if(strlen($label) > $max) {
                    $label = substr ($label, 0, $max) . "...";
                }
                ossim_set_error(_("Invalid label: ").$label._(", it's too long"));
            }
            else {
                ossim_set_error(_("Invalid Host name"));
            }
            return false;
        }
    }
...
function valid_fqdns($fqdns)
{
    ...
    foreach ($array_fqdns as $k => $v)
    {
        ...
        if( strlen($v) > 255 ) 
        {
            $max = 30;
            $v   = substr ($v, 0, $max)."[...]";
            ossim_set_error(_("Invalid FQDN/Alias: $v. It's too long"));
            return false;
        }
        else 
        {
            if ( !preg_match('/\./',$v) )
            {
                if ( !preg_match('/^([a-zA-Z])([a-zA-Z0-9\-]*[a-zA-Z0-9]+)$/',$v) )
                {
                    $max = 40;
            ...
            else
            {
                ...
                {
                    // Validate FQDN label
                    if ( preg_match('/^([a-zA-Z])([a-zA-Z0-9\-]+[a-zA-Z0-9]?)$/',$label) ) 
                    {
                        if(strlen($label) > 63 ) 
                        {
                            $max = 30;
                    ...
                    else
                    {
                        ...
                        ossim_set_error(_("Invalid format label: $label"));
                        return false;
                    }
...
function valid_filename($filename)
{
    ...
    ossim_set_error(_("Invalid filename."));
    return false;
}
