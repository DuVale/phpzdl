...
class ApacheNtopProxyManager:
    ...
    def __getSensorList(self):
        ...
        #read sensor list.
        query = 'select sensor.ip, sensor.name from sensor join sensor_properties where sensor.ip = sensor_properties.ip and sensor_properties.has_ntop=1;'
        tmp = None
...
#        self.__myDB_connected = False
    ...
    def refreshDefaultNtopConfiguration(self,first_sensor_name = None, must_reload = False):
        ...
        default_sensor_config_file = open(Const.NTOP_REWRITE_DEFUALT_CONFIG_FILE, 'w')
        sensor_ip = ""
        line = ""
        if valid_ntop_link:                
