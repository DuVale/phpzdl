...
class BPMember:
    ...
    def update_status(self):
        ...
        #logger.info("----------------------------------------------------------------------")
        #logger.info("----------------------------------------------------------------------")
    def get_net_availability(self, netname):
        query = "select ip from host;" 
        host_list = _DB.exec_query(query)
        tmp_availability = 0
        for host in host_list:
            query = "select cidr,name from net_cidrs c,net n where n.ips=c.cidr and begin<=inet_aton('%s') and end>=inet_aton('%s') and name='%s' order by end-begin asc;" % (host['ip'], host['ip'], netname)
            result = _DB.exec_query(query)
            if result != []:
                query = "select severity as ha from bp_member_status where member='%s' and measure_type='host_availability';" % host['ip']
    ...
    def get_netgroup_availability(self, netgroup):
        query = """SELECT net_name FROM net_group_reference where net_group_name = '%s';""" % netgroup
        result = _DB.exec_query(query)
        tmp_availibility = 0
        for row in result:
            n_name = row['net_name']
        ...
        ###NET_Group measures ###
            ...
            'net_group_vulnerability': \
                MeasureDB (
                    ...
                    request="""
                        select severity as net_group_vulnerability from ossim.net_group_reference as refer,
                            ossim.bp_member_status as stat where net_group_name = '%s' and
                            refer.net_name = stat.member and
            ...
            'net_group_alarm': \
                MeasureDB (
                    ...
                    request="""
                        select severity as net_group_alarm from ossim.net_group_reference as refer,
                            ossim.bp_member_status as stat where net_group_name = '%s' and
                            refer.net_name = stat.member and
            ...
            'net_group_metric': \
                MeasureDB (
                    ...
                    request="""
                        select severity as net_group_metric from ossim.net_group_reference as refer,
                            ossim.bp_member_status as stat where net_group_name = '%s' and
                            refer.net_name = stat.member and
            ...
            'net_group_incident': \
                MeasureDB (
                    ...
                    request="""
                        select severity as net_group_incident from ossim.net_group_reference as refer,
                            ossim.bp_member_status as stat where net_group_name = '%s' and
                            refer.net_name = stat.member and
...
#                        (m['member_type'])
                ...
                if m['member_type'] == 'host':
                    query = "select ip  from host where hostname='%s'" % (m['member'])
                    if not re.match(ipv4_regex, m['member']):
                        result = _DB.exec_query(query)
                        for row in result:#Must by only one row.
                            member = BPMemberHost(row['ip'])
                    else:
                    ...
                    ng_name = m['member']
                    query = """SELECT net_name FROM net_group_reference where net_group_name = '%s';""" % (m['member'])
                    result = _DB.exec_query(query)
                    for row in result:
                        n_name = row['net_name']
                        member = BPMemberNet(row['net_name'])
                        if member:
                            member.update_status()
                    member = BPMemberNetworkGroup(m['member'])
