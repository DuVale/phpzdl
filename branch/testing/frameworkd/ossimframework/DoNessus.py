...
class NessusManager:
    ...
    def process(self, message):
        ...
        if action == "status":
            logger.info("status: %s" % self.__nessus.status())
        ...
        if action == "reset" and self.__nessus.status() == -1:
            ...
            if self.__nessus.status() == -1:
                logger.info(" Previous scan aborted raising errors, please check your logfile. Error: %s" % self.__nessus.get_error())
...
# Short sanity check.
    ...
    def __test_write_fatal (self, path) :
        if not os.access(path, os.W_OK):
            ...
            logger.info("Nessus scan failed. Write permission needed on %s for user %d or group %d" % (path, os.geteuid(), os.getegid()))
            self.__cleanup()
            return
    ...
    def __update_cross_correlation (self, nsr_file) :
        ...
        try:
            ...
            logger.error("Unable to open file %s: %s" % (nsr_file, e))
            return
    ...
    def __update_vulnerability_tables(self, nsr_txt_file, today):
        ...
        try:
            ...
            logger.error("Unable to open file %s: %s" % (nsr_txt_file, e))
            return
            ...
            for host in hosts:
                ...
                if vuln_networks is not None:
                    for net in vuln_networks:
                        if not net_vuln_lvl.has_key(net):
                            net_vuln_lvl[net] = 0
#                    for net in vuln_networks:
#                        try: 
#                            net_vuln_lvl[net]
...
#                            net_vuln_lvl[net] = 0
    ...
    def __backup_vulnerability_tables(self, today):
        ...
        try:
            ...
            logger.error("Error executing sql backup query: %s" % str(e))

        query = "SELECT * FROM net_vulnerability INTO OUTFILE '%s'" % result_vuln_sql_net
        try:
            self.__conn.exec_query(query)
        except Exception, e:
            logger.error("Error executing sql backup query:" % str(e))
    ...
    def run (self) :
        ...
        if not self.__test_write(self.__dirnames["nessus_tmp"]):
            logger.info("Creating temp dir: %s" % self.__dirnames["nessus_tmp"])
            try :
        ...
        if nessus_distributed:
            ...
            for row in hash:
                try:
                    if Util.isIpInNet(row["temporal"], scan_networks):
                        logger.error("Dup: %s. Please check your config" % row["temporal"])
                    else:
                        self.__debug("Adding host %s" % row["temporal"])
                        scan_hosts.append(row["temporal"])
                        sensorfd.writelines(row["temporal"] + "\n")
        ...
        if os.path.exists(self.__filenames["result_nsr"]):
            ...
            logger.error("Scan failed check output and try enabling debug")
            return
    ...
    def load_shedule(self, id):
        ...
        logger.info("Schedule scan type: %s" % scan_type)
        if scan_type == "sensor":
            sensors = []
            sensors = self.get_sensors_by_id(id)
            if sensors == []:
                # Wrong scheduler id
                logger.warning("Wrong scheduler id %d" % int(id))
                sys.exit()
            else:
                logger.warning("Sensor_list: %s" % sensors)
            ...
            if nets_list == [] and hosts_list == [] and hostgroups_list == []:
                # Wrong scheduler id
                logger.info("Wrong scheduler id %d" % int(id))
                sys.exit()
