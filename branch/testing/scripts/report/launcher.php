...
function get_timestamp ($dbconn, $login, $datetime) {
    ...
    return gmdate("Y-m-d H:i:s",Util::get_utc_unixtime($dbconn, $datetime)+(3600*$tz));
}
...
foreach ( $scheduled_reports as $value )
{
    ...
    $pass  = base64_encode(getUserWeb($conn, $value['user']));

    $uuid = get_report_uuid($conn, $user);

    $db->close($conn);

	$step1 = exec('wget -U "AV Report Scheduler ['.$id_sched.']" -t 1 --timeout=7200 --no-check-certificate --cookies=on --keep-session-cookies --save-cookies='.$cookieName.' --post-data="user='.$user.'&pass='.$pass.'" "'.$server.'/session/login.php" -O - 2>> /tmp/logscheduler_err',$output);
	...
	// Run Report
	$output  = null;
	$step2   = exec('wget -U "AV Report Scheduler ['.$id_sched.']" -t 1 --timeout=7200 --no-check-certificate --cookies=on --keep-session-cookies --load-cookies='.$cookieName.' "'.$server.'/report/wizard_run.php?run='.$value['id_report'].'&'.$params.'" -O - 2>> /tmp/logscheduler_err', $output);
	...
	{
		$step3 = exec('wget -U "AV Report Scheduler ['.$id_sched.']" -t 1 --timeout=7200 --no-check-certificate --cookies=on --keep-session-cookies --load-cookies='.$cookieName.' "'.$server.'/report/wizard_run.php?pdf=true&extra_data=true&run='.$value['id_report'].'" -O '.$dirUserPdf.$pdfName.'.pdf 2>> /tmp/logscheduler_err', $output);
	...
	// Logout
	exec('wget -U "AV Report Scheduler ['.$id_sched.']" -t 1 --timeout=7200 --no-check-certificate --cookies=on --keep-session-cookies --load-cookies='.$cookieName.' "'.$server.'/session/login.php?action=logout" -O /dev/null 2>> /tmp/logscheduler_err');
