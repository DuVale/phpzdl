...
$CONFIG{'NMAPPATH'} = $nessus_vars{'nmap_path'};
...
my $job_id_to_log            = "";
my $server_slot              = 1;
my $sinput                   = TRUE; # use standar input stream by default
my $latest_binary_location   = "/usr/local/bin/omp";
...
sub select_job {
    ...
    my $vuln_nessus_servers = $sth_sel->fetchall_hashref('id');
    $sth_sel->finish;
    ...
    foreach my $serverid (sort(keys(%{$vuln_nessus_servers}))) {
        ...
        my $sql_filter = "AND ( t1.scan_ASSIGNED='$serverid' OR t1.scan_ASSIGNED IS Null ) ";
...
sub manually_import_report {
    ...
    if ( !$job_id ) {
        $sql = qq{ INSERT INTO vuln_jobs ( name, username, job_TYPE, meth_TARGET, 
            meth_VSET, status, authorized, author_uname ) VALUES ( 
...
sub queueWork {
    ...
    while( my ( $subID, $subnetName, $purpose ) = $sth_sel->fetchrow_array) {
		...
		logwriter( "\tRESTRICTED [ NOT QUEUED ] subID: $subID\tNAME: $subnetName\n", 2 );
	            ...
	            meth_VSET, scan_SUBMIT, meth_Wcheck, status, notify, authorized, author_uname
                ) VALUES (
                    'CRON - $subnetName', 'Admin', '$subnetName', 'C', 'C', '$subnetName', $useCred, 
                    '$usevSet', '$sql_time', $winaudit, 'S', '$notify', '1', 'ACL' ); };
...
sub process_results {
    ...
    if ( !$isTop100Scan ) { # GENERATE FULL REPORT WHEN NOT TOP100 AUDIT
        ...
        # there are networks
        if ($targets =~ /\//) {
            my @anets = split(/,/,$targets);
            foreach my $anet (@anets) {
                $anet =~ s/\s|\n|\r|\t//g;
                if ($anet=~/(.*)\/(.*)/) { 
                    $sql = qq{ SELECT name FROM net WHERE ips='$anet'};
                    $sth_sel = $dbh->prepare( $sql );
                    $sth_sel->execute;
                    my ($nname) = $sth_sel->fetchrow_array( );
                    $nname = $anet if ($nname eq "");
                    $ntargets{$anet} = $nname;
                }
            }
        }
...
sub check_schedule {
    ...
    while ( my ($jid, $name, $username, $fk_name, $jobTYPE, $schedule_type, $day_of_week, $day_of_month, $time_run, $email,
        ...
        my $nextscan = gen_sched_next_scan ( $jid, $schedule_type );
...
sub get_server_credentials {
    ...
    $sql = qq{ SELECT id, max_scans, current_scans FROM vuln_nessus_servers 
        WHERE enabled='1' AND status='A' AND id='$select_id' };
...
sub generate_email {	#REWROTE TO USE ONE ROUTINE TO GENERATE ALL EMAIL
    my ( $job_id, $email_type ) = @_;

    my $org = $CONFIG{'siteORG'};
    my $company = $CONFIG{'company'};
    my $sig = $CONFIG{'mailSignature'};

    my $txt_cred = "";
    if ( $isComplianceAudit ) {
        $txt_cred = "Compliance Audit ";
    } elsif ( defined ( $cred_name ) && $cred_name ne "NULL" ) {
        $txt_cred = "Credential based ";
    } else {
        $txt_cred = "Null based";
    }

    my ( $sql, $sth_sel, $subject, $body, $formname, $statistics, $pHostCnt, $txt_system, $email_addr, $email_cc ) = "";

    $sql = qq{ SELECT t1.id, t1.name, t1.username, t1.fk_name, t1.job_TYPE, t1.meth_TARGET,
        t1.meth_CRED, t1.meth_VSET, t1.scan_START, t1.status, t1.notify, t2.hostname
        FROM vuln_jobs t1 LEFT JOIN vuln_nessus_servers t2 ON t2.id = t1.scan_SERVER  WHERE t1.id='$job_id' LIMIT 1 };
    logwriter( $sql, 5 );
    $sth_sel = $dbh->prepare( $sql );
    $sth_sel->execute(  );

    my ( $Jid, $Jname, $juser, $jbfk_name, $Jtype, $host_list, $credAudit, $Jvset, $JStart, $Jstatus, $sNotify,  $jServerName ) = $sth_sel->fetchrow_array(  );

    if ( !$Jid ) {
        die ("Failed to Connect to Database / Invalid record for JobID \"$job_id\n");
    }

    if ( $email_type =~/start/ ) {

	my ( $dtLastScan, $jHostCnt, $slruntime, $slSerious, $slHigh ) = "";
	$sql = "";

	if ( $Jtype =~ /C/ ) {
	    $sql = qq{ SELECT t2.name, t1.cntHosts, t2.dtLastScanned, t2.iScantime, t2.vSerious, t2.vHigh
                FROM vuln_subnets t1
		    LEFT JOIN vuln_nessus_report_stats t2 on t1.report_id = t2.report_id
	        WHERE t1.CIDR='$jbfk_name' LIMIT 1; }
        } elsif ( $Jtype =~ /S/ ) {
	    #$sql = qq{ SELECT t2.name, t1.cntHosts, t2.dtLastScanned, t2.iScantime, t2.vSerious, t2.vHigh
        	#FROM vuln_systems t1
		#    LEFT JOIN vuln_nessus_report_stats t2 on t1.report_id = t2.report_id
	        #WHERE t1.acronym='$jbfk_name' LIMIT 1; }
        }
	if ( $sql ne "" ) {
            logwriter( $sql, 5 );
	    $sth_sel = $dbh->prepare( $sql );
	    ...
	    ( $dtLastScan, $jHostCnt, $slruntime, $slSerious, $slHigh ) = $sth_sel->fetchrow_array(  );
	}

	if ( $jServerName ) { $statistics .= "\t Nessus Server: $jServerName\n";	}
        if ( $jHostCnt )    { $statistics .= "\t   Known Hosts: $jHostCnt\n";		}
        if ( $host_list )   { $statistics .= "\tTargeted Hosts: $host_list\n";		}

        if ( $Jstatus ) {
	    switch ($Jstatus) {
                case "C"    { $statistics .= "\t   Scan Status: Completed\n"		}
                case "F"    { $statistics .= "\t   Scan Status: Failed\n"		}
                case "H"    { $statistics .= "\t   Scan Status: Hold\n"			}
                case "R"    { $statistics .= "\t   Scan Status: Running\n"		}
                case "S"    { $statistics .= "\t   Scan Status: Scheduled\n"	        }
                else        { $statistics .= "\t   Scan Status: Staring [$Jstatus]\n"   }
            }  
        }

        if ( $JStart )	{ $statistics .= "\t  Scan Started: $JStart\n";		        }
        if ( $Jtype =~ /C/ ) {
	    if ( $dtLastScan ) {
		$statistics .= "\n"
                        ."        Scan Details: \n"
                        ."      Date Last Scan: $dtLastScan\n"
                        ."   Elapsed Scan Time: $slruntime\n"
                        ." Last Scan Ser Vulns: $slSerious\n"
                        ."Last Scan High Vulns: $slHigh\n";
            } else {
	        $statistics .= "\n\t Scan Details: This is the first recorded scan against address space \"$jbfk_name\"\n";
            }
	}
	
	if ( $Jtype =~ /S/i ) { $txt_system = "SYSTEM "; } else { $txt_system = " "; }

	if ( $Jtype =~ /C/ ) {
	    $subject = "$txt_cred Nessus Scan start job [ $jbfk_name ]"
	} else {
	    $subject = "$txt_cred Nessus Scan start job [ $Jname" . $txt_system . "]"
	}
    } else {

	$sql = qq{ SELECT t3.scantime, t1.job_TYPE, t1.meth_TARGET, t1.scan_START, 
            t1.scan_END, t1.scan_SUBMIT, t1.status, t2.iHostCnt, 
            t2.iScantime, t2.vSerious, t2.vHigh, t2.vMed, t2.vLow, t2.vInfo, 
	    t3.report_id, t3.report_key
            FROM vuln_jobs t1
	        LEFT JOIN vuln_nessus_report_stats t2 ON t1.report_id = t2.report_id and t2.dtLastScanned >= t1.scan_END
        	LEFT JOIN vuln_nessus_reports t3 ON t2.report_id = t3.report_id
            WHERE t1.id = '$job_id' LIMIT 1  };
	...
	my ( $scantime, $cJobType, $txtHosts, $start_date, $end_date, $submit_date, $nstatus, 
            $scntHosts, $siScantime, $svSerious, $svHigh, $svMed, $svLow, $svInfo, $report_id, $report_key
         ) = $sth_sel->fetchrow_array(  );

	if ( $Jtype =~ /S/i ) { $txt_system = "SYSTEM "; }

        if ( $scntHosts && $scntHosts > 0) { #USE SCAN RESULTS
            # do nothing $scntHost is fine
        } else {
            $scntHosts = 0;
        }
        
        $statistics .= "\t  Known Hosts: $scntHosts \n";

        if ( !defined($svSerious))  { $svSerious = 0;    };
        if ( !defined($svHigh))     { $svHigh = 0;       };
        if ( !defined($svMed))      { $svMed = 0;        };
        if ( !defined($svLow))      { $svLow = 0;        };
        if ( !defined($svInfo))     { $svInfo = 0;       };

        my $total_vulns = $svSerious + $svHigh + $svMed + $svLow + $svInfo;

        if ( $total_vulns > 0 ) {
            my $report_url = "";
            if ( $isComplianceAudit && ! $isNessusScan ) {
                $report_url = "$CONFIG{'inprotect_url'}/respdfc.php?scantime=$scantime&scantype=$cJobType&key=$report_key";
            } else {
                $report_url = "$CONFIG{'inprotect_url'}/respdf.php?scantime=$scantime&scantype=$cJobType&key=$report_key";
            }
            ...
            $statistics .= "\n"
                ...
                ."\t\t    Low: $svLow\n"
                ."\t\t   Info: $svInfo\n\n"
                    ."\tA full report can be downloaded from the following URL: $report_url .  "
                ."Additionally, a shorter report (Limited to Critical/High Vulnerabilities only) can be downloaded from the following URL: $report_url&critical=3 .  "
                ."This report is dynamically generated PDF and should be saved to disk for off-line viewing and archival purposes.\n";

        } else {

            $statistics .= "\n\t  Scan Details: No known vulnerabilities for job \"$Jname $txt_system\"\n";
        }

	if ( $Jtype =~ /S/i ) { $txt_system = "SYSTEM "; }

        if ( $email_type =~/failed/ ) {
            $subject = "$txt_cred Nessus Scan failed for [ $Jname $txt_system]"
        } else {
            if ( $Jtype =~ /C/ ) {
	        $subject = "$txt_cred Nessus Scan completed for [ $jbfk_name ]"
            } else {
	        $subject = "$txt_cred Nessus Scan completed for [ $Jname $txt_system]"
            }
        }
    }

    #READ FORM EMAIL MESSAGE
    #PICK THE PROPER EMAIL FORM ( cron_start/cron_finish/sched_start/sched_finish)
    if ( $email_type =~/failed/ ) {
        $formname = "scan_failed";
    } else {
        if ( $Jtype =~ /C/ ) { $formname = "cron_$email_type"; } else { $formname = "sched_$email_type"; }
    }
    open (MESSAGE, "$messages_dir/$formname") || die "couldn't open the file!";
    while (my $line = <MESSAGE>) {
        $body .= $line;
    }
    close(MESSAGE);

    #UPDATE MESSAGE VARIABLES WITH APPROPRIATE DATA
    $body =~ s/\[USERNAME\]/\t$juser/;
    $body =~ s/\[JOBNAME\]/  JOB NAME: $Jname/;
    $body =~ s/\[ORG\]/$org/g;
    $body =~ s/\[COMPANY\]/$company/g;
    $body =~ s/\[SIGNATURE\]/$sig/;  

    $body =~ s/\[ADDRESS_SPACE\]/\t$host_list/;

    $body =~ s/\[STATISTICS\]/$statistics/;
    $body =~ s/\[EXCLUSIONS\]/$exclude_hosts/;


    if ( $Jtype =~ /C/ ) {

	$sql = qq{ SELECT t2.tiAutoEmail, t2.poc, t2.poc_alt FROM vuln_jobs t1
	    LEFT JOIN vuln_subnets t2 on t1.fk_name=t2.CIDR WHERE t1.id = '$job_id' LIMIT 1  };
	...
	$sth_sel->execute(  );
        ...
        my ( $bAutoEmail, $poc, $poc_alt ) = $sth_sel->fetchrow_array(  );

	if ( $bAutoEmail && $sNotify ne "-1" ) {
            if ( $poc ne "" ) { $email_addr = "$poc"; }
            if ( defined ($poc_alt) && $poc_alt ne "" ) {
	        if ( $email_addr ne "" ) { $email_addr .= ", "; }
                $email_addr .= "$poc_alt"; 
            }
	    if ( $email_addr eq "" ) { $email_addr = "$CONFIG{'replyto'}"; }
        } else {
	    $email_addr = "$CONFIG{'replyto'}";
        }
    } else {
    ...
    if ( $Jtype =~ /S/i ) { 
        $email_cc = "$CONFIG{'mailcc'}, $CONFIG{'systemcc'}";
    } else {
        $email_cc = $CONFIG{'mailcc'};
    }

    #SEND EMAIL RESULTS AS PLAIN TEXT
    if ( $sNotify ne "-1" ) {
        print "JOB [ $job_id ] SENDING START EMAIL!\n";
        send_email( $email_addr, $CONFIG{'mailcc'}, $subject, "text", $body, "" );
    } else {
        print "NOTIFICATION OVERRIDE - NO EMAIL!\n";
    }
}
...
sub execute_omp_command {
    ...
    if(-e $latest_binary_location and $sinput) {
        $openvas_manager_common = "$latest_binary_location -h $nessushost -p $nessusport -u $nessususer -w $nessuspassword -iX";
        ...
        if ($debug_file ne "") {
            system ("echo '$openvas_manager_common \"$cmd\"' >> '$debug_file'");
            system ("cat $xml_output >> '$debug_file'");
            system ("echo '' >> '$debug_file'");
        }
...
sub check_running_scans {
    ...
    $sql = qq{ SELECT id, scan_PID, meth_Wfile FROM vuln_jobs WHERE status='R' };
    $sthse=$dbh->prepare( $sql );
    $sthse->execute;
    while ( my($id, $scan_pid, $semail) = $sthse->fetchrow_array() ) {
