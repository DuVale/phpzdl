...
sub main {
    ...
    if( -e $latest_binary_location and $nessus =~ /omp\s*$/ and $sinput) {
        $openvas_manager_common = "$latest_binary_location -h $CONFIG{'NESSUSHOST'} -p $CONFIG{'NESSUSPORT'} -u $CONFIG{'NESSUSUSER'} -w $CONFIG{'NESSUSPASSWORD'} -iX";
    }
    else {
            ...
            while (my ($psid, $pname, $powner) =$sth_sids->fetchrow_array ) {
                ...
                if($id_config ne "") {
                    ...
                    foreach my $family ( keys %familyHash ) {
                        ...
                        if( -e $latest_binary_location and $sinput ) { # with standar input
                            my $cmd = "<modify_config config_id='$id_config'><nvt_selection><family>$family</family>";
...
CREATE TABLE `vuln_nessus_preferences_defaults` (
  ...
  PRIMARY KEY  (`nessus_id`)
    ...
    if ($nessus =~ /omp\s*$/) {
        ...
        if( -e $latest_binary_location and $sinput ) { # standar input
            $file_omp_command = "/usr/share/ossim/www/tmp/omp_command_$$".int(rand(1000000)).".xml";
            ...
            $cmd = "$openvas_manager_common \"<get_preferences/>\" > $xml_output";
        }
...
sub create_profile {
    ...
    if( -e $latest_binary_location and $sinput ) { # standard input
        $file_omp_command = "/usr/share/ossim/www/tmp/omp_command_$$".int(rand(1000000)).".xml";
    ...
    if($type eq "F") { # Families
        ...
        foreach my $family ( keys %familyHash ) {
            if ($sinput) {
                $cmd = "<modify_config config_id='$new_config_id'><nvt_selection><family>$family</family>";
                logwriter("Updating family '$family'...", 4);
                $i = 0;
                foreach my $oid ( keys %{$familyHash{$family}} ) {
                    $cmd .= "<nvt oid='$oid'/>";
...
sub fill_preferences {
    ...
    if( -e $latest_binary_location and $sinput ) {
        ...
        $cmd = "$openvas_manager_common \"<get_preferences config_id='$id_ff'/>\" > $xml_output";
    }
    ...
    foreach my $profile (@items) {                                         
        if ($profile->{'in_use'} eq "0") {
            ...
            if( -e $latest_binary_location and $sinput ) {
                ...
                system("echo \"<delete_config config_id='$id_delete' />\" > '$file_omp_command'");
                
                $cmd = "$openvas_manager_common - < ".$file_omp_command." > $xml_output 2>&1";
            }
            else {
                $cmd = "$openvas_manager_common \"<delete_config config_id='$id_delete' />\" > $xml_output";
            }
...
sub get_openvas_manager_families {
    ...
    if( -e $latest_binary_location and $sinput ) {
        ...
        $cmd = "$openvas_manager_common \"<get_nvt_families />\" > $xml_output";
    }
...
sub execute_omp_command {
    ...
    if( -e $latest_binary_location and $sinput ) { # use standard input
        $file_omp_command = "/usr/share/ossim/www/tmp/omp_command_$$".int(rand(1000000)).".xml";
        ...
        $cmd = "$openvas_manager_common \"$cmd\" > $xml_output 2>&1";
    }
...
sub enable_port_scanner_plugins {
    ...
    while (my ($sid, $name, $user) = $sth_sel->fetchrow_array ) {
        $sql = qq{ UPDATE vuln_nessus_settings_plugins SET enabled='Y' WHERE sid=$sid AND ( id=14259 OR id=100315 ); };
        safe_db_write( $sql, 4 );
    ...
    logwriter( "FINISH - UPDATE PORT SCANNER [ Process took $time_run seconds ]", 4 );
}
