...
require_once('/usr/share/ossim/www/vulnmeter/config.php');
require_once('/usr/share/ossim/www/vulnmeter/functions.inc');
require_once('classes/Util.inc');
require_once('ossim_conf.inc');

ini_set('include_path', '/usr/share/ossim/www/vulnmeter');
require('/usr/share/ossim/www/vulnmeter/inc/pdf.php');
...
$query = "SELECT report_id FROM vuln_nessus_reports  WHERE scantime='$scantime' AND scantype='M' LIMIT 1";
...
if ($report_id) {
    $query ="SELECT t1.username, t1.name, t2.name, t2.description
            ...
            WHERE t1.report_id in ($report_id) order by t1.SCAN_END DESC";
	...
	$pdf->SetFont('Helvetica', '', 9);
	$pdf->Cell(30,6,_($profile_label).":",1,0,'L');
	...
	{
		$job_n = mb_convert_encoding($job_n,'UTF-8','ISO-8859-1');
	}
	
	
	
	$pdf->SetFont('Helvetica', '', 9);
	$pdf->Cell(30,6,_($job_label).":",1,0,'L');
...
#    $pdf->Ln();
	...
	$query = "SELECT COUNT( risk ) AS count, risk FROM (SELECT DISTINCT t1.hostIP, t1.risk, t1.port, t1.protocol, t1.app, t1.scriptid, t1.msg
			FROM vuln_nessus_results t1
			WHERE report_id in ($report_id) AND scantime=$scantime AND t1.falsepositive<>'Y') as t GROUP BY risk";
    ...
    foreach ($hosts as $hostIP=>$hostname) {
       ...
       $host_risk = array ( 0,0,0,0,0,0,0,0);
        ...
        WHERE report_id in ($report_id) AND hostIP='$hostIP' AND scantime=$scantime and falsepositive <> 'Y') as t1 group by hostIP, risk";
   ...
   if ( $query_critical == "" ) {
      ...
      $query="SELECT distinct hostip, service, port, protocol, app, risk, msg FROM vuln_nessus_results
           WHERE report_id in ($report_id) AND scantime=$scantime AND msg='' AND risk='7' AND falsepositive='N' ORDER BY result_id ASC";
   ...
   $query = "SELECT distinct t1.hostIP, t2.hostname, t1.service, t1.port, t1.protocol, t1.app, t1.risk, t1.scriptid, v.name, t1.msg
			 ...
			 WHERE t1.report_id in ($report_id) AND scantime=$scantime and t1.falsepositive<>'Y'
 ...
 //Vulnerability table configs
   ...
   // iterate through the IP is the results
    ...
    foreach ($arrResults as $hostIP=>$scanData) {
        ...
        $text_color, $line_color, $boarder_type, $row_height, $hostIP, $links_to_vulns, $chinese);
        $pdf->Ln();
    }
    $pdf->Ln();
} 
