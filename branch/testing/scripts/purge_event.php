...
if ( ($cid=="" && !file_exists($sid)) || ($sid=="" && $cid=="") ) {
	echo "Usage: php purge_event.php SID CID or php purge_event.php file (file includes all sid,cid)\n";
	exit;
}
//
// Load sid,cid to delete
//
$delete = array();
if (!file_exists($sid)) {
	$delete[] = array($sid,$cid);
} else {
	$lines = file($sid);
	foreach ($lines as $line) if (preg_match("/(\d+)\s*,\s*(\d+)/",$line,$fnd)) {
		$delete[] = array($fnd[1],$fnd[2]);
	}
}
// Deleting
foreach ($delete as $data) {
	...
	echo "Event SID:$sid, CID=$cid successfully deleted.\n";
}
