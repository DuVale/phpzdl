...
class Detector(threading.Thread):
    ...
    def __init__(self, conf, plugin, conn):
        ...
        self._correlationContextID=None
        self._sensorID=None
        if "cctx_id" in self._conf.hitems("plugin-defaults"):
            self._correlationContextID = self._conf.get("plugin-defaults", "cctx_id")
...
#        self.patternUTClocalized = re.compile('(?P<year>\d+)[\s-](?P<month>\d+)[\s-](?P<day>\d+)\s+(?P<hour>\d+):(?P<minute>\d+):(?P<second>\d+)(?P<tzone_symbol>[-|+])(?P<tzone_hour>\d{2}):(?P<tzone_min>\d{2})')
    ...
    def _plugin_defaults(self, event):
        ...
        if self._correlationContextID is not None:
            event['cctx_id'] = self._correlationContextID
        ...
        # 6) sensor,source ip and dest != localhost
            ...
            if event["src_ip"] in ('127.0.0.1', '127.0.1.1','::1') and 'src_ip' in event.EVENT_ATTRS:
            ...
            if event['src_ip'] is not None and 'src_ip' in event.EVENT_ATTRS:
                ...
                if not re.match(ipv4_reg, event['src_ip']) and not self.isIPV6(event['src_ip']):
                    ...
                    logger.warning("Event's field src_ip (%s) is not a valid IP.v4/IP.v6 address, set it to default ip 0.0.0.0 and real data on userdata8" % data)
            ...
            if event['dst_ip'] is not None and 'dst_ip' in event.EVENT_ATTRS:
                if not re.match(ipv4_reg, event['dst_ip']) and not self.isIPV6(event['dst_ip']):
                    data = event['dst_ip']
                    logger.warning("Event's field dst_ip (%s) is not a valid IP.v4/IP.v6 address, set it to default ip 0.0.0.0 and real data on userdata9" % data)
            ...
            if event['sensor'] is not None and not self.override_sensor:
                if not re.match(ipv4_reg, event['sensor'])  and not self.isIPV6(event['sensor']):
                    data = event['sensor']
                    logger.warning("Event's field sensor (%s) is not a valid IP.v4/IP.v6 address, set it to default local and real data on userdata7" % data)
