...
DATE_REGEXPS = [
    ...
    re.compile(r'(?P<month>\d+)/(?P<day>\d+)/(?P<year>\d{4})\,(?P<hour>\d+):(?P<minute>\d+):(?P<second>\d+)'),
...
def normalize_date(string, american_format=False):
    ...
    if dict.has_key('timestamp'):
        ...
        return datetime.datetime.strptime(dict['lilian'],"%y%j %H:%M:%S")
    # year
                        ...
                        if re.match(ipv4_reg, ip) and re.match(hostname_valid, hostname):
                            if HostResolv.HOST_RESOLV_DYNAMIC_CACHE.has_key(hostname):
                                if HostResolv.HOST_RESOLV_DYNAMIC_CACHE[hostname] != ip:
                                    ...
                                    logger.debug("Host/ip without changes %s->%s" % (hostname, ip))
                            else:
